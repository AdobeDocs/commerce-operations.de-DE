---
title: Erweiterte Installation vor Ort
...
# Erweiterte Installation vor Ort

>[!TIP]
>
>Verloren? Benötige eine helfende Hand? Probieren Sie unsere [Schnellstart-Installation](composer.md) oder [Mitarbeiter installieren](https://developer.adobe.com/commerce/contributor/guides/install/) Handbücher.
...
In diesem Thema wird die Installation der Adobe Commerce- oder Magento Open Source-Software mithilfe der CLI erläutert. Weitere Informationen zur Konfiguration finden Sie unter [Konfigurationshandbuch](../configuration/overview.md).
...
* Installieren von Adobe Commerce oder Magento Open Source in einer anderen Datenbankinstanz
...
* Überprüfen Sie die [Schnellstart der Installation](composer.md) Übersicht.

>[!NOTE]
>
>Sie müssen Adobe Commerce oder Magento Open Source über die `bin` -Unterverzeichnis.
...
* Installieren Sie Adobe Commerce oder Magento Open Source in einer anderen Datenbankinstanz.
...
| ------------------ | ------------------------------- |
| Sprache | bin/magento info:language:Liste |
| Währung | bin/magento info:currency:Liste |
| Zeitzone | bin/magento info:timezone:Liste |
...
| `--timezone` | Standardzeitzone zur Verwendung in der Admin- und Storefront. (Wenn Sie dies noch nicht getan haben, können Sie die Liste der Zeitzonen durch Eingabe von `bin/magento info:timezone:list` von `bin/` directory.) | Nein |
...
| `--cleanup-database` | Um Datenbanktabellen vor der Installation von Adobe Commerce oder Magento Open Source abzulegen, geben Sie diesen Parameter ohne Wert an. Andernfalls bleibt die Datenbank intakt. | Nein |
...
| Name | Wert | Erforderlich? |
|--- |--- |--- |
| `--search-engine` | Die Version von Elasticsearch oder OpenSearch, die als Suchmaschine verwendet werden soll. Mögliche Werte sind `elasticsearch7`, `elasticsearch6`und `elasticsearch5`. Der Standardwert ist `elasticsearch7`. Um OpenSearch zu verwenden, geben Sie `elasticsearch7`. Elasticsearch 5 ist veraltet und wird nicht empfohlen. | Nein |
...
| `--elasticsearch-index-prefix` | Ein Präfix, das den Suchmaschinenindex angibt. Der Standardwert ist `magento2`. | Nein |
...
| `--elasticsearch-username` | Die Benutzer-ID zur Authentifizierung der Suchmaschine | Nein, sofern die Authentifizierung nicht aktiviert ist |
| `--elasticsearch-password` | Das Kennwort zur Authentifizierung der Suchmaschine | Nein, sofern die Authentifizierung nicht aktiviert ist |
...
>[!NOTE]
>
>Informationen zum Aktivieren oder Deaktivieren von Modulen nach der Installation von Adobe Commerce oder Magento Open Source finden Sie unter [Module aktivieren und deaktivieren](tutorials/manage-modules.md).
...
Im folgenden Beispiel wird Adobe Commerce oder Magento Open Source mit den folgenden Optionen installiert:
...
* OpenSearch 1.2 ist auf installiert. `es-host.example.com` und Verbindungen an Port 9200
...
Sie können Adobe Commerce oder Magento Open Source installieren, ohne den Administratorbenutzer zu erstellen, wie im folgenden Beispiel gezeigt.
