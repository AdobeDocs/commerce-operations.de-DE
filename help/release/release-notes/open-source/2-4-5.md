...
* Sie können jetzt einen Daten-Patch umbenennen und den alten Klassennamen als Alias im `patch_list` Datenbanktabelle. Magento Open Source überprüft nun, ob bereits in der Datenbank vorhandene Datenpatchaliasen vorhanden sind, bevor der Patch angewendet wird. Zuvor hatte applicationOpen Source unter diesen Bedingungen einen Fehler ausgelöst.
...
* Inhalt des Warenkorbs und Anmeldestatus werden jetzt erwartungsgemäß neu geladen, nachdem die Sitzung beendet wurde. **Aktivieren der Persistenz** auf **Ja**. [GitHub-35182](https://github.com/magento/magento2/issues/35182)
...
* Der Mini-Warenkorb zeigt jetzt zuvor hinzugefügte Produkte nach einem Sitzungs-Timeout an, wenn **Aktivieren der Persistenz** aktiviert ist. [GitHub-35183](https://github.com/magento/magento2/issues/35183)
...
* Magento Open Source zeigt jetzt konfigurierbare Attribute wie bei der Erstellung eines globalen `select` -Attribute über ein Patch-Skript. Zuvor waren die zulässigen globalen Attribute ausgeblendet.
...
* Die `longblog` Datenbankdefinition zu `long blob`. [GitHub-35108](https://github.com/magento/magento2/issues/35108)
...
* Die Magento Open Source erzeugt keinen Fehler mehr, wenn Sie die  **Hier können Sie einen RSS-Feed mit Ihrer Wunschliste verknüpfen** Kontrollkästchen vor dem Klicken auf **Wunschliste freigeben**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* Die `products` -Abfrage gibt keine Attribute mehr als Aggregat zurück, wenn die **Verwendung in Navigation mit Ebenen von Suchergebnissen** ist deaktiviert. [GitHub-33318](https://github.com/magento/magento2/issues/33318)
...
* Die `getTypeID` -Funktion gibt nun die Produkttyp-ID und nicht die Produkt-ID zurück. [GitHub-35458](https://github.com/magento/magento2/issues/35458)

<!-- AC-2911-->

* jQuery UI-Regler und `SelectMenu` Die Zuordnung wurde in `vendor/magento/module-theme/view/base/requirejs-config.js`.

* Beobachter, die auf `sales_order_state_change_before` unterstützt jetzt das Abrufen von Daten aus dem Bestellobjekt. Die `sales_order_state_change_before` `event` -Argument wurde aktualisiert. [GitHub-26789](https://github.com/magento/magento2/issues/26789)
...
* Kunden-, Kunden- und Bestellaktionen werden jetzt korrekt im Admin-Aktionsbericht protokolliert. Zuvor hat die applicationOpen Source keine Aktionen protokolliert, wenn die `postDispatch` -Handler nicht in den Konfigurationseinstellungen angegeben.
...
* Magento Open Source zeigt jetzt Datensätze aus dem angeforderten Speicher auf der Raster-Seite der Credit Memos in Implementierungen mit PHP 7.4 an. Zuvor hatte applicationOpen Source den folgenden Fehler ausgegeben, nachdem Sie ein Kreditmemo erstellt und versucht haben, es anzuzeigen: `The store that was requested wasn't found. Verify the store and try again`.
...
* Die `increment_id` in der `sales_order` wurde erhöht. Zuvor waren Module von Drittanbietern, die davon ausgingen, dass `sales_order.increment_id` eine Länge von 50 Zeichen hatte, die nur die ersten 32 Zeichen einer `increment_id`. [GitHub-34521](https://github.com/magento/magento2/issues/34521)
...
* Elasticsearch-Abfragen funktionieren jetzt erwartungsgemäß, wenn `int` wird als durchsuchbares Backend konfiguriert `type` -Attribut. Zuvor gab die AnwendungOpen Source eine `Elasticsearch\Common\Exceptions\BadRequest400Exception` Ausnahmefehler.
...
* Magento Open Source zeigt nun eine genaue Anzahl von Suchergebnisvorschlägen in -Bereitstellungen an, in denen Suchvorschläge und die **Ergebnisanzahl für jeden Vorschlag anzeigen** -Einstellung aktiviert ist. Zuvor war die Anzahl, die neben den Keywords angezeigt wurde, null.
