...
* **Beschränkungen für die Anzahl der automatisch generierten Couponcodes**. Magento Open Source beschränkt jetzt die Anzahl der Gutscheincodes, die automatisch generiert werden. Der standardmäßige Maximalwert ist 250.000. Händler können die neue **[!UICONTROL Code Quantity Limit]**-Konfigurationsoption (**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Promotions]**) verwenden, um zu verhindern, dass das System mit vielen Coupons überfordert wird. <!-- AC-8753 -->
...
Magento Open Source 2.4.7 ist weiterhin nur zu Upgrade-Zwecken mit PHP 8.1 kompatibel. PHP 8.1 wird nicht unterstützt und nicht empfohlen. Magento Open Source 2.4.7 Kerncode, alle gebündelten Erweiterungen und alle Adobe-eigenen Erweiterungen und SaaS-Services sind mit PHP 8.3 kompatibel. <!-- AC-9668 -->
...
Alle JavaScript-Bibliotheken und NPM-Abhängigkeiten im Magento Open Source-Kerncode wurden auf die neuesten verfügbaren Versionen aktualisiert. Alle Laminas-Bibliotheksabhängigkeiten wurden auf die neueste Version aktualisiert, die mit PHP 8.3 kompatibel ist. <!-- AC-9667 9665 -->
...
Magento Open Source 2.4.7 umfasst die folgenden Verbesserungen der Leistung und Skalierbarkeit der Magento Open Source:
...
* **Kontroll-Webhooks** - Wenn ein Kunde eine Transaktion auf Braintree anficht, wird der Streitstatus jetzt an Commerce weitergegeben. Er ist im Raster **[!UICONTROL Sales]** > **[!UICONTROL Order]** durchsuchbar und an jede Bestellung angehängt.
...
PWA Studio v14.0 ist kompatibel mit Magento Open Source 2.4.7-beta1. Es enthält mehrere Verbesserungen zur Verbesserung der Barrierefreiheit. Informationen zu Fehlerbehebungen finden Sie unter [PWA Studio-Versionen](https://github.com/magento/pwa-studio/releases). Unter [Versionskompatibilität](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) finden Sie eine Liste der PWA Studio-Versionen und der kompatiblen Magento Open Source-Kernversionen.
...
Wir haben Hunderte von Problemen im Kern-Code von Magento Open Source 2.4.7 behoben. Nachfolgend werden einige der in dieser Version enthaltenen behobenen Probleme beschrieben.
...
* `bin/magento setup:install` wird jetzt erfolgreich abgeschlossen, nachdem `app/etc/config.php` gelöscht wurde. Zuvor wurde die fehlende Datei während der Installation nicht neu generiert, und bei der Magento Open Source ist ein Fehler aufgetreten. [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* Das Generieren eines Datenbank-Backups funktioniert jetzt sowohl von der Admin- als auch von der Befehlszeile erwartungsgemäß. Zuvor gab es in Magento Open Source diesen Fehler: `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`.
...
* Bundle-Produkte können jetzt mithilfe von SKUs mit einem Schrägstrich (`/`) erfolgreich zu einer Admin-Bestellung hinzugefügt werden. Zuvor konnten Admin-Benutzende diese Methode nicht verwenden, um Produkte zu einer Admin-Bestellung hinzuzufügen, und Magento Open Source gab einen JavaScript-Fehler aus.
...
* Magento Open Source zeigt jetzt eine informative Fehlermeldung an, wenn beim Checkout ein Fehler auftritt, und gibt den Einkäufer nicht mehr zu einem früheren Checkout-Schritt zurück. Zuvor wurde der Käufer auf die Versandseite zurückgeleitet.
...
* Sie können jetzt erfolgreich eine neue Warenkorb-Preisregel speichern, wobei die `Magento_OfflineShipping`-Erweiterung deaktiviert ist. Zuvor gab es in Magento Open Source diesen Fehler: `report.ERROR: Warning: Undefined array key "simple_free_shipping"`. [GitHub-37580](https://github.com/magento/magento2/issues/37580)
...
* Händler können jetzt das Produktkarussell verwenden, um CMS-Blöcke zu erstellen, wenn `catalog_product_price dimensions-mode` auf `website` festgelegt ist. Zuvor konnte Magento Open Source den Baustein nicht speichern und gab folgenden SQL-Fehler aus: `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.
...
* Magento Open Source fügt keinen Produktbeschreibungstext mehr in das Feld **[!UICONTROL Meta Description]** der Produktdetailseite ein, wenn dieses Feld leer gelassen wurde. Wenn eine Produktbeschreibung zuvor HTML-Code enthielt, der von Page Builder oder einem anderen HTML-Editor generiert wurde, wurde die HTML im Feld Meta-**[!UICONTROL Description]** angezeigt, und Sie konnten dieses Feld nicht leer lassen.
...
* Der REST-`V1/products/<sku>/media`-Endpunkt kann jetzt das gleichzeitige Hochladen mehrerer Medienbilder verarbeiten. Zuvor wurden bei der gleichzeitigen Verarbeitung mehrerer Anfragen inkonsistente Daten erstellt, und beim Speichern dieser Daten in der Magento Open Source wurde ein Fehler ausgegeben.
...
* Händler können jetzt vom Administrator aus ein GIF-Bild zur Bildergalerie eines Produkts hinzufügen. Zuvor hat Magento Open Source diesen Fehler angezeigt: `imagecolorsforindex(): Argument #2 ($color) is out of range`.
...
* Die Magento Open Source gibt jetzt ein gültiges Ergebnis für die benutzerdefinierte `01/01/1970` zurück. Zuvor hat das benutzerdefinierte Attribut folgenden Fehler zurückgegeben: `Invalid input datetime format of value '1/01/1970`. [GitHub-37274](https://github.com/magento/magento2/issues/37274)
...
* Der Warteschlangennachrichten-Validator validiert jetzt den Untertyp aller Array-Elemente. Zuvor gab es bei Magento Open Source eine Ausnahme.
...
* Bei Anfragen für einen RSS-Feed für Kategorien treten bei aktivierter **[!UICONTROL RSS Feed Top Level Category]** keine Fehler mehr auf. Zuvor führte das Durchsuchen der Seite mit der RSS-Feed-Kategorie zu Elasticsearch-`CRITICAL`-Fehlern in Protokolldateien, wenn **[!UICONTROL RSS Feed Top Level Category]** aktiviert wurde.
...
* Wenn Sie auf die Schaltfläche **[!UICONTROL Send invitation]** (Admin **[!UICONTROL Marketing]** > **[!UICONTROL Private sales]** > **[!UICONTROL Invitation]**) klicken, wird jetzt zusammen mit dem Formularschlüssel eine Anfrage zur POST gesendet und die Einladung erfolgreich gesendet. Wenn Sie diese Schaltfläche zuvor angeklickt haben, waren weder die HTTP-POST-Methode noch der Formularschlüssel vorhanden.
...
* Magento Open Source berücksichtigt jetzt die **[!UICONTROL Disable Automatic Group Change Based on VAT ID]** beim Import von Kundendatensätzen. Zuvor wurde der `disable_auto_group_change` Wert in den Importdateien ignoriert. [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* Der Produktimport wird nicht mehr unterbrochen, wenn Produktbilder fehlen. Zuvor wurde bei einem Produktbild, das beim Import eines Drittanbieterdienstes mithilfe einer CSV-Datei fehlte, in Magento Open Source folgender Fehler angezeigt: `Maximum error count has been reached or a system error is occurred!`.
...
* Magento Open Source enthält nicht mehr den vollständigen Pfad zum Stammverzeichnis des Servers in der Fehlermeldung, die angezeigt wird, wenn eine fehlerhafte HTTP-Anforderung an den Server gesendet wurde. Als zweites Argument werden jetzt nur noch Zeichenfolgen an die `explode`-Methode übergeben.
...
* Händler können jetzt eine Gutschrift für Bestellungen erstellen, aus denen ein einfaches Produkt gelöscht wurde, das mit einem konfigurierbaren Produkt verknüpft ist. Zuvor konnten Händler die Gutschrift nicht erstellen, und Magento Open Source gab eine `TypeError` Ausnahme aus.
...
* Magento Open Source gibt nach dem Neuladen der Seite keinen Fehler mehr aus, wenn ein Einkäufer eine Produktmenge von einer Bestellung abzieht, die in den verfügbaren Produktbereich fällt, während er die Versandquelle auswählt.
...
* Magento Open Source wendet jetzt den richtigen Rabattbetrag auf teilweise stornierte Bestellungen an. Zuvor wurde der Rabattbetrag, wie er in der Spalte `base_discount_cancelled` berechnet wurde, bei einer Änderung eines Auftrags falsch berechnet. Die Unterstützung für negative Werte wurde den Rechnungen für reduzierte Bestellungen hinzugefügt.
...
* Die Seite „Admin-Kundenauftragsraster“ wird jetzt erwartungsgemäß geladen, wenn ein Händler eine Suche auf dieser Seite initiiert. Magento Open Source Wenn ein Händler zuvor versucht hat, Bestellungen aus dieser Tabelle zu durchsuchen, friert die Seite ein, und in der Browser-Konsole wird ein `Request-URI Too Long`-Fehler angezeigt.
...
* Käufer können jetzt eine Bestellung, die an mehrere Adressen versendet wird, mit einer Braintree-Zahlungsmethode auschecken, wenn der Tresor für Kartenzahlungen deaktiviert ist. [GitHub-37615](https://github.com/magento/magento2/issues/37615)
...
* Magento Open Source gibt bei der Erstellung einer zweiten Teilrechnung für eine per Braintree bezahlte Bestellung keinen Fehler mehr aus, wenn die **[!UICONTROL Enable Vault for Card Payments]** aktiviert ist. Zuvor gab es bei dem Versuch eines Händlers, eine zweite Rechnung zu erstellen, folgenden Fehler von Magento Open Source: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* Käufer können jetzt den Checkout mit Braintree Payments und Magento Open Source 2.4.6 mit unsichtbarem ReCAPTCHA v2 oder v3 abschließen. [GitHub-37241](https://github.com/magento/magento2/issues/37241)
...
* Durch mehrere Klicks im **Verkauf** > **Bestellungen** Dropdown-Menü für Massenaktionen werden keine Anforderungen für mehrere POST mehr generiert.  [GitHub-37997](https://github.com/magento/magento2/issues/37997)
...
* Die Leistung der `addProductToCart`-Mutation bei der Ausführung mit vielen benutzerdefinierten Produktattributen wurde verbessert. Zuvor hat Magento Open Source, als diese Mutation mit vielen Produktattributen ( ca. 1.000) ausgeführt wurde, versucht, alle Produktattribute zu laden.
...
* Die Magento Open Source wechselt nun wie erwartet auf eine neue Lieferseite, nachdem eine Teillieferung für eine Bestellung erstellt wurde. Zuvor hat Magento Open Source nach der Erstellung einer Teillieferung keine vollständige Sendungsseite für die restlichen Produkte der Bestellung erstellt. [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* Magento Open Source zeigt jetzt die korrekten Preise für Bundle-Produkte mit Sonderpreisen an.
...
* Magento Open Source gibt keinen Fehler mehr aus, wenn Sie versuchen, auf der Admin-Bestellseite schnell zwischen „Kommentar-Verlauf“ und „Rückgabe“ zu wechseln. Zuvor zeigte Magento Open Source diesen allgemeinen Fehler an: `A technical problem with the server created an error`.
...
* Fehlermeldungen im Zusammenhang mit dem Senden von Kommentaren auf der Seite „Bestelldetails“ sind jetzt informativer. Wenn ein Käufer zuvor auf **[!UICONTROL Submit comment]** geklickt hat, ohne den Bestellstatus zu ändern oder Text in den Kommentarbereich einzugeben, hat Magento Open Source diese Meldung angezeigt: `The comment is missing. Enter and try again`.

<!-- ACP2E-1659 -->

* Das Kalendersymbol wird nun wie erwartet in den Datumsbereichsfeldern unter Admin **[!UICONTROL Marketing]** > **[!UICONTROL Cart Price Rules]** angezeigt, nachdem ein Datum für ein Feld ausgewählt wurde. Nachdem Sie zuvor ein Datum für ein Feld ausgewählt haben, wurde das Kalendersymbol nicht mehr in anderen Feldern angezeigt, und die Magento Open Source zeigte Platzhaltertext an. [GitHub-36499](https://github.com/magento/magento2/issues/36499)
...
* Magento Open Source generiert keine Produkt-URL-Neuschreibungen mehr für Stammkategorien und für Produkte, die in Setups mit mehreren Stores auf **[!UICONTROL Not Visible Individually]** eingestellt sind, wenn fehlende URL-Neuschreibungen für benutzerdefinierte Stores erzeugt werden.
...
* Magento Open Source zeigt in Admin **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** keine `Integration not secure` mehr an, wenn die **[!UICONTROL Callback URL]**- und **[!UICONTROL Identity link URL]** leer sind. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* Die REST-`V1/carts/mine/estimate-shipping-methods`-Methode validiert jetzt alle Ebenen der Payload-Felddaten und gibt einen 400-HTTP-Code für ungültige Daten zurück. Zuvor wurde nur die erste Ebene der API-Payload validiert, und wenn ungültige Daten in nicht validierten Ebenen der Payload zurückgegeben wurden, hat Magento Open Source einen Status-Code von 500 zurückgegeben.
...
* Magento Open Source gibt jetzt erfolgreich das `content`-Attribut (base64-Bildcode) in `GET V1/products/sku/media` Produktmedien-API-Antworten zurück. Das `content`-Attribut war zuvor nicht in der Antwort enthalten.
...
Sie können Magento Open Source 2.4.7 mit dem [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html) installieren.
