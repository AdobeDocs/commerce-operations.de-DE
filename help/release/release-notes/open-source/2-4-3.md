...
description: Erfahren Sie mehr über neue Funktionen, Verbesserungen, Fehlerbehebungen und bekannte Probleme in der Magento Open Source-Version 2.4.3.
...
Magento Open Source 2.4.3 bietet verbesserte Leistung und Sicherheit sowie wesentliche Plattformverbesserungen. Zu den Sicherheitsverbesserungen gehören die Erweiterung der reCAPTCHA-Abdeckung und die Einbeziehung der integrierten Ratenbegrenzung. Kernkomponentenabhängigkeiten und Drittanbieter-Bibliotheken wurden auf die neuesten Versionen aktualisiert, die mit PHP 8.x kompatibel sind. Page Builder ist jetzt als gebündelte Erweiterung in Magento Open Source verfügbar. Es ist jetzt das Standard-Tool zur Inhaltsbearbeitung für Adobe Commerce und Magento Open Source.

Diese Version enthält über 370 neue Fehlerbehebungen im Kerncode und 33 Sicherheitsverbesserungen. Dazu gehört die Lösung von fast 290 GitHub-Problemen durch unsere Community-Mitglieder. Diese Community-Beiträge reichen von der geringfügigen Bereinigung des Kerncodes bis hin zu wesentlichen Verbesserungen in GraphQL.

Diese Version enthält über 370 neue Fehlerbehebungen im Kerncode und 33 Sicherheitsverbesserungen. Alle bekannten Probleme, die in den Versionshinweisen zu Magento Open Source 2.4.2 identifiziert wurden, wurden in dieser Version behoben.
...
Der folgende schwerwiegende Fehler kann während der Aktualisierung auf Magento Open Source 2.4.3 auftreten:

```terminal
PHP Fatal error: Uncaught Error: Call to undefined function Magento\Framework\Filesystem\Directory\str_contains() in [...]/magento/vendor/magento/framework/Filesystem/Directory/DenyListPathValidator.php:74
```

Dieser Fehler resultiert aus der Verwendung des `str_contains` -Funktion, die eine PHP 8.x-Funktion ist. Die Anwendung Open Source 2.4.3 unterstützt PHP 8.x nicht. Dieses Hotfix ersetzt diese Funktion durch eine unterstützte PHP 7.x-Funktion. Siehe [Adobe Commerce-Upgrade 2.4.3, 2.3.7-p1 PHP Schwerwiegender Fehler Hotfix](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix) Knowledge Base-Artikel.
...
* Magento Open Source 2.4.3 wurde getestet und bestätigt, dass es mit Redis 6.0.12 kompatibel ist. (Version 2.4.x ist weiterhin mit Redis 5.x kompatibel.)

* Die Abhängigkeiten der Laminas-Bibliothek wurden auf PHP 8.x-kompatible Versionen aktualisiert. Einige redundante Abhängigkeiten wurden aus der `composer.json` -Datei. **Magento Open Source 2.4.3 verwendet Laminas 3.4.0**. <!-- MC-39513-->
...
* Die `updateProductsInWishlist` Mutation aktualisiert jetzt erfolgreich Elemente, die zu einem Bundle-Produkt gehören, in einer Wunschliste. Zuvor hat diese Mutation das Element &quot;wishlist&quot;nicht aktualisiert, sondern das Element gelöscht und ein neues erstellt, durch das die Element-ID geändert wurde.
...
* Magento Open Source aktualisiert die Gesamtanzahl der Seiten wie erwartet, wenn Sie den Seitenwert pro Seite der Liste &quot;Admin Related Products, Up-Sells and Cross-Sells&quot;ändern. [GitHub-31059](https://github.com/magento/magento2/issues/31059)
...
* Plug-ins für globale Magento Open Sourcen (z. B. `webapi_rest` und `graphql` ) nicht mehr für einen neuen benutzerdefinierten Bereichstyp ausgelöst werden, wenn die Variable `di.xml` enthält keine registrierten Plugins.
...
* Die Leistung der  `catalog_product_alert` Der Cron-Prozess bei Ausführung auf großen Tabellen (mehrere Millionen Zeilen) wurde verbessert. Zuvor `catalog_product_alert` alle Produktwarnungen geladen haben, was zu einer Speicherausnahme geführt hat.
...
* Administratoren, die Safari verwenden, können jetzt ein Produkt erfolgreich einer Bestellung vom Administrator hinzufügen. Zuvor, als der Administrator auf die **Ausgewählte Produkte zur Bestellung hinzufügen** -Schaltfläche, zeigte die Anwendung das Drehlastsymbol an und die Seite hängte an.
...
* Sie können jetzt ein benutzerdefiniertes Elasticsearch-Feld-Mapper zu `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`.

<!-- ENGCOM-9029 MC-24026-->

* Entwickler können jetzt Elasticsearch-Mappings ändern. Zuvor waren dynamische Vorlagen, die in den Standardzuordnungen an den Elasticsearch-Server gesendet wurden, hartcodiert, was Entwicklern die Indizierung zusätzlicher Daten, die mit einem benutzerdefinierten  verknüpft sind, in Elasticsearch verhinderte.

<!-- ENGCOM-8464 -->

* Suchfelder (Formular-minisearch) funktionieren jetzt erwartungsgemäß, wenn Suchvorschläge deaktiviert sind.
...
* Admin-Benutzer, die auf eine bestimmte Website beschränkt sind, können jetzt eine Sendung für eine Bestellung erstellen, die auf derselben Website platziert wird. Zuvor hat die Anwendung diese Ausnahme ausgelöst, wenn ein Admin-Benutzer ohne Berechtigung für die standardmäßige Store-Ansicht versucht hat, eine Bestellung zu versenden, die in einer Store-Ansicht platziert wurde, auf die der Admin-Benutzer Zugriff hatte: `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* Die Anwendung hebt die Überprüfung der **Kommentare anhängen** Kontrollkästchen aktivieren, wenn ein Kunde klickt **Versandmethoden und -preise abrufen** und wählt beim Erstellen einer Bestellung durch den Administrator eine Versandmethode aus.
...
* Die Produktmenge bleibt jetzt wie erwartet unverändert, nachdem ein Käufer die Menge auf der Seite &quot;Versandadresse&quot;auf die Seite mit mehreren Adressen ändert und auf die Schaltfläche &quot;Zurück&quot;des Browsers klickt.
...
* Die Anwendung gibt keinen Fehler mehr aus, wenn ein Händler versucht, eine Bestellung mit DHL zu versenden, wenn der **Versandtitel erstellen** aktiviert ist und der Produktname Unicode-Zeichen enthält. Zuvor wurde in der Anwendung bei der Anforderung der Beschriftungserstellung dieser Fehler angezeigt: `The response is in wrong format`.
...
* Das Programm zeigt jetzt eine **MwSt.-Nummer** auf der Seite zur Kundenregistrierung ein, wenn `customer/create_account/vat_frontend_visibility` aktiviert ist.
...
* Entfernt `CacheCleaner::cleanAll();` aus Integrationstests.

<!-- ENGCOM-8898-->

* Es wurde ein Test für dieses Szenario hinzugefügt: Administratoren können ein Kundenkonto bearbeiten, wenn der Kunde einen Newsletter in der Warteschlange abonniert.

<!-- ENGCOM-8484-->

* Entfernt die `cache:flush` -Befehl aus Tests zur Verbesserung der Ausführungszeit für `Catalog`, `CatalogUrlRewrite`und `LoginAsCustomer` Module.
...
* Das Programm zeigt jetzt Seitenelemente konsistent auf Storefront-Seiten an, die standardmäßige Magento Open Sourcen-Designs verwenden. Zuvor wurden nicht alle Stile im leeren Design angewendet, sodass nicht alle Seitenelemente auf Seiten mit diesem Design angezeigt wurden. (Beispielsweise war im Suchfeld Meine Bestellungen kein Lupensymbol vorhanden.)

### Übersetzung und Gebietsschemata

<!-- MC-23553-->

* Klammern, die zu Zeichenfolgen hinzugefügt werden, werden nicht mehr maskiert, wenn die Inline-Übersetzung konfiguriert ist. Zuvor wurde die Variable `escapeHtmlAttr` -Methode konvertierte die Klammern in HTML-Entitätscodes.
...
* Textzeichenfolgen in der Vorlage, die zur Verwaltung gespeicherter Zahlungsmethoden verwendet werden (**Mein Konto**  >  **Gespeicherte Zahlungsmethoden** ) jetzt übersetzt werden können.

<!-- MC-30798 ENGCOM-8717-->

* Bestellungsnachrichten, die vom Admin gesendet werden, verwenden jetzt das Gebietsschema store und nicht das Gebietsschema, das mit dem Administratorkonto verknüpft ist.
...
* Die Seite zur Bestellüberprüfung, die beim Checkout mit PayPal Express Checkout angezeigt wird, wird jetzt erfolgreich geladen. Zuvor enthielt die Vorlage die nicht verwendeten **Update der Versandmethode** -Schaltfläche, die beim Rendern der Seite nur teilweise vom Skript ausgeblendet wurde.

<!-- MC-37941-->

* Bewertungssterne und Überprüfungstext im Abschnitt &quot;Kundenüberprüfungen&quot;der Produktseite haben jetzt den richtigen Abstand, wenn längere Bewertungsnamen vorhanden sind.
...
* Das Programm verwendet jetzt beim Generieren von E-Mails für neue Admin-Benutzer die richtige benutzerdefinierte E-Mail-Vorlage. Zuvor verwendete die Anwendung die alte Standardvorlage, in der die Vor- und Nachnamen des Administrators weggelassen wurden.
...
Sie können Magento Open Source 2.4.3 mit [Verfasser](../../../installation/composer.md).
