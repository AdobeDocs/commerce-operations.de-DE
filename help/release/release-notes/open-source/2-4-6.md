...
Magento Open Source 2.4.6 führt die Unterstützung für PHP 8.2 ein. PHP 8.1 bleibt vollständig unterstützt, aber die Unterstützung für PHP 7.4 wurde entfernt. Sie umfasst erhebliche Verbesserungen der Leistung und Skalierbarkeit. GraphQL-Vorgänge für Massenkorb-Vorgänge und die Antwort auf das Rendern von Kategoriestrukturen wurden optimiert.
...
Magento Open Source 2.4.6 führt die Unterstützung für PHP 8.2 ein. PHP 8.1 wird weiterhin vollständig unterstützt. <!-- AC-6598 -->
...
Die neue `adobe-ims-metapackage` Version 2.2.0 ist mit Adobe Commerce 2.4.6 und Magento Open Source 2.4.6 integriert. In dieser Version wurde die Funktionalität nicht geändert.<!-- CABPI-505-->
...
**Problem**: New Relic ist derzeit nicht mit PHP 8.2 kompatibel. Bereitstellungen, die New Relic ausführen, können Adobe Commerce 2.4.6 auf PHP 8.1, aber nicht auf PHP 8.2 ausführen. **Problemumgehung**: Führen Sie Adobe Commerce 2.4.6 auf PHP 8.1 aus, bis eine neue Version von New Relic veröffentlicht wird, die PHP 8.2 unterstützt. Diese Version wird voraussichtlich im Sommer 2023 veröffentlicht. <!-- AC-7769-->
...
* Die `bin/magento setup:db:status` -Befehl schlägt in Umgebungen mit MySQL 8.0.29 nicht mehr fehl. Zuvor wurde in der Befehlsausgabe fälschlicherweise angegeben, dass das deklarative Schema veraltet war. [GitHub-35671](https://github.com/magento/magento2/issues/35671)
...
* Google reCAPTCHA funktioniert jetzt bei Storefront-Bestellungen wie erwartet. Bisher wurden in Magento Open Source diese Fehler angezeigt, wenn Google ReCAPTCHA nicht zuerst ausgewählt wurde, bevor ein Käufer zum ersten Mal versuchte, eine Bestellung aufzugeben: `ReCaptcha validation failed, please try again` und `No such cart with id = ID`.
...
* AMQP-Verbraucher schlagen nicht mehr fehl, da die `trim()` -Funktion auf Bereitstellungen mit PHP 8.1. Zuvor, als der AMQP-Kunde nach der Installation startete, löste Magento Open Source diesen Fehler aus: `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* Die `getFilteredTimerIds` -Methode funktioniert jetzt wie erwartet.  Klasse `Magento\Framework\Profiler\Driver\Standard\Stat` ist jetzt mit PHP 8.1 kompatibel. Zuvor hatte Magento Open Source eine Ausnahme ausgelöst, wenn diese Methode aufgerufen wurde. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Die Magento Open Source zeigt jetzt einen genauen Zeitbereich an, in dem ein Admin-Benutzer die **[!UICONTROL Last 24 Hours]** im Dashboard Admin-Statistiken . Zuvor wurde in Magento Open Source ein falscher Zeitraum angezeigt. Dies trat auf, weil die Zeitzone der Datenbank unverändert blieb.
...
* Magento Open Source kann nun in Implementierungen mit PHP 8.1 steuerpflichtige gruppierte Produkte auf der Storefront-Kategorieseite anzeigen. Zuvor zeigte Magento Open Source keine Produktliste an und gab diese Ausnahme zurück: `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`. [GitHub-35500](https://github.com/magento/magento2/issues/35500)
