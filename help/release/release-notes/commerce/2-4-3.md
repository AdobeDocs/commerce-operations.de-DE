...
Weitere Informationen zu Adobe Commerce 2.4.2-p2 finden Sie ](../security/2-4-2-patches.md) den Versionshinweisen zu Adobe Commerce 2.4.2-p2.[
...
[Live Search](https://experienceleague.adobe.com/en/docs/commerce/live-search/overview) unterstützt von [Adobe AI](https://business.adobe.com/ai.html) bietet ein intuitives Sucherlebnis, indem Algorithmen für künstliche Intelligenz und maschinelles Lernen verwendet werden, um eine umfassende Analyse aggregierter Besucherdaten durchzuführen. Siehe [Versionshinweise zur Live Search](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html).
...
Mit Version 2.4.3 wird die B2B-Version 1.3.2 eingeführt. Diese Version enthält mehrere Fehlerbehebungen. Siehe [B2B-](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* Administratoren können sich jetzt erfolgreich bei einer -Bereitstellung anmelden, wenn die Anwendung entweder mit der Option `—use-rewrites=0` oder in `core_config_data_table` mit `web/seo/use_rewrites` auf 0 installiert wurde. [GitHub-32100](https://github.com/magento/magento2/issues/32100)
...
* Die Sortierung nach Position bei der Produktsuche mithilfe von GET `/rest/V1/products/?searchCriteria[filterGroups]` funktioniert jetzt erwartungsgemäß. Zuvor hatte die Produktsammlung keinen Feld- `position` Sortierwert. [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* Die Verwendung von `bin/magento setup:db-schema:split-sales` zum Verschieben von Datenbanktabellen funktioniert jetzt erwartungsgemäß. Tabellen werden jetzt korrekt erstellt und mit Daten ausgefüllt. Da die Prüfung auf Tabellenexistenz falsch war, hat die Anwendung beim Ausführen von `bin/magento setup:db-schema:split-sales` zuvor `SQLSTATE` Fehler ausgegeben.
...
* Produkte sind jetzt wie erwartet in den Suchergebnissen der Storefront verfügbar, wenn Produkte über eine REST PUT `/V1/products/:sku/links`-Anfrage verknüpft werden und der Indexermodus auf &quot;**beim Speichern“** ist.
...
* Seiten-Layouts sind in `Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\Container` nicht mehr hartcodiert.  Infolgedessen gibt die Funktion `getPageLayouts()` jetzt die tatsächliche Liste der Seiten-Layouts zurück, die von den verschiedenen Modulen erwartungsgemäß deklariert wurden. Zuvor wurden nur hartcodierte Layouts zurückgegeben. [GitHub-31168](https://github.com/magento/magento2/issues/31168)
...
* Die Anwendung gibt keinen Fehler mehr aus, wenn ein Plug-in zu einer übergeordneten Klasse hinzugefügt wird. (Der `optionsProvider` Parameter wird jetzt nach der `getContentIdentities` in `Assest.php` deklariert.) Zuvor gab die Anwendung den folgenden Fehler aus: `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* Das `ArrayIterator` PHP-Objekt wurde aktualisiert, damit es mit PHP 7.4 wie erwartet funktioniert. [GitHub-32088](https://github.com/magento/magento2/issues/32088)
...
* Der REST-Aufruf GET `/V1/customers/search` gibt jetzt korrekte Informationen für Kunden zurück, die mehrere Newsletter abonniert haben. [GitHub-31168](https://github.com/magento/magento2/issues/31168)
...
* Redundante Parameter wurden entfernt und in POST geändert, sodass sie bei Bedarf in `\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping` abgerufen werden können.
