...
**Problem**: Die Infrastrukturelemente von RabbitMQ (Austausche, Warteschlangen) werden möglicherweise nach einer Neuinstallation (nicht Aktualisierung) von Adobe Commerce 2.4.5 nicht initialisiert. Diese falsche Konfiguration von RabbitMQ kann zu einem schwerwiegenden Fehler führen, wenn asynchrone Vorgänge ausgeführt oder ein Leistungsprofil generiert wird. Cloud-Implementierungen sind nicht betroffen. **Problemumgehung**: Händler, die eine Neuinstallation von Adobe Commerce 2.4.5 vor Ort durchführen, sollten `bin/magento setup:upgrade --keep-generated` nach der Verwendung von `bin/magento setup:config:set` zur Konfiguration von RabbitMQ oder zur Installation von Adobe Commerce durch Übergabe der RabbitMQ-Konfigurationsoptionen an die `setup:install` Befehl. Siehe [RabbitMQ](../../../installation/prerequisites/rabbitmq.md).
