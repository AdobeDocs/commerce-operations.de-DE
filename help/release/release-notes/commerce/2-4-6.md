...
>[!NOTE]
>
>Die Versionshinweise für Beta-Versionen sind unvollständig und können von GA geändert werden.
...
## 2.4.6 Beta-Testumgebung

Die Beta-Version von Adobe Commerce 2.4.6 wurde mit den folgenden Komponentenversionen getestet:

* Verfasser 2.2
* Elasticsearch 7.17/8.4
* Öffnen Sie Search 2.4.
* MariaDB 10.6
* MySQL 8.0
* PHP 8.2
* RabbitMQ 3.9
...
Diese Version umfasst acht Sicherheitsverbesserungen und Verbesserungen der Plattformsicherheit. Diese Sicherheitskorrekturen wurden auf Adobe Commerce 2.4.5-p2 und Adobe Commerce 2.4.4-p3 zurückportiert.
...
Adobe Commerce 2.4.6 bietet Unterstützung für PHP 8.2. PHP 8.1 wird weiterhin vollständig unterstützt. <!-- AC-6598 -->

Die Unterstützung für PHP 7.4 wurde aus dieser Version entfernt. Sie können Adobe Commerce nicht auf PHP 7.4 ausführen.

Adobe Commerce 2.4.6-Beta5 unterstützt jetzt:
...
* Aktualisiert `jQuery/fileUpload` -Bibliothek auf Version 10.32. <!-- AC-2658-->

* Komponenten des Zend-Frameworks (ZF1), die das Ende des Lebenszyklus erreicht haben, wurden aus der Codebasis entfernt. Siehe [Rückwärtskompatible Änderungen - Highlights](https://developer-stage.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->

* Laminas-Abhängigkeiten wurden aktualisiert, um PHP 8.2 zu unterstützen. <!-- AC-7019-->

* Die `laminas/laminas-dependency-plugin` Das Composer-Plug-in wurde entfernt. [GitHub-36515](https://github.com/magento/magento2/issues/36515) <!-- AC-7100-->

* Ersetzt `Zend_Filter` mit `laminas/laminas-filter`. <!-- AC-5922-->

* Aktualisiert `web-token/jwt-framework` auf Version 3.0.5. (neueste Version) <!-- AC-3772-->

* Aktualisiert `allure-framework/allure-phpunit` auf v2.x. <!-- AC-2660-->

* Die `Chart-js` Bibliothek zu v3.9.1 <!-- AC-2661-->


### Leistungsverbesserungen und Verbesserungen der Skalierbarkeit

* **Verbesserte Leistung bei Vorgängen, die viele Kundensegmente betreffen**. Merchants können die Leistung von Vorgängen mit mehr als 100 Kundensegmenten verbessern, indem sie eine neue Konfigurationseinstellung aktivieren. Die **Echtzeitüberprüfung, ob der Kunde durch Segment übereinstimmt** kann bei Bedarf deaktiviert werden, um die Verarbeitungszeit zu verkürzen, indem die Validierung jedes Kundensegments gegenüber Kunden verringert wird. Siehe [Validierung von Kundensegmenten](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation). <!-- AC-6832 ACPT-835-->

* **Neue Systemkonfigurationseinstellung zur Begrenzung der Anzahl der im Produktraster angezeigten Produkte**. Händler können die neue **Anzahl der Produkte im Raster begrenzen** Konfigurationseinstellungen zur Verbesserung der Leistung des Produktrasters. Diese Beschränkung ist standardmäßig auf 20000 konfiguriert und betrifft nur Produktsammlungen, die von UI-Komponenten verwendet werden. Durch die Begrenzung der Anzahl angezeigter Produkte kann die Leistung von Vorgängen für Raster mit mehr als 200.000 Produkten gesteigert werden. Siehe [Anzahl der Produkte im Raster begrenzen](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->

* **Verbesserte Importleistung (bis zu 100.000 Datensätze pro Minute)**. Händler können die neue `POST /rest/<store_view_code>/V1/import/csv` REST-API-Endpunkt zum Importieren von Daten in Adobe Commerce. Dieser Endpunkt bietet dieselben Funktionen wie die Admin-Importfunktion und unterstützt die Verwendung einer CSV-Datei zum Erstellen, Aktualisieren und Entfernen von Produkten, Produktpreisen und Kundenentitäten. <!-- ACPT-6 -->
...
Die neue `adobe-ims-metapackage` Version 2.2.0 ist im Lieferumfang von Adobe Commerce 2.4.6 enthalten. Die Funktionalität wurde in dieser Version nicht geändert.<!-- CABPI-505-->
...
Diese Version enthält mehrere Fehlerbehebungen. Die Beta-Versionshinweise für B2B sind in diesem Thema enthalten. Sie finden die Adobe Commerce 2.4.6 GA-Versionshinweise für B2B in [B2B-Versionshinweise](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
**Produktberechtigungen wurden optimiert** mit Inline-Produktfragmenten. <!-- B2B-2459-->

**Kundenabfrage unterstützt jetzt Sortierung von Bestellungen**. Die `orders` output -Objekt der `customer` -Abfrage enthält jetzt ein Sortierfeld. Sie können Bestellungen nun nach Bestellnummer oder Erstellungsdatum und in auf- oder absteigender Reihenfolge sortieren. <!-- AC-6903-->
...
Hunderte von Problemen werden im Adobe Commerce 2.4.6-Kerncode behoben. Nachfolgend wird eine Teilmenge der in der Beta-Version von Adobe Commerce 2.4.6 enthaltenen behobenen Probleme beschrieben.
...
* Gesamtzahl der in den Suchergebnissen gefundenen Elemente <!--- AC-3522-->
* Name des angemeldeten Benutzers, wenn sich ein Benutzer erfolgreich mit Tastaturbefehlen anmeldet <!--- AC-6477-->
* Die Kopfzeile der Produktergebnisseite liest jetzt den richtigen Wert für die `<title>` element <!--- AC-2489-->
...
* Wenn der Seiteninhalt aktualisiert wurde, nachdem der Benutzer auf die **[!UICONTROL Create an account]** button <!---AC-6676-->

### B2B

Diese Version enthält mehrere Fehlerbehebungen. Die Beta-Versionshinweise für B2B sind hier enthalten. Sie finden die Adobe Commerce 2.4.6 GA-Versionshinweise für B2B in [B2B-Versionshinweise](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* Der Filter Meine Anführungszeichen anzeigen auf der Seite Meine Anführungszeichen funktioniert jetzt wie erwartet. Zuvor enthielten die Filterergebnisse die Anführungszeichen anderer Admin-Benutzer.
...
* Es wurden Leistungsprobleme mit der Validierung von Unternehmensberechtigungen behoben, die über GraphQL bereitgestellt wurden, wenn bereits viele Unternehmensberechtigungen zugewiesen wurden.
...
* Produkte, die nicht Kategorien zugewiesen sind, aber Produktoptionen für Bundle oder gruppierte Produkte sind, werden jetzt in den Antworten zurückgegeben, wenn die Variable **Kategoriedurchsuchen zulassen** Die Konfigurationseinstellung ist für den Kunden aktiviert, der die Abfrage sendet.
...
* Der exportierte Wert von **Maximale im Warenkorb zulässige Menge** entspricht nun dem in der Admin-Konfigurationseinstellung festgelegten Standardwert.
...
* Warenkorbregeln, die Anforderungen an die konfigurierte Produktmenge enthalten, funktionieren jetzt erwartungsgemäß.Die `attribute_scope` für `quote_item_qty` -Bedingung wurde geändert in `parent`. [GitHub-35981](https://github.com/magento/magento2/issues/35981)
...
* Die Lagerbestandsanzahl der Storefront ist jetzt genau, wenn die Variable **Nicht vorrätige Produkte anzeigen** -Konfigurationseinstellung aktiviert ist. [GitHub-35899](https://github.com/magento/magento2/issues/35899)

<!-- AC-6384-->

* Die Kategorieseite ist nicht mehr leer, wenn die Variable **Alle Produkte pro Seite zulassen** -Einstellung aktiviert ist. [GitHub-35900](https://github.com/magento/magento2/issues/35900)

<!-- AC-6330-->

* Die **Nicht vorrätige Produkte anzeigen** -Konfigurationsoption funktioniert jetzt wie erwartet, wenn sie aktiviert ist. Zuvor wurden alle Produkte als nicht vorrätig im Storefront angezeigt, wenn diese Option in der Admin-Konsole aktiviert war. [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* Einfache, nicht vorrätige Produkte, die Optionen für ein konfigurierbares Produkt sind, werden jetzt in der Storefront als deaktiviert angezeigt, wenn **Nicht vorrätige Produkte anzeigen** auf **[!UICONTROL Yes]**. Zuvor wurden alle Optionen trotz ihres Status und ihrer tatsächlichen Verfügbarkeit als verfügbar angezeigt.
...
* Konfigurierbare Produktoptionen werden jetzt wie erwartet auf der Storefront als nicht auf Lager angezeigt, wenn die Option mit einer auf 0 gesetzten Menge erstellt wurde und **Nicht vorrätige Produkte anzeigen** aktiviert ist.
...
* Muster werden nicht mehr als auf der Storefront angezeigt, wenn die Verkaufsmenge des konfigurierbaren Produkts 0 beträgt. [GitHub-33697](https://github.com/magento/magento2/issues/33697)
...
* Dem Workflow &quot;Coupon-Codes verwalten&quot;wurde eine Validierungslogik hinzugefügt, um die Einstellungen des Coupon-Codes beim Generieren von Gutscheinen zu erzwingen. Zuvor wurde bei der Erstellung des Gutscheincodes die **Code-Länge** Einstellung, wenn **Couponanzahl** war größer als 2500.
...
* E-Mails mit Administratorbestellungen werden nicht mehr gesendet, wenn die Variable **Bestätigung der E-Mail-Bestellung** -Einstellung deaktiviert ist. [GitHub-36256](https://github.com/magento/magento2/issues/36256)
...
* Die Verwendung der `mixed` -Typ als Parameter oder Rückgabetyp wird jetzt unterstützt. Zuvor. Adobe Commerce hat diesen PHP-Fehler ausgegeben, wenn dieser Typ verwendet wurde: `Type "mixed" cannot be nullable`. [GitHub-35579](https://github.com/magento/magento2/issues/35579)

<!-- AC-6223-->

* Die `NewRelic` Wrapper-Klasse erfasst jetzt alle throwable `TypeError` Ausnahmen. [GitHub-35854](https://github.com/magento/magento2/issues/35854)
...
* Interne Server-Fehler in `/V1/products/{sku}/links` API-Endpunkt wurde aufgelöst. [GitHub-35923](https://github.com/magento/magento2/issues/35923)

<!-- AC-6421-->

* Interne Server-Fehler in `/V1/creditmemo/refund` API-Endpunkt wurde aufgelöst. [GitHub-35925](https://github.com/magento/magento2/issues/35925)

<!-- AC-7497-->

* `get.php` löst diese Warnung nicht mehr aus, wenn `resource_config.json` ist beschädigt: `Trying to access array offset on value of type null`. [GitHub-36640](https://github.com/magento/magento2/issues/36640)

<!-- AC-6440-->

* Die feste Version von PHP Code Sniffer-Abhängigkeit vom Magento 2-Projekt wurde entfernt `composer.json` -Datei. [GitHub-35994](https://github.com/magento/magento2/issues/35994)

<!-- AC-7300-->

* Die unnötige Manipulation von `version` Schlüssel in `sampledata:deploy`. Bisher wurde ausgeführt `bin/magento sampledata:deploy` eine Fehlermeldung erzeugt hat, die angibt, dass eine `version` Schlüssel fehlt in `composer.json`. Wenn der Befehl erfolgreich ausgeführt wurde, bestätigt Adobe Commerce jetzt den Erfolg und zeigt die nächsten Schritte an. [GitHub-36554](https://github.com/magento/magento2/issues/36554)
...
* Breadcrumbs aus der CMS-Seitenhierarchie werden nicht mehr angezeigt, wenn **Aktivieren der Hierarchiefunktion** deaktiviert ist. Zuvor wurden Breadcrumbs angezeigt, und als Käufer darauf klickten, wurde eine 404-Seite angezeigt. [GitHub-35513](https://github.com/magento/magento2/issues/35513)
...
* `base64`-kodierte URLs enthalten keine reservierten Zeichen mehr gemäß RFC 3986. [GitHub-36042](https://github.com/magento/magento2/issues/36042)
...
* Das New Relic-Reporting-Modul entspricht jetzt der CSP. Die für die Browserberichterstellung angegebenen New Relic-Domänen sind in CSP-Kopfzeilen enthalten, wenn das Tracking von Website-Daten aktiviert ist. [GitHub-34707](https://github.com/magento/magento2/issues/34707)
...
* Adobe Commerce dupliziert keine Fehlermeldungen mehr bei ungültigen Formulareingaben. [GitHub-36201](https://github.com/magento/magento2/issues/36201)

<!-- AC-7508-->

* Admin-Benutzeranmeldungen werden jetzt korrekt im `admin_user` -Tabelle, nachdem das Admin-Benutzerkonto aktualisiert wurde. [GitHub-36642](https://github.com/magento/magento2/issues/36642)

<!-- AC-7507-->

* Informationen zur Tabellenerstellung wurden im Abschnitt `Authorization/README.md`. [GitHub-36644](https://github.com/magento/magento2/issues/36644)

<!-- AC-7511-->

* Der Verhaltenskodex für Projekte (`CODE_OF_CONDUCT.md`). [GitHub-36639](https://github.com/magento/magento2/issues/36639)

<!-- AC-7500-->

* Die Google Adwords-Codevorlage wurde überarbeitet, um die Lesbarkeit zu verbessern und veraltete Maskierungsfunktionen durch korrekte Funktionen zu ersetzen. [GitHub-36641](https://github.com/magento/magento2/issues/36641)
...
* Sie können jetzt die zulässige Fehleranzahl vom Administrator auf 0 setzen **[!UICONTROL System]** > **[!UICONTROL Import]**. [GitHub-36252](https://github.com/magento/magento2/issues/36252)
...
* Die `Fixex.io` Der Importdienst funktioniert jetzt wie erwartet. Die Währungsraten werden jetzt erfolgreich importiert. [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* Verbotene `@author` -Tag wurde aus Modulen in der gesamten Codebasis entfernt. [GitHub-36338](https://github.com/magento/magento2/issues/36338)

### Inventory management

Diese Version enthält mehrere Fehlerbehebungen. Die Beta-Versionshinweise für Inventory management finden Sie hier. Sie finden die Adobe Commerce 2.4.6 GA-Versionshinweise für Inventory management in [Versionshinweise](https://experienceleague.adobe.com/docs/commerce-admin/inventory/release-notes.html).
...
* Die Produktanzahl der Kategorielisten ist jetzt korrekt, wenn der Lagerbestand im Einzelquellenmodus mit der Variablen **Nicht vorrätige Produkte anzeigen** -Einstellung aktiviert ist. Adobe Commerce überprüft jetzt, ob ein Produkt beim Zählen verkauft werden kann.
...
* Konfigurierbare Produkte werden nicht mehr als nicht vorrätig auf der Storefront angezeigt, wenn eine ihrer Varianten in Implementierungen, die das Inventar aus mehreren Quellen implementieren, manuell auf Lager zurückgegeben wurde.
...
* Die veraltete Media Gallery ist jetzt standardmäßig deaktiviert. Zuvor **Aktivieren der Galerie für alte Medien** auf **[!UICONTROL Yes]**.
...
* Der Status **Geschlossen** wird nun virtuellen Bestellungen wie erwartet zugewiesen, nachdem ein Kreditmemo ausgestellt und der Kunde zurückerstattet wurde. Zuvor wurden virtuelle Produkte als **Fertig** in der Dropdown-Liste Status , aber **Geschlossen** in der Fußzeile der Bestellungskommentare, als der Kommentar vom Administrator hinzugefügt wurde. [GitHub-35283](https://github.com/magento/magento2/issues/35283)
...
* Käufer können jetzt konfigurierbare Produkte mit optionalen benutzerdefinierten Optionen wie erwartet neu anordnen. Zuvor gab Adobe Commerce diesen Fehler aus: `Some of the selected options are not currently available`. [GitHub-35409](https://github.com/magento/magento2/issues/35409)

<!-- ACP2E-1313-->

* Versanderstattungen werden jetzt korrekt berechnet, wenn die **Preisrabatt anwenden** Konfigurationsoption auf **einschließlich Steuern**.
...
* Zeileneinträge werden nun für PayPal-Transaktionen übergeben, wenn die Variable **Zeileneinträge senden** Das Konfigurationsfeld wird vom Administrator aktiviert.

<!-- BUNDLE-3187-->

* Validierungslogik zur ACH-Zahlungsmethode **[!UICONTROL Account Number]** -Feld unterstützt jetzt vierstelligen bis siebenstelligen Text.
...
* Der Admin-Protokollbericht zeigt nun eine genaue **Wert vor Änderung** -Wert, wenn eine Kategorieregel aktualisiert wird.
...
* Die Suche nach Zeichenfolgen in Anführungszeichen funktioniert jetzt erwartungsgemäß, wenn die Variable **Mindestens passende Begriffe** -Konfigurationseinstellung aktiviert ist. Zuvor gab Adobe Commerce diesen Fehler aus: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Die Suchergebnisse enthalten jetzt nicht vorrätige Optionseingaben für alle verfügbaren Produkte, wenn **Nicht vorrätige Produkte anzeigen** ist aktiviert [GitHub-36055](https://github.com/magento/magento2/issues/36055)
...
* Adobe Commerce berücksichtigt jetzt die Admin-Kundenkonfiguration **Firma anzeigen** Konfigurationseinstellung bei der Anzeige der Versand- oder Kundenadresse. Wenn diese Einstellung deaktiviert ist, ist die Versandunternehmen nicht mehr im Versand-, Kunden- oder Datenbankauftragsdatensatz enthalten. Zuvor wurden die Informationen trotz der Einstellung der Deaktivierung angezeigt. [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* Fehlt `scope` wurde hinzugefügt `isShippingCarrierAvailable`. [GitHub-36645](https://github.com/magento/magento2/issues/36645)

<!-- AC-7503-->

* Die von der Warenkorbseite bereitgestellte Versandschätzung ist jetzt korrekt. collectShippingRates wird jetzt nur einmal festgelegt. Zuvor wurde die Versandschätzung dupliziert. [GitHub-36648](https://github.com/magento/magento2/issues/36648)
...
* Die automatische Kundengruppenzuweisung funktioniert jetzt, wenn der Kundenadresse keine Mehrwertsteuer-ID zugeordnet ist. Wenn die Adresse keine MwSt-ID enthält, wird sie als ein nicht mehrwertsteuerpflichtiger Kunde behandelt und Adobe Commerce ändert die Kundengruppe in eine Nicht-MwSt-Gruppe. Bisher wurde **Automatische Zuweisung zur Kundengruppe aktivieren** aktiviert wurde, wurde das Kundenkonto nicht automatisch dem `customerGroup` wenn die MwSt.-ID aus dem Kundenkonto gelöscht wurde.
...
* Die ISO-3166-Regionen für die Tschechische Republik wurden der `directory_country_region` Tabelle. [GitHub-36346](https://github.com/magento/magento2/issues/36346)

### Benutzeroberfläche

<!-- ACP2E-1018-->

* Die websitespezifische Währung wird nun wie erwartet für die Spalten &quot;Sonderpreis&quot;, &quot;Kosten&quot;und &quot;MSRP&quot;im Admin-Produkt-Raster verwendet. Zuvor wurde das richtige Währungssymbol nicht im Produktraster des Admin-Katalogs angezeigt, wenn nach der Store-Ansicht nach einem Sonderpreis gefiltert wurde.
...
* Das Kundenraster wird jetzt wie erwartet in Bereitstellungen mit benutzerdefinierten Erweiterungen sortiert. `Shared` ist jetzt auf `false` für `SortOrderBuilder`. Zuvor verursachte dieser Builder unerwünschte Nebenwirkungen, und das Klicken auf den Titel des Kunden-Rasters hatte keine Auswirkungen. [GitHub-35939](https://github.com/magento/magento2/issues/35939)
...
* Admin-Benutzer können nun das Protokoll abgeschlossener Admin-Aufgaben erfolgreich löschen. Zuvor zeigte der Administrator diesen Fehler an: `Something went wrong`. [GitHub-36255](https://github.com/magento/magento2/issues/36255)

<!-- AC-6441-->

* Sie können jetzt eine Sendung für Bestellungen erstellen, die eine Artikelmenge enthalten, die größer als null und kleiner als 1 ist, wenn **[!UICONTROL Quantity uses decimals]** aktiviert ist. Zuvor wurde bei der Eingabe einer Warenmenge von weniger als 1 die Rechnung korrekt erstellt, die Sendung jedoch nicht erstellt. [GitHub-35971](https://github.com/magento/magento2/issues/35971)

<!-- AC-7510-->

* GitHub-Problemvorlagen wurden in Formulare konvertiert, um Probleme mit der Benutzererfahrung und Funktionsanfragen zu beheben[GitHub-36651](https://github.com/magento/magento2/issues/36651)

<!-- AC-7509-->

* Die redundante `getCacheTags` -Methode aus der `Topmenu` -Klasse. [GitHub-36643](https://github.com/magento/magento2/issues/36643)

<!-- AC-7504-->

* Eine Klasse wurde hinzugefügt, um das Modal für die Adressenauslieferung zu gestalten. Ohne diese spezifische Klasse können Frontend-Entwickler das gesamte Modal-Stil-Modell nicht vervollständigen, ohne andere Modale im Checkout-Workflow zu beeinträchtigen. [GitHub-36647](https://github.com/magento/magento2/issues/36647)
...
* Admin-Benutzer mit Zugriff, der auf nur einen Store beschränkt ist, können Produkte nicht mehr in eine Kategorie ziehen und dort ablegen. Stattdessen können sie jetzt die **[!UICONTROL Position]** -Spalte, um die Produktposition neu anzuordnen. Bisher konnten diese Benutzer die Produktposition innerhalb einer Kategorie nur per Drag-and-Drop ändern.
...
Sie können Adobe Commerce 2.4.6 mit [Verfasser](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
