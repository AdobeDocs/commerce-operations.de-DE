...
Mit Adobe Commerce 2.4.6 wird PHP 8.2 unterstützt. PHP 8.1 wird weiterhin vollständig unterstützt, aber die Unterstützung für PHP 7.4 wurde entfernt. Er bietet deutliche Verbesserungen hinsichtlich Leistung und Skalierbarkeit. GraphQL-Vorgänge für Massenvorgangsvorgänge und die Wiedergabe der Kategoriestruktur wurden optimiert. Mit dieser Version wird das Metapaket der Adobe Commerce-Erweiterung eingeführt.

Diese Version umfasst mehr als 300 Qualitätskorrekturen und Verbesserungen. Core Composer-Abhängigkeiten und Bibliotheken von Drittanbietern wurden auf die neuesten Versionen aktualisiert, die mit PHP 8.2 kompatibel sind.
...
Die neueste Diskussion über diese behobenen Probleme [&#128279;](https://helpx.adobe.com/de/security/products/magento/apsb23-17.html) Sie im Adobe-Sicherheitsbulletin.
...
Mit Adobe Commerce 2.4.6 wird PHP 8.2 unterstützt. PHP 8.1 wird weiterhin vollständig unterstützt. <!-- AC-6598 -->

Die PHP 7.4-Unterstützung wurde aus dieser Version entfernt. Sie können Adobe Commerce 2.4.6 nicht auf PHP 7.4 ausführen.
...
* `jquery-migrate` wurde aus der Commerce-Code-Basis entfernt. [GitHub-21406](https://github.com/magento/magento2/issues/21406)-<!-- AC-1199 -->
...
* [Schneller Checkout für Schrauben](https://help.bolt.com/platforms/adobe/adobe-commerce-setup-guide/m2-migrate/)
...
Adobe I/O Events für Adobe Commerce erfasst Commerce-Ereignisse und überträgt Daten von Adobe Commerce an andere Adobe-Systeme. Entwickler können Cloud-native Apps in Adobe Developer App Builder erstellen, die Commerce-Ereignisse wie Inventaraktualisierungen, Auftragsstatusänderungen und Preisaktualisierungen abonnieren. Entwickler können diese Funktion verwenden, um Daten für die Orchestrierung der gesamten Käufer-Journey in ihren unternehmenskritischen Systemen auszutauschen.
...
Die Funktion „Bestellungen für Unternehmen“ wird jetzt vollständig von der GraphQL-API verfügbar gemacht. Mit dieser Version werden zwei GraphQL-Schemata eingeführt:

* Das GraphQL-Schema für Bestellungen deckt Funktionen von B2B-Firmenbenutzenden für den Kauf von Order Management ab, z. B. das Erstellen, Bearbeiten und Löschen von Bestellungen. Siehe [Bestellungen (B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/).
...
PWA Studio v.13.0.x ist mit Adobe Commerce 2.4.6 kompatibel. Es enthält mehrere Verbesserungen zur Verbesserung der Barrierefreiheit. Informationen zu Fehlerbehebungen finden Sie unter [PWA Studio-Versionen](https://github.com/magento/pwa-studio/releases). Unter [Versionskompatibilität](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) finden Sie eine Liste der PWA Studio-Versionen und ihrer kompatiblen Adobe Commerce-Kernversionen.


### Bekanntes Problem

**Problem**: New Relic ist derzeit nicht mit PHP 8.2 kompatibel. Bei Bereitstellungen mit New Relic kann Adobe Commerce 2.4.6 auf PHP 8.1, aber nicht auf PHP 8.2 ausgeführt werden. **Problemumgehung**: Führen Sie Adobe Commerce 2.4.6 auf PHP 8.1 aus, bis eine neue Version von New Relic veröffentlicht wird, die PHP 8.2 unterstützt. Diese Version wird voraussichtlich im Sommer 2023 veröffentlicht. <!-- AC-7769-->
...
* Adobe Commerce gibt jetzt einen Fehler bei der Parametervalidierung aus, wenn Sie die Befehlszeile verwenden, um eine Redis-Verbindung ohne Kennwort festzulegen.
...
* Das Produktraster löst keine Ausnahme mehr aus, wenn Bundle-Produkte einen Sonderpreis von über 1.000 haben. Wenn Sie zuvor nach dem Produkt anhand der SKU gesucht oder zu **[!UICONTROL Catalog]** > **[!UICONTROL Products]** navigiert sind, hat Adobe Commerce folgenden Fehler ausgegeben: `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* Adobe Commerce berechnet jetzt die Gesamtsumme für Rückerstattungen von Bestellungen mit einem Rabatt für den Versand korrekt.

<!-- ACP2E-876-->

* Google reCAPTCHA funktioniert jetzt für Storefront-Bestellungen wie erwartet. Zuvor wurden diese Fehler in Adobe Commerce angezeigt, wenn Google ReCAPTCHA nicht zum ersten Mal ausgewählt wurde, bevor ein Erstkäufer versuchte, eine Bestellung aufzugeben: `ReCaptcha validation failed, please try again` und `No such cart with id = ID`.
...
* Adobe Commerce gibt jetzt eine informative Nachricht zurück, wenn eine `addProductsToCart` Mutation fehlschlägt, wenn ein Produkt mit unzureichenden Berechtigungen zum Warenkorb hinzugefügt wird.
...
* Die `setGiftOptionsOnCart` Mutation erfordert jetzt die Autorisierung des Kunden für Warenkorbvorgänge.
...
* Kunden, die einer Großhandelsgruppe zugewiesen sind, können jetzt ein gruppiertes Produkt zu ihrem Warenkorb hinzufügen, wenn dies durch Kategorieberechtigungen zulässig ist. Wenn diese Berechtigung für eine Kundengruppe nicht definiert ist, hat die Konfigurationseinstellung Vorrang.
...
* Couponcodes können jetzt erfolgreich in Excel XML exportiert werden. Zuvor enthielt die generierte Datei nicht alle ausgewählten Couponcodes, sondern nur so viele Elemente, wie in der Auswahl „Elemente pro Seite“ ausgewählt wurden.[GitHub-36073](https://github.com/magento/magento2/issues/36073)

### Cron

<!-- ACP2E-1243-->

* Der `catalog_index_refresh_price` Cron-Vorgang wird jetzt erfolgreich ausgeführt. Zuvor hat Adobe Commerce diesen Fehler während der Cron-Ausführung protokolliert: `report.ERROR: Cron Job catalog_index_refresh_price has an error: SQLSTATE[HY093]: Invalid parameter number: number of bound variables does not match number of tokens`.
...
* Der Filter Ja/Nein im Admin-Kundensegmente -Raster während der Erstellung einer neuen Warenkorb-Preisregel funktioniert jetzt erwartungsgemäß.

### E-Mail

<!-- ACP2E-1063-->

* Adobe Commerce sendet keine doppelten E-Mails mehr an Kunden, nachdem eine Bestellung vom Administrator erstellt wurde.
...
* AMQP-Verbraucher schlagen nicht mehr aufgrund einer veralteten `trim()`-Funktion auf Bereitstellungen mit PHP 8.1 fehl. Zuvor gab Adobe Commerce beim Start des AMQP-Verbrauchers nach der Installation folgenden Fehler aus: `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* Die `getFilteredTimerIds`-Methode funktioniert jetzt erwartungsgemäß.  Die Klasse `Magento\Framework\Profiler\Driver\Standard\Stat` ist jetzt mit PHP 8.1 kompatibel. Zuvor gab Adobe Commerce eine Ausnahme aus, wenn diese Methode aufgerufen wurde. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Eine Datenbankabfrage des `BundleGraphQl`-Moduls, das diesen Fehler zurückgab, wurde korrigiert: `"debugMessage": "Notice: Undefined offset: 16 in vendor/magento/module-bundle-graph-ql/Model/Resolver/Options/Collection.php on line 130"`.[GitHub-34991](https://github.com/magento/magento2/issues/34991)

### Allgemeine Fehlerbehebungen

<!-- ACP2E-1068-->

* Adobe Commerce zeigt jetzt einen genauen Zeitbereich an, wenn ein Administrator bzw. eine Administratorin die Option &quot;**[!UICONTROL Last 24 Hours]**&quot; im Dashboard „Administratorstatistiken“ auswählt. Zuvor wurde in Adobe Commerce ein falscher Zeitbereich angezeigt. Dies trat auf, weil die Zeitzone der Datenbank unverändert blieb.
...
* Admin-Benutzende, deren Zugriff auf eine Website beschränkt ist, können sich jetzt mit der Funktion „Als Kunde anmelden“ erfolgreich beim Admin anmelden.
...
* Die Produktmengen der Geschenkkarte bleiben nun während des Imports gleich der in einer CSV-Datei angegebenen Menge. Es wurden Prüfungen für vorhandene Optionen hinzugefügt und frühere Werte werden jetzt auf der Grundlage eines eindeutigen Schlüssels aktualisiert. Zuvor wurden die Beträge der Geschenkkarten verdoppelt, wenn Geschenkkarten mehrmals importiert wurden.

### Geschenkverpackung

<!-- AC-3061-->

* Der Preis für Geschenkverpackungen wird jetzt wie erwartet im Block Bestellzusammenfassung angezeigt, wenn Geschenkverpackungen auf Bestellebene auf eine Bestellung angewendet wurden, die sowohl physische als auch virtuelle Produkte enthält. Zuvor zeigte Adobe Commerce den Preis für Geschenkverpackungen nicht an.
...
* Bereinigung von `DATE`- und `DATETIME`-Werten in Datenbank-Dumps zur Unterstützung von Adobe Commerce hinzugefügt. Zuvor wurden Datums-/Uhrzeittypfelder für Kunden-/Angebots-/Auftragstabellen in der Datenbank-Sicherungstabelle nicht bereinigt.

<!-- ACP2E-804-->

* Mit der Funktion `escapeHtml` `escaper.js` wird nun das kaufmännische Und-Zeichen (&amp;) gemäß der `InnerHtml` in `"$amp;"` umgewandelt. Diese generische Lösung löst Konvertierungsprobleme bei Sonderzeichen und Zeichenverweisen im Eingabefeld für den Titel und speichert den genauen Wert als eingefügt. Zuvor waren kaufmännische Und-Zeichen (&amp;) in Knotennamen in der Inhaltshierarchie falsch kodiert.
...
* Die alte Mediensammlung ist jetzt standardmäßig deaktiviert. Zuvor war **[!UICONTROL Enable old Media gallery]** auf **[!UICONTROL Yes]** gesetzt.

<!-- ACP2E-1302 -->

* Die neue Mediensammlung wurde optimiert, um das Laden des Verzeichnisbaums auf der Kategorieseite zu verbessern.

### Protokollierung

<!-- ACP2E-1312-->

* Sie können jetzt Systemberichte für Protokolldateien mit 0 Byte generieren. Zuvor gab Adobe Commerce beim Generieren eines Berichts für eine Sammlung von Protokolldateien einen Fehler aus, wenn eine oder mehrere Dateien leer waren.

### Reihenfolge

<!-- ACP2E-1107-->

* Auf der Seite „Gutschrift für Kundenaufträge in Admin“ wird nun die korrekte Gesamtsumme für Bestellungen angezeigt, die mit „Gutschrift für Lager“ und mehreren Rechnungen erstellt wurden. Zuvor war die Berechnung des Gesamtbetrags falsch, und der/die Benutzende konnte keine Gutschrift erstellen.
...
* Käufer können jetzt die Braintree-Zahlungsmethode in Storefronts verwenden, in denen **[!UICONTROL Enable Card Payments]** deaktiviert wurde. Zuvor gab Adobe Commerce diesen JavaScript-Fehler aus: `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* Der Admin-Protokollbericht zeigt jetzt einen genauen **[!UICONTROL Value before change]** an, wenn eine Kategorieregel aktualisiert wird.
...
* Das RMA-Kommentarfeld (**[!UICONTROL Orders]** > **[!UICONTROL Return]**) enthält keine redundanten Leerzeichen mehr. (Die Tags `<textarea>` und `<?php` befinden sich nun in einer Zeile.

<!-- ACP2E-1279-->

* Das Kontrollkästchen **[!UICONTROL Use Config Settings]** für das Feld **[!UICONTROL Enable RMA]** speichert nun seinen Wert wie erwartet, wenn ein Produkt gespeichert wird.
...
* Die Suche nach Zeichenfolgen in Anführungszeichen funktioniert jetzt erwartungsgemäß, wenn die **[!UICONTROL Minimum Terms to Match]** Konfigurationseinstellung aktiviert ist. Zuvor gab Adobe Commerce diesen Fehler aus: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Adobe Commerce kann jetzt in PHP 8.1-Bereitstellungen gruppierte Produkte mit MwSt. auf der Kategorieseite der Storefront anzeigen. Zuvor zeigte Adobe Commerce keine Liste von Produkten an und löste diese Ausnahme aus: `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`. [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* Die automatische Zuordnung von Kundengruppen funktioniert jetzt, wenn mit der Adresse des Kunden keine MwSt.-Kennung verknüpft ist. Wenn die Adresse keine MwSt.-ID enthält, wird sie als zu einem nicht MwSt.-registrierten Kunden gehörend behandelt, und Adobe Commerce ändert die Kundengruppe in eine nicht MwSt.-Gruppe. Zuvor wurde bei aktivierter **[!UICONTROL Enable Automatic Assignment to Customer Group]** das Kundenkonto nicht automatisch dem `customerGroup` zugewiesen, wenn die MwSt.-ID aus dem Kundenkonto gelöscht wurde.
...
* Unter dem `Storefront Properties` Eingabefeld des `Search Weight` Attributs wurde informativer Text unter Admin **[!UICONTROL Stores]** > [!UICONTROL Attributes] > **[!UICONTROL Product]** hinzugefügt.
...
* Der Service „Fixer-Währung“ funktioniert jetzt wie erwartet mit Schlüsseln, die über APIlayer generiert werden, und die Währungskurse werden korrekt exportiert. Zuvor gab es in Adobe Commerce einen Fehler, wenn ein Händler versuchte, Wechselkurse über die Admin zu importieren.
...
Sie können Adobe Commerce 2.4.6 mit [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html?lang=de) installieren.
