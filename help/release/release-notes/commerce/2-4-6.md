...
Adobe Commerce 2.4.6 bietet Unterstützung für PHP 8.2. PHP 8.1 wird weiterhin vollständig unterstützt, aber die Unterstützung für PHP 7.4 wurde entfernt. Sie umfasst erhebliche Verbesserungen der Leistung und Skalierbarkeit. GraphQL-Vorgänge für Massenkorb-Vorgänge und die Antwort auf das Rendern von Kategoriestrukturen wurden optimiert.
...
## 2.4.6 Beta2-Testumgebung

Adobe Commerce 2.4.6-beta2 wurde mit den folgenden Komponentenversionen getestet:
...
Adobe Commerce 2.4.6-beta2 führt Unterstützung für PHP 8.2 ein. PHP 8.1 bleibt vollständig unterstützt.  <!-- AC-6598 -->

Die Unterstützung für PHP 7.4 wurde aus dieser Version entfernt. Sie können Adobe Commerce nicht auf PHP 7.4 ausführen.

Adobe Commerce 2.4.6-Beta2 unterstützt jetzt:

* Composer 2.2.x <!-- AC-2839 -->

* Redis 7.0.x. Obwohl die Kompatibilität mit Redis 6.2 weiterhin besteht, empfehlen wir die Bereitstellung dieser Version mit Redis 7.0.x, da Redis 6.2 im April 2024 das Ende des Lebenszyklus erreichen wird. <!-- AC-6606 -->
...
* Aktualisiert `jQuery/fileUpload`Bibliothek in v10.32  <!-- AC-2658-->
...
Die Funktion &quot;Bestellung für Unternehmen&quot;wird jetzt von der GraphQL-API vollständig bereitgestellt. In dieser Version werden zwei GraphQL-Schemata eingeführt:

* Das GraphQL-Schema für die Bestellung umfasst die Funktionen für die Verwaltung von B2B-Kundenaufträgen, z. B. Erstellen, Bearbeiten und Löschen von Bestelldaten.
...
**Verbesserte Reaktionszeit bei der Abfrage von Kategorien mit aktivierten Kategorieberechtigungen**. Die Antwortzeiten bei der Abfrage von Produkten in Kategorien, für die Kategorieberechtigungen aktiviert sind und die viele Kundengruppen oder freigegebene Kataloge enthalten, wurden verbessert. <!-- B2B-2179-->

**Optimierte Leistung beim Rendern von Kategoriestrukturen in GraphQL**. Die `categoryList` -Abfrage wurde optimiert, um Kunden mit einer großen Anzahl von Kategorien zu unterstützen. Bei Merchants, die diese Abfrage verwenden, um mehr als 1500 Kategorien mit bis zu fünf Ebenen verschachtelter Unterkategorien zu durchsuchen, wird eine Leistungsverbesserung festgestellt.
<!-- B2B-2484-->
...
Hunderte von Problemen werden im Adobe Commerce 2.4.6-Kerncode behoben. Nachfolgend wird eine Teilmenge der in Adobe Commerce 2.4.6-beta2 enthaltenen behobenen Probleme beschrieben.
...
* GraphQL-Abfrageantworten enthalten jetzt Informationen zu den untergeordneten Produkten der Bundle-Produkte in `product` Abschnitt `options` blockieren.
...
* Die Anzahl der Kategorieelemente ist jetzt nach der Filterung auf der Katalogseite genau, wenn **Nicht vorrätig anzeigen** aktiviert ist. Zuvor war die Anzahl der nicht vorrätigen Produkte an der Storefront falsch.
...
* Der Kunde `order` -Abfrage gibt jetzt Bestellungen von allen angegebenen Websites, Stores und Store-Ansichten wie erwartet zurück. Diese Fehlerbehebung umfasst die Erstellung eines neuen Anfrage-Arguments.
...
* Die Seitenladeleistung wurde beim Laden der Produktliste des Katalog-Widgets verbessert. Zuvor wurden nicht globale Attribute direkt aus der Datenbank geladen.
