...
Verwenden Sie für Valley mit der alten Cache-Implementierung:
...
Für Valkey mit der modernen Symfony L2-Cache-Implementierung siehe [Konfigurieren des Symfony L2-Cache](#configure-symfony-l2-cache).
...
Wenn Sie die `VALKEY_BACKEND` Bereitstellungsvariable auf `symfony_l2` setzen, wird automatisch die vollständige L2-Cache-Konfiguration aus Ihren Valkey-Service-Verbindungsdetails erstellt, einschließlich eines `default`-Frontends und eines `stale_cache_enabled`-Frontends, mit zwischenspeicherbaren Typen wie `layout`, `block_html`, `full_page` und `translate`, die bereits dem veralteten Frontend zugeordnet sind. Die Definition von `CACHE_CONFIGURATION` ist optional und nur erforderlich, wenn Sie bestimmte Backend-Optionen anpassen möchten.

>[!NOTE]
>
>Patch ACP2E-5132 für Adobe Commerce 2.4.9 verbessert die Leistung und Zuverlässigkeit des Symfony L2-Cache - optimierte Tag-Speicherung, eine veraltete Cache-Regenerierungssperre und Fehlerbehebungen für veraltete Tag-Mitgliedschaften nach Retagging, redundante Remote-Schreibvorgänge bei unveränderten Speichervorgängen und L1-größenbasierte Entfernung (`cleanup_percentage`). Dadurch werden Datenträger-E/A und Backend-Last reduziert und gleichzeitig die Cache-Konsistenz verbessert. Siehe [Verbesserte Leistung und Zuverlässigkeit des Symfony L2](../../../configuration/cache/level-two-cache.md#enhanced-symfony-l2-cache-performance-and-reliability)Cache im _Adobe Commerce-Konfigurationshandbuch_.
>
...
#### Anpassen der Konfiguration des Symfony L2-Cache
...
>[!CAUTION]
>
>Überschreiben Sie beim Definieren von `CACHE_CONFIGURATION` für `symfony_l2` nicht `server` oder `port`, es sei denn, Sie verweisen absichtlich auf einen anderen Cache-Endpunkt als den Valkey-Service Ihres Projekts. Das ECE-Tools-Paket leitet diese Werte automatisch von Ihrer Valley-Service-Beziehung ab.
>
