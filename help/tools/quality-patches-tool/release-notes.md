...
* **ACP2E-4630** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass lange Produktnamen mit benutzerdefinierten Optionen nach Seitenumbrüchen benachbarte Spalten oder Artikel in mehrseitigen Rechnungs-, Versand-, Gutschriften- und Rückgabe-PDFs überschneiden, sodass Zeileneinträge unlesbar werden.
* **ACP2E-4680** (für Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Es wird das Problem behoben, dass nicht verkaufbare oder gelöschte Produkte aus abgeschlossenen verhandelbaren Angeboten verschwinden.
* **ACP2E-4709** (für Adobe Commerce >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass CMS-Seiten bei Verwendung von Page Builder nicht gespeichert werden.
* **ACP2E-4786** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.10) - Behebt das Problem, dass beim Exportieren von Produkten Dateien nicht geschrieben werden können, wenn der AWS S3-Remote-Speicher konfiguriert ist, da im Speichertreiber eine falsche Pfadauflösung vorgenommen wurde.
* **ACP2E-4801** (für Adobe Commerce, B2B >=1.4.2 &lt;1.5.3) - Es wird das Problem behoben, dass die Aktualisierung der Bundle-Produktoptionsmengen über die Schaltfläche Konfigurieren in einem verhandelbaren Angebot vom Administrator nicht angewendet wird und Änderungen verworfen werden.
* **ACP2E-4815** (für Adobe Commerce und Magento Open Source >=2.4.6-p15 &lt;2.4.7 || >=2.4.7-p1 &lt;2.4.9) - Behebt mehrere GraphQL-Probleme, die zu PHP-Ausnahmefehlern in Protokollen, korrekter Zuordnung von Bestellungen mit Kundenkonten, die nach der Bestellung über GraphQL erstellt wurden, und Ausrichtung von Antworten mit GraphQL über HTTP-Spezifikationen führten.
* Aktualisierte Versionen: **ACSD-64212**, **ACSD-64523**, **ACSD-67347**
* Ersetzte Patches: **ACSD-67941**, **ACSD-64212**, **ACSD-68925**

## v1.1.80 {#v1-1-80}

* **ACP2E-4239** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wird das Problem behoben, dass Admin-Rasterfilter, die Datumsattribute verwenden, aufgrund von Zeitzonenunterschieden zwischen dem ausgewählten Datum, gespeicherten UTC-Werten und der konfigurierten Speicherzeitzone falsche Ergebnisse zurückgeben.
* **ACP2E-4472** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass beim Fluss „Als Kunde anmelden“ ein Nullangebot erstellt wurde.
* **ACP2E-4481** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, dass die Verkaufsfähigkeit von Bundle-Produkten nach Stornierung einer Bestellung nicht korrekt neu berechnet wurde.
* **ACP2E-4488** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass das Speichern oder Bearbeiten von Produkten in Admin bei Produkten mit großen Attributsätzen langsam ist.
* **ACP2E-4493** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass im Auftragsarchivraster ein falscher Auftragsstatus angezeigt wird, wenn die asynchrone Indizierung aktiviert ist.
* **ACP2E-4496** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass der Cron-Vorgang von Analytics während der Ausführung Leistungseinbußen verursacht, was zu einer verbesserten Gesamtsystemleistung führt.
* **ACP2E-4533** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass Platzhalterbilder nicht in die Storefront geladen werden, wenn ein Store-Code in der URL enthalten ist.
* **ACP2E-4552** (für Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Es wurde das Problem behoben, bei dem der Unternehmensstatus in der GraphQL-Antwort nicht zurückgegeben wurde.
* **ACP2E-4610** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass der Cron-Auftrag „sales_clean_quotes“ Leistungsprobleme aufweist.
* **ACP2E-4615** (für Adobe Commerce >=2.4.4-p13 &lt;2.4.4-p17 || >=2.4.4-p18 &lt;2.4.5 || >=2.4.5-p12 &lt;2.4.6 || >=2.4.6-p10 &lt;2.4.7 || >=2.4.7-p5 &lt;2.4.9) - Behebt das Problem, dass Online-Bestellerstattung mit dem Fehler „PayPal“ fehlschlagen Gateway lehnt Anfrage ab. Interner Fehler.“
* **ACP2E-4626** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass einige Storefront-JavaScript-Dateien zweimal angefordert und ausgeführt werden, was zu zeitweiligen doppelten Ladevorgängen und instabilem Verhalten führt.
* **ACP2E-4653** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, dass der Attributbereich Warenkorbpreisregel für „Kategorie (nur übergeordnetes Element)“ und „Kategorie (nur untergeordnete Elemente)“ beim Abrufen oder Aktualisieren von Regeln über die REST-API nicht offen gelegt wurde.
* **ACP2E-4808** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.10) - Es wird das Problem behoben, dass das Attribut „Weight“ auf der Produktseite für Storefronts nur einen numerischen Rohwert im Abschnitt „Zusätzliche Informationen“ oder „Weitere Informationen“ ohne die konfigurierte Maßeinheit (lbs oder kgs) anzeigt.
...
* **AC-10737** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7-p5) - Es wurde ein Problem behoben, bei dem der Befehl bin/magento setup:db:status den JSON-Datentyp nicht erkennt.
* **ACP2E-4402** (für Adobe Commerce >=2.4.5 &lt;2.4.6 || >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass als deaktiviert erstellte Produkte nach der Aktivierung nicht wieder zu den zugehörigen Target-Regelergebnissen hinzugefügt wurden.
* **ACP2E-4505** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, dass es möglich war, eine Kategorie mit veralteten Daten von einer doppelten Browser-Registerkarte zu speichern, wodurch eine zirkuläre Abhängigkeit erzeugt wurde.
* **ACP2E-4531** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass beim Ändern des URL-Schlüssels einer CMS-Seite die hierarchische URL der Seite nicht aktualisiert wurde.
* **ACP2E-4601** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, dass die Verarbeitung von Zahlungsvorgängen unter bestimmten Bedingungen ineffizient sein konnte.
* **ACP2E-4603** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebt das Problem, dass bei der Ausführung der Produktneuindizierung für Katalogberechtigungen vorhandene Berechtigungsindexzeilen unverändert blieben, was dazu führte, dass aktualisierte Kategorieberechtigungen nicht zuverlässig auf Produkten angezeigt wurden.
* **ACP2E-4706** (für Adobe Commerce >=2.4.5 &lt;2.4.9) - Es wurde das Problem behoben, dass Produkte, die nicht im Admin-Bereich aktiviert waren, vom Zielregelindizierer übersprungen wurden.
* **ACP2E-4720** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, dass der kostenlose Versand für Bundle-Produkte mit Regeln zum Warenkorbabschlag nicht ordnungsgemäß angewendet oder entfernt wurde.
* **ACP2E-4411** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass der falsche Preis für ein Bundle-Produkt auf der Warenkorbseite und im Mini-Warenkorb für Geschäfte mit mehreren Währungen angezeigt wird.
* **ACP2E-4475** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass auf der Produktlistenseite nicht vorrätige Produkte fälschlicherweise nach Preis gefiltert und sortiert werden, wenn die Option „Nicht vorrätige Produkte anzeigen“ aktiviert ist.
* **ACP2E-4110** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass Paketprodukte mit einem Sonderpreis falsche Beträge in PDP und PLP in einer nicht standardmäßigen Währung angezeigt haben.
* **AC-10698** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Das Problem wurde behoben, dass das System die Währung auf der Ebene aller Bestellungen sendete, anstatt sie einzelnen Bestellungen zuzuordnen. Transaktionspreise und -summen werden jetzt pro Bestellung an Google Tag gesendet, wodurch die Tracking-Genauigkeit der E-Commerce-Daten verbessert wird.
* Aktualisierte Versionen: **ACSD-67383**, **ACSD-69203**
* Patches ersetzt: **ACSD-67370**, **ACSD-69319**

## v1.1.78 {#v1-1-78}

* **ACP2E-4416** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass Kundenbelohnungspunkte nicht initialisiert wurden, wenn sie in der Admin erstellt wurden.
* **ACP2E-4419** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebt das Problem, dass Geschenkkarten nach erfolgreicher Validierung von reCAPTCHA v2 („Ich bin kein Roboter„) in der Storefront beim Checkout nicht korrekt angewendet werden.
* **ACP2E-4431** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass verwandte Produkte, die von den Zielregeln abgeglichen werden, während des Neuindizierungsprozesses gelöscht werden.
* **ACP2E-4448** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wird das Problem behoben, dass Konfigurationsänderungen, die während Redis-Ausfällen vorgenommen wurden, nach der Wiederherstellung von Redis nicht widergespiegelt werden, was dazu führt, dass veraltete Werte bestehen bleiben.
* **ACP2E-4452** (für Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Behebt das Problem, dass Produktpreise auf der Schnellbestellungsseite unabhängig von der Steueranzeigekonfiguration Steuern enthalten.
* **ACP2E-4456** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde ein Problem behoben, bei dem durch die Stornierung einer Bestellung mit einer GraphQL-Mutation eine Bestellung, die vollständig mit Geschenkgutscheinen bezahlt wurde, nicht in den Status „Geschlossen“ wechselt.
* **ACP2E-4507** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, dass bei Anfragen zum Zurücksetzen des Kundenkennworts durch GraphQL-Mutationen die Kennwortoptionen-Konfiguration nicht angewendet wurde.
* **ACP2E-4513** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass abgelaufene CAPTCHA-Bilder nicht aus dem System gelöscht werden.
* **ACP2E-4522** (für Adobe Commerce >=2.4.7 &lt;2.4.9) - Es wird das Problem behoben, dass ein zeitweiliger Fehler bei Schlüsselduplikaten in der Tabelle quote_coupons auftritt, wenn mehrere Anfragen zum Zusammenführen des Warenkorbs oder zum Speichern von Angeboten gleichzeitig ausgeführt werden.
* **ACP2E-4528** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem der Stadtvalidierung in Kundenadressen, die jetzt einen Schrägstrich (/) zulässt und ungültige Zeichen wie !, &quot;, # und ? zurückweist.
* **ACP2E-4535** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt ein Problem, bei dem das Senden des Formulars bei vergessenem Kennwort dazu führt, dass die Sitzung zerstört oder neu generiert wird (PHPSESSID-Änderungen) und der Gastkorb gelöscht wird.
* **ACP2E-4540** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, dass die Fotorama-Bibliothek nicht korrekt geladen wurde, sodass nur das erste angehängte Bild angezeigt wurde.
* **ACP2E-4555** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass moderne Telefonnummern &quot;.“ enthalten. oder &quot;/&quot; werden nicht ordnungsgemäß validiert.
* **ACP2E-4565** (für Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Behebt das Problem, dass die Abfrage von Company GraphQL „Der aktuelle Kunde ist nicht autorisiert“ zurückgibt, wenn der Header X-Adobe-Company verwendet wird.
* **ACP2E-4591** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass Kundensegmente, die auf der Bestellanzahl basieren, wie z. B. „Erstkäufer“, nicht aktualisiert wurden, wenn Bestellungen über die REST-API aufgegeben wurden.
* **ACP2E-4609** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.9) - Behebt das Problem, dass auf der Seite Meine Anführungszeichen keine Anführungszeichen angezeigt werden, wenn einige Anführungszeichen gelöschte Produkte enthalten.
* **ACP2E-4613** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.9) - Behebt das Problem, dass große Medienverzeichnisstrukturen zu langsamen gettree-Antworten führten, was zu verlängerten Ladezeiten der Media Gallery-Verzeichnisstruktur führte.
* **ACP2E-4628** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass der Import von Kunden mit E-Mail-Adressen in Großbuchstaben zu dem Fehler „Nicht definierter Array-Schlüssel“ führt, wenn die Kontofreigabe auf „Global“ festgelegt ist.
* **ACP2E-4665** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, dass untergeordnete Produkte von konfigurierbaren Produkten, die Videos in den Produktgalerien enthalten, nicht aufgeführt wurden, wenn sie über die REST-API angefordert wurden.
...
* **ACSD-68892** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt das Problem, dass zwischenspeicherbare Seiten nicht ordnungsgemäß gespeichert oder aus dem Fastly-Cache bereitgestellt werden, was zu inkonsistentem Caching-Verhalten und verminderter Leistung führt.
* **ACSD-69016** (für Adobe Commerce >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass der Sonderpreis nicht für Websites gilt, die in verschiedenen Zeitzonen erstellt wurden.
* **ACSD-69020** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Es wird das Problem behoben, dass ein konfigurierbares Produkt automatisch in die PageBuilder-Produktkarusselllisten aufgenommen wird, wenn eines der untergeordneten Produkte die Filterbedingungen erfüllt.
* **ACSD-69237** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass die Anzahl der Einträge, die über die cron-Aufträge sales_*_async_insert verarbeitet und eingefügt werden können, auf 100 pro Ausführung begrenzt ist.
* **ACSD-69311** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Behebt das Problem mit der falschen Steuerberechnung in Gutschriften bei der Erstellung einer Teilerstattung aus einer Rechnung, wenn eine vorherige Gutschrift auf der Bestellansichtsseite erstellt wurde.
* **ACSD-69351** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass Geschenkgutscheine und Ablaufdaten nicht entsprechend dem zugewiesenen Website-Umfang angezeigt werden.
* **ACSD-69494** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Behebt die Probleme bei asynchronen Rückerstattungsvorgängen, bei denen Rückerstattungsanfragen mit dem Parameter „is_online“ nicht korrekt verarbeitet werden.
* Aktualisierte Versionen: **ACSD-67250**
* Patches ersetzt: **ACSD-62629**, **ACSD-66157**
* Veraltete Patches: **ACSD-66157**
...
* **ACSD-69261** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde ein Problem behoben, bei dem ein Warenkorbpreisregelcoupon, der für den einmaligen Gebrauch pro Kunde konfiguriert war, aufgrund einer falschen Handhabung des Attributs `times_used` in Szenarien für Teilrechnungen und Restmengenstornierungen mehrfach wiederverwendet wurde.
* **ACSD-69308** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde ein Problem behoben, bei dem Katalogpreisregeln nicht anwendbar waren, wenn `special_price` nur auf Website-Ebene (nicht in „Alle Store-Ansichten„) festgelegt wurde. Nach der Fehlerbehebung gelten die Katalogpreisregeln korrekt, indem zunächst der Standardspeicher der Website überprüft wird.
...
* **ACSD-69331** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.9) - Es wurde ein Problem behoben, bei dem Ersteller von Inhalten in der Mediensammlung keine Ordner mit nur der `create_folder`-Berechtigung erstellen konnten. Nach der Behebung können sie wie erwartet Ordner erstellen.
...
* **ACSD-67171** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass B2B-Benutzer eine *[!UICONTROL Access Denied]*-Seite sehen, wenn ihre Sitzung abgelaufen ist oder während des Checkouts entfernt wurde.
...
* **ACSD-68206** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Behebt den Fehler bei der Verwendung des GraphQL-Anwendungsservers mit der Funktion **[!UICONTROL Rate Limiting]**, wobei die [!DNL PHP Redis]-Erweiterung installiert ist.
...
* **ACSD-68040** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Es wurde das Problem behoben, dass die Leistung der Frontend-Suchseite auf [!DNL MariaDB] 10.6 und 11.4 mit vielen historischen Suchanfragen abnimmt.
...
* **ACSD-66807** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass in der Tabelle `report_viewed_product_index` eine falsche Anzahl von Produktseitenansichten angezeigt wird.
* **ACSD-67383** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass die Verwendung von **[!UICONTROL Login as Customer]** mit zwei Unternehmensadministratorkonten in derselben Sitzung zu einem Fehler *Keine solche Entität mit cartId* führt.
...
* **ACSD-67639** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Das Problem, dass das Erstellen einer Gutschrift für Bundle-Produkte fehlschlägt, wenn **[!UICONTROL Dynamic Price]** auf &#39;*&#39; gesetzt*, wurde behoben.
...
* **ACSD-68011** (für Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Behebt das Problem, dass nicht vorhandene SKUs über die `/V1/sharedCatalog/:id/assignProducts` [!DNL REST]-API einem freigegebenen Katalog zugewiesen werden können.
* **ACSD-68118** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.9) - Es wird das Problem behoben, bei dem die GraphQL-Abfrage `customerCart` Produktattributwerte zurückgibt, die nicht die Store-Kopfzeile widerspiegeln, was zu inkonsistenter Lokalisierung führt.
...
* **ACSD-67424** (für Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Es wird das Problem behoben, bei dem der Wert `updated_at` in der `GET /carts/search` [!DNL REST]-API-Antwort nicht mit dem in der **[!UICONTROL Admin panel]** angezeigten Wert übereinstimmt, wenn verhandelbare Anführungszeichen verwendet werden.
...
* **ACSD-60624** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass **[!UICONTROL Upload Image]** bei leeren Inhalten in [!UICONTROL Image]-, [!UICONTROL Banner]- und [!UICONTROL Slider]-Abschnitten in Page Builder nicht funktioniert.
...
* **ACSD-67102** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass das Adobe Commerce-Backend **[!UICONTROL Categories]** sehr langsam lädt.
...
* **ACSD-67166** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wird das Problem behoben, dass die Abfrage `cataloginventory_stock_status` beim Laden eines Angebots in der Storefront mehrmals ausgeführt wird, was zu redundanten Datenbankaufrufen führt.
...
* **ACSD-67686** (für Adobe Commerce und Magento Open Source >=2.4.4-p15 &lt;2.4.5 || >=2.4.5-p14 &lt;2.4.6 || >=2.4.6-p12 &lt;2.4.7) - Behebt das Problem, dass beim Senden einer leeren [!DNL GraphQL]-Anfrage ein `Syntax Error: Unexpected <EOF>`-Fehler auftritt.
* **ACSD-67250** (für Adobe Commerce >=2.4.7-p4 &lt;2.4.8) - Es wurde das Problem behoben, dass durch den Speichervorgang **[!UICONTROL Shared Catalog]** alle Elemente und nicht nur die betroffenen aktualisiert wurden, wodurch die Leistung verbessert wurde, indem unnötige Vorgänge vermieden wurden.
...
* **ACP2E-3731** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass Produktexporte mit **[!UICONTROL Catalog, Search]** Sichtbarkeit fälschlicherweise Datensätze aus anderen Store-Ansichten in Multi-Store-Umgebungen enthalten.
...
* **ACP2E-3977** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass das Feld **[!UICONTROL Cap Reward Points Balance At]** beim Festlegen von [!UICONTROL Rewards Points Balance Redemption Threshold] nicht leer sein darf und einen Validierungsfehler verursacht.
...
* **ACSD-56226** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Es wird das Problem behoben, dass READ-Abfragen am Slave-Knoten veraltete Daten zurückgeben, wenn das Flag `synchronous_replication` aktiviert ist.
...
* **ACSD-66153** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass eine Seite einen *500*-Fehler aufgrund einer falschen Layout-Struktur im Cache zurückgibt.
...
* **ACSD-66404** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Behebt das Problem, dass der Cron-Vorgang keine Changelog-Tabellen löscht, was bei der Verarbeitung großer Datenmengen zu [!DNL Galera Cluster] Abstürzen führt.
...
* **ACSD-67347** (für Adobe Commerce und Magento Open Source >=2.4.5-p11 &lt;2.4.6) - Behebt das Problem, dass die Bestellung mit *Keine Sperre erhalten kann* Fehler, wenn Coupons mit Sonderzeichen verwendet werden und die Dateisperrung aktiviert ist.
...
* **ACSD-66179** (für Adobe Commerce und Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.9) - Fehlerkorrektur: Die Stornierung einer Rechnung, die mit dem Zahlungstyp „Nicht erfasst“ erstellt wurde, führte zu einer 404-Fehlerseite.
...
* **ACSD-66963** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Es wurde das Problem behoben, bei dem die `estimateTotals`-Mutation bei Anwendung eines Rabattcodes auf einen Warenkorb mit virtuellen Produkten den Wert Null zurückgab.
* **ACSD-67039** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass Kundendatensätze aufgrund der Validierung des `rp_token` Systemattributs nicht gespeichert wurden und die Diakritik-Validierung jetzt nur mehr auf die resultierende Kunden-E-Mail angewendet wird.
...
* **ACSD-66049** (für Adobe Commerce und Magento Open Source >=2.4.5-p3 &lt;2.4.6 || >=2.4.7 &lt;2.4.9) - Behebt das Problem, dass nicht englische Storefronts aufgrund der ICU-Bibliotheksversion falsche Preise anzeigen.
* **ACSD-66084** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.9) - Es wurde das Problem behoben, bei dem `row_total_incl_tax` in der Antwort der Auftrags-API als Restwert von nahezu null anstelle von 0,00 für vollständig reduzierte Elemente zurückgegeben wurde.
* **ACSD-66118** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass bei Aktualisierung des Store-Ansichtscodes die [!UICONTROL Design Configuration] Einstellungen gelöscht wurden, wenn der Konfigurationscache nicht aktualisiert wurde.
* **ACSD-66139** (für Adobe Commerce >=2.4.7 &lt;2.4.8) - Es wurde das Problem behoben, bei dem GraphQL-Aufrufe zum Aufgeben einer Bestellung für einen nicht vorhandenen oder inaktiven Warenkorb einen **-Fehlercode zurückgaben.
* **ACSD-66301** (für Adobe Commerce und Magento Open Source >=2.4.6-p9 &lt;2.4.7 || >=2.4.7-p4 &lt;2.4.8) - Es wurde das Problem behoben, bei dem die Verschiebung von Produkten von einer Bestellung zurück in den Warenkorb im Admin zu einer Mengenabweichung führte.
...
* **AC-14984** (für Adobe Commerce und Magento Open Source >=2.4.6-p10 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - Behebt den Fehler *Ungültiger Frame-Typ 21* bei RabbitMQ-SSL-Verbindung.
...
* **ACSD-65750** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, dass die GraphQL-Abfrage `route` Produkte in den Inhaltstypen von Page Builder-Produkten in nicht ordnungsgemäßer Reihenfolge zurückgab.
* **ACSD-65775** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, bei dem die [!DNL REST] API-Bestelldetails falsche `base_row_total`- und `row_total` zurückgaben, wenn mehrere Mengen desselben Artikels bestellt wurden.
* **ACSD-65777** (für Adobe Commerce und Magento Open Source >=2.4.8 &lt;2.4.9) - Es wurde das Problem behoben, dass das Feld `types` für Produktbildtypen in der `MediaGallery` GraphQL-Anfrage fehlte.
...
* **ACSD-65913** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.9) - Es wurde das Problem behoben, bei dem [!DNL OpenSearch] einen Fehler *Illegal_Argument_Exception* für Kategorien mit Produkten mit demselben Preis ausgab.
* **ACSD-66041** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.9) - Es wurde das Problem behoben, dass Postcodes in Irland (IE) aufgrund einer fehlenden `CountryID` nicht nach Abholstandorten durchsucht werden konnten.
...
* **ACSD-64813** (für Adobe Commerce >=2.4.4 &lt;2.4.9) - Es wird das Problem behoben, dass das Aufheben der Zuweisung von Kategorien aus einem [!DNL B2B] freigegebenen Katalog über [!DNL REST] API zu lange dauert oder bei großen Katalogen zu lange dauert.
...
* **ACSD-65254** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass Kundinnen und Kunden keine E-Mail-Benachrichtigungen erhalten hatten, nachdem ihre E-Mail-Adressen in ihren Konten mit der `updateCustomerEmail` [!DNL GraphQL]-Mutation aktualisiert worden waren.
...
* **ACSD-65822** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass Bundle und konfigurierbare Produktmengen im Warenkorbbereich unter [!UICONTROL Customer's Activities] nicht korrekt angezeigt wurden.
...
* **ACP2E-3838** (für Adobe Commerce und Magento Open Source >=2.4.4-p9 &lt;2.4.4-p13 || >=2.4.5-p8 &lt;2.4.5-p12 || >=2.4.6-p6 &lt;2.4.6-p10 || >=2.4.7 &lt;2.4.7-p5) - Behebt das Problem, bei dem [!DNL Page Builder] CORS-Fehler das Speichern von Änderungen im Admin-Panel im Produktionsmodus verhindern.
...
* **ACSD-65195** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass die GraphQL-Mutation `createCompany` einen Fehler für ein Land ohne erforderliche Region auslöst.
...
* **ACSD-63520** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass Bilder, die über **[!UICONTROL Configurations]** im Admin-Bedienfeld hinzugefügt werden, nicht die maximale Upload-Größe erfüllen.
...
* **ACSD-65164** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.8) - Behebt das Problem, bei dem die Fehlermeldung *Einige der ausgewählten Elementoptionen derzeit nicht verfügbar sind* auftritt, wenn ein konfigurierbares Produkt mit einer einzigen benutzerdefinierten Kontrollkästchen-Option neu angeordnet wird.
...
* **ACSD-63883** (für Adobe Commerce >=2.4.4 &lt;2.4.7-p4) - Behebt das Problem, dass die Anforderungsliste in der GraphQL-Antwort eine falsche `items_count` zurückgibt.
...
* **ACSD-63323** (für Adobe Commerce >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass die Option **[!UICONTROL Select All]** nicht funktioniert, wenn Produkte zu einer Kategorie hinzugefügt werden. Darüber hinaus wird sichergestellt, dass die Paginierung und die Beschriftung der Datensatzanzahl korrekt funktionieren, wenn Produkte über das Popup-Raster zu einer Kategorie hinzugefügt werden.
...
* **ACSD-64111** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass beim Festlegen verschachtelter Bedingungen für eine Produktkomponente in [!DNL Page Builder] ein Fehler auftritt.
...
* **ACSD-48570** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Das Problem, dass die Seite zum Zurücksetzen des Kennworts nicht erreicht werden konnte, wurde behoben, indem auf den Link zum Zurücksetzen des Kennworts geklickt wurde, wenn **Speichern-Code zu URLs** *aktiviert* war, was zuvor dazu führte, dass die Anmeldeseite oder eine 404-Seite angezeigt wurde.[!UICONTROL Admin]
* **ACSD-62118** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebt das Problem, dass die Tabelle `sales_order_tax_item` nicht vollständig aktualisiert wird, wenn [!DNL B2B] Bestellungen mit der Bestellmethode aufgegeben werden.
* **ACSD-63067** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass alle Produktmengen falsch hervorgehoben sind und die Meldung *[!DNL Please specify the quantity of product(s).]* für alle Produkte in einem gruppierten Produkt angezeigt wird, wenn nur eine Menge falsch ist.
...
* **ACSD-63182** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass beim Speichern eines duplizierten Bundle-Produkts mit **[!DNL MSI]** *enabled* ein Fehler auftritt.
...
* **ACSD-63325** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Es wird das Problem behoben, bei dem beim Senden einer leeren [!DNL GraphQL]-Anfrage ein `Syntax Error: Unexpected <EOF>`-Fehler auftritt.
...
* **ACSD-59083** (für Adobe Commerce >=2.4.4 &lt;2.5.0) - Behebt das Problem, dass einige Datenbankaktualisierungsvorgänge zu einer *Basistabelle oder -ansicht nicht gefunden werden* führen, wenn das [!DNL mview] Update gleichzeitig ausgeführt wird.
* **ACSD-61622** (für Adobe Commerce und Magento Open Source >=2.4.6-p1 &lt;2.4.7) - Behebt das Problem, dass in der Antwort die kontospezifischen Raten von [!DNL FedEx] fehlen.
* **ACSD-61895** (für Adobe Commerce >=2.4.4 &lt;2.5.0) - Es wird das Problem behoben, dass die Abfrage „categories [!DNL GraphQL]&quot; Kategorien mit der Berechtigung **allow** zurückgibt, selbst wenn die Stammkategorie nicht über die Berechtigung **allow** verfügt.
* **ACSD-62212** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.5.0) - Behebt das Problem, dass der **E-**-Mail-Inhalt mit vergessenem Kennwort nicht in die Sprache der Store-Ansicht übersetzt wird.
* **ACSD-62481** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.5.0) - Behebt das Problem, dass der Warenkorb des Kunden auch dann leer wird, wenn **[!UICONTROL Persistence]** aktiviert ist.
* **ACSD-62629** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.5.0) - Behebt das Problem, dass eine in **[!UICONTROL Widgets]** verwendete Produktliste die Kategoriebedingung nicht widerspiegelt.
* **ACSD-62635** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.5.0) - Es wurde das Problem behoben, dass Multi-Store-bezogene Produkte in der [!DNL GraphQL] -Produktabfrage nicht ordnungsgemäß angezeigt wurden.
* **ACSD-62671** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.5.0) - Behebt das Problem, dass die [!DNL GraphQL]-Anfrage beim ersten Versuch keine aktuellen Adressinformationen zurückgibt.
* **ACSD-62689** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.5.0) - Behebt das Problem, dass der Kunde keine Kategorien in **[!UICONTROL Related Product Rules and Widgets]** nach {*}* hinzufügen kann.
* **ACSD-62708** (für Adobe Commerce und Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6-p2 || >=2.4.6-p8 &lt;2.4.7-p1) - Behebt das Problem, dass [!DNL TinyMCE] 7-Editor-Schriftgröße im Admin *PT* und nicht *PX* anzeigt. Jetzt können Sie die Schriftgröße auch in &quot;*&quot;* &quot;*&quot;*.
* **ACSD-62758** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.5.0) - Behebt das Problem, dass Produktvideos auf der Detailseite von **[!UICONTROL Configurable Product]** nicht korrekt gerendert werden, wenn die [!DNL URL] ausgewählte Optionen enthält.
...
* **ACSD-62965** (für Adobe Commerce >=2.4.7 &lt;2.5.0) - Behebt das Problem, dass eine `LocalizedException`-Nachricht nicht in der Antwort auf die Auftragserteilung [!DNL GraphQL] ist.
* **ACSD-63286** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebt das Problem, dass Produkte, die über [!DNL API] einem freigegebenen Katalog zugewiesen sind, erst dann in der Storefront angezeigt werden, wenn eine manuelle Neuindizierung ausgeführt wird.
...
* **ACSD-63244** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass ein [!DNL JavaScript]-Fehler [!DNL Google Maps] daran hindert, korrekt zu rendern. Es wurde das Problem behoben, bei dem es viele *Nicht erfasster TypeError: this.each ist keine Funktion* Fehler in der Konsole im [!UICONTROL Admin] gibt.
...
* **ACSD-62428** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Es wird das Problem behoben, bei dem `is_out_of_stock` im Katalogsuchindex auf einen falschen Wert eingestellt ist, wenn der [!DNL SKU] nicht als durchsuchbares Attribut festgelegt ist.
...
* **ACSD-58383** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass die Ausgabe einer Rückerstattung über das [!DNL REST API] mit zwei identischen Anfragen, die gleichzeitig ausgeführt werden, zu doppelten Gutschriften führt.
...
* **ACSD-58566** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Es wird das Problem behoben, bei dem [!DNL GraphQL] bei der Abfrage des `created_at` in der `addPurchaseOrderComment`-Mutation einen internen Server-Fehler zurückgibt.
...
* **ACSD-58735** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass ein Admin mit eingeschränktem Administratorzugriff die abgebrochenen Warenkörbe auf der Kundenkontoseite in der [!UICONTROL Admin] für eine zugehörige Website nicht anzeigen konnte.
* **ACSD-58828** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.8) - Es wird das Problem behoben, dass die Server-seitige Validierungsmeldung *Adresse erforderlich ist* angezeigt wird, wenn ein erforderliches Feld leer gelassen wird, zusammen mit der Client-seitigen Validierungsmeldung. Bei der Server-seitigen Validierung wird die Meldung für leere erforderliche Felder nicht angezeigt, und die Client-seitige Validierung verarbeitet die Fehlerbenachrichtigung mit dem Hinweis: *Dies ist ein erforderliches Feld.*
* **ACSD-60344** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass doppelte Auftragsbestätigungs-E-Mails gesendet wurden, wenn ein **[!UICONTROL Purchase Order]** mit automatischer Genehmigung verwendet wurde.
* **ACSD-61348** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass Wunschlistenelemente in einer Umgebung mit mehreren Websites über [!DNL GraphQL], aber nicht in der Storefront sichtbar sind.
...
* **ACSD-61785** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, dass das Aktualisieren des Attributs `reward_warning_notification` nicht über [!DNL GraphQL]-Mutation und [!DNL REST API]-Aufrufe möglich war, wodurch das Verhalten an `reward_update_notification` angepasst wurde.
...
* **ACSD-62793** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Es wurde das Problem behoben, dass `datetime` -Attribute in exportierten Daten die Zeitkomponente nicht enthielten. Wenn **[!UICONTROL Fields Enclosure]** *aktiviert* ist, werden Attributwerte in der `additional_attributes` Spalte außerdem in doppelte Anführungszeichen gesetzt.
* **ACSD-62332** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Es wurde das Problem behoben, dass die Abfrage zur Produktauflistung [!DNL GraphQL] auf eine `total_count` von 10.000 Produkten beschränkt war. Es wurde ein Problem behoben, bei dem [!DNL Live Search] bei der Abfrage über [!DNL GraphQL] die aktuelle Seite ** statt Seite *2* in den Suchkriterien setzte.
...
* **ACSD-61103** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebt das Problem, dass die Fehleranzahl in der Tabelle `customer_entity` nicht auf null zurückgesetzt wird, nachdem sich ein Kunde erfolgreich über API-Endpunkte angemeldet hat.
* **ACSD-61134** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Es wurde das Problem behoben, dass die Zahlungsmethode [!DNL Braintree Vault] im Checkout-Workflow automatisch deaktiviert wird, wenn ein Käufer seine Rechnungsadresse aktualisiert, indem er das Kontrollkästchen *[!UICONTROL My billing and shipping address are the same]* deaktiviert.
...
* **ACSD-61522** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Es wurde das Problem behoben, bei dem es möglich war, E-Mail-Adressen in die Felder *[!UICONTROL First Name]* und *[!UICONTROL Last Name]* des Gastkunden einzugeben und ungültige E-Mails zur Bestellbestätigung zu senden.
* **ACSD-61756** (für Adobe Commerce >=2.4.4 &lt;2.4.7) - Verbessert die Leistung von `AdvancedSalesRule` Filtern.
...
* **ACSD-61845** (für Adobe Commerce und Magento Open Source >=2.4.7-p1 &lt;2.4.8) - Behebt den Fehler, der auftritt, wenn eine Anfrage nur mit dem Accept-Header *text/html* gesendet wird.
...
* **ACSD-62485** (für Adobe Commerce >=2.4.4 &lt;2.4.6-p8 || >=2.4.7 &lt;2.4.8) - Es wurde das Problem behoben, dass `async.operations.all` Consumer beim Erstellen eines Unternehmens nicht mehr funktioniert.
...
* **ACSD-59930** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Verbessert die Performance der **[!UICONTROL Create]**-, **[!UICONTROL Save]**- und **[!UICONTROL Delete]**.
...
* **ACSD-60804** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass das Bearbeiten eines Kunden, der mit einem gelöschten Unternehmen verknüpft ist, den Fehler *Aufruf einer Mitgliederfunktion `getSuperUserId()` null* verursacht.
...
* **ACSD-61528** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Es wird das Problem behoben, bei dem das Abrufen von Rollen aus der [!UICONTROL Admin] mithilfe von [!DNL GraphQL] keine Ergebnisse zurückgibt.
* **ACSD-61553** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Es wurde das Problem behoben, dass **[!UICONTROL Cart Price Rule]** Rabatte falsch berechnet wurden, wenn mehrere Rabatte mit unterschiedlichen Prioritäten und **[!UICONTROL Maximum Qty Discount is Applied To]** auf das Produkt angewendet wurden.
...
* **ACSD-59952** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass beim Erstellen einer [!UICONTROL Shared Catalog] mit einer Gruppen-ID, die einer bestehenden [!UICONTROL Shared Catalog] zugewiesen ist, ein Fehler auftritt.
* **ACSD-60590** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Verbessert die Performance der Generierung [!UICONTROL Bestsellers Aggregated Daily Reports] für eine große Anzahl aufgegebener Bestellungen.
...
* **ACSD-60788** (für Adobe Commerce >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass benutzerdefinierte Skripte für [!DNL Google Tag Manager] aufgrund von CSP-Fehlern (Content Security Policy) nicht ausgeführt werden.
* **ACSD-61322** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebt das Problem, dass [!UICONTROL Products/Categories], die nicht dem [!UICONTROL Shared Catalog] für die Standardgruppe (General Group) zugewiesen sind, weiterhin in der XML-Sitemap enthalten sind.
* **ACSD-61366** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass der Befehl `setup:static-content:deploy --jobs 4` mit mehreren fehlgeschlagenen Aufträgen ausgeführt wird und *Port muss im Host-Parameter konfiguriert werden* Fehler, wenn der Port für die DB-Verbindung angegeben wird.
...
* **ACSD-60234** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebt das Problem, dass am [!DNL PayPal] ein falscher Betrag angezeigt wird, wenn der Rabatt von der Zahlungsmethode angewendet wird.
* **ACSD-59967** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7-p2) - Behebt das Problem, dass ein JavaScript-Fehler das korrekte Rendern von [!DNL Google Maps] verhindert.
...
* **ACSD-60538** (für Adobe Commerce >=2.4.7 &lt;2.4.8) - Behebt das Problem, dass, wenn ein Produkt in *[!UICONTROL All Store Views]* deaktiviert und nur in bestimmten Bereichen der Store-Ansicht aktiviert ist, die Produktattribute in der GraphQL-Antwort nicht korrekt angezeigt werden, was dazu führt, dass das Produkt nicht richtig angezeigt wird.
...
* **ACSD-60816** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebt das Problem, dass die vom APM-Agenten injizierten [!DNL New Relic Browser Monitoring] -Skripte nicht mit CSP (Content Security Policy) konform sind und ihre Ausführung verhindern.
...
* **ACSD-59280** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.5) - Behebt den Fehler *Aufruf der undefinierten Methode ReflectionUnionType::getName()* der bei der Installation der 2.4.4-pX-Versionen auftritt.
* **ACSD-45049** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.4-p8 || >=2.4.5 &lt;2.4.6) - Es wurde das Problem behoben, dass eine Attributeinstellung von Customer *[!UICONTROL Is required]* gemäß dem Website-Umfang in Admin nicht ordnungsgemäß funktioniert.
* **ACSD-46938** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.6) - Behebt das Problem mit der Performance der DB Trigger-Neuerstellung während `setup:upgrade`.
* **ACSD-48210** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Es wird das Problem behoben, bei dem beim Aktualisieren eines *[!UICONTROL Website Scope]* -Attributs in einer bestimmten Store-Ansicht die Attributwerte im globalen Umfang überschrieben werden.
...
* **ACSD-60441** (für Adobe Commerce und Magento Open Source 2.4.4-p9 || 2.4.5-p8 || 2.4.6-p6 || 2.4.7-p1) - Behebt das Problem beim Aktualisieren von Kunden über `V1/customers` [!DNL REST API]-Endpunkt bei Verwendung des vom Backend generierten Integrations-Zugriffstokens.
...
* **ACSD-57588** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Es wurde das Problem behoben, dass beim Versand einer Bestellung an mehrere Adressen bei der Verarbeitung der Regions-ID ein Fehler auftrat.
...
* **ACSD-58375** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Es wird das Problem behoben, dass die falsche *[!DNL YouTube API Key]*-Konfiguration beim Hinzufügen eines [!DNL YouTube] auf Store-Ansichtsebene einen Fehler verursacht.
...
* **ACSD-58163** (für Adobe Commerce >=2.4.3 &lt;2.4.7) - Behebt das Problem, dass ein *[!UICONTROL Cart Price Rule]* für einen Gastkunden keinen Rabatt vom entsprechenden *[!UICONTROL Customer Segment]* ohne Couponcode anwendet.
* **ACSD-57045** (für Adobe Commerce >=2.4.5 &lt;2.4.7) - Es wurde das Problem behoben, dass URL-Neuschreibungen zu unendlicher Seitenschleife führen, nachdem *[!UICONTROL Website Root]* in der *[!UICONTROL Hierarchy]* deaktiviert wurde.
* Aktualisierte Patches: ACSD-46815, ACSD-47027, ACSD-51683, ACSD-55031, ACSD-51819, ACSD-54965-v2

## v1.1.48 {#v1-1-48}

* **ACSD-55566** (für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.7) - Es wird das Problem behoben, bei dem die `mergeCart`-Mutation mit einem &quot;*Interner Server-Fehler*&quot; in der [!DNL GraphQL]-Antwort fehlschlägt, wenn Quell- und Zielkarten zusammengeführt werden, die dieselben Bundle-Elemente enthalten.
* **ACSD-56546** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebt das Problem, dass konfigurierbare und gebündelte Produkte in der Storefront als **Nicht vorrätig** angezeigt werden, wenn die **Display Out of Product Configuration** *Disabled*.
...
* **ACSD-56741** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebt die Fehlermeldung &quot;*Versuch, auf Array-Offset mit dem Wert vom Typ null zuzugreifen*&quot;, die während der `setup:upgrade` angezeigt wird, wenn die Datenbank einen benutzerdefinierten [!DNL MySQL]-Trigger enthält, der nicht mit dem Indexierungsmechanismus und der [!DNL MView] in Verbindung steht.
* **ACSD-57315** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, wenn in [!DNL PayPal Payflow Pro] jedes Mal, wenn auf die Schaltfläche [!UICONTROL Fetch] im Bildschirm &quot;**[!UICONTROL View transaction]**&quot; in Admin geklickt wird, eine neue Transaktion erstellt wird.
* **ACSD-57337** (für Adobe Commerce >=2.4.4 &lt;2.4.6) - Es wurde das Problem behoben, dass ein Admin-Benutzer mit Zugriffsbeschränkungen auf bestimmte Websites Unternehmen von allen Websites im **[!UICONTROL Companies]** Raster sehen kann.
...
* **ACSD-57565** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebt das Problem, dass im **[!UICONTROL Order]**-Dashboard falsche Bestellinformationen angezeigt werden, bis der Zeitraum aktualisiert wird. Das Dashboard zeigt jetzt die richtigen Bestellstatistiken beim ersten Laden an.
* **ACSD-57854** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Es wurde das Problem behoben, dass [!DNL GraphQL]-Anfragen des Produkts deaktivierte Kategorien in den Kategorieaggregationen zurückgaben.
...
* **ACSD-55241** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Es wurde das Problem behoben, dass *[!UICONTROL Used]*- und *[!UICONTROL Times Used]*-Attribute falsche Werte für generierte Coupons anzeigen, wenn sie beim Auschecken mit mehreren Adressen verwendet werden.
...
* **ACSD-57074** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Es wurde das Problem behoben, dass das benutzerdefinierte *Ja/Nein*-Attribut mit `attrbute_code`, das mit `price_` beginnt, bei der Indizierung nicht richtig funktioniert und Produkte mit solchen Attributen am Frontend nicht verfügbar sind.
...
* **ACSD-56842** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, dass die verzögerten Proxys und die verzögerten Proxy-Factories nach der Ausführung von `setup:di:compile` fehlen.
...
* **ACSD-56415** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebt das Problem, dass die partielle Preisindizierung aufgrund einer `DELETE`-Abfrage verlangsamt wird, wenn die Datenbank viele partielle Preisdaten zum Indizieren hat.
...
* **ACSD-56790** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Es wurde das Problem behoben, dass Benutzende beim Sortieren von Kategorieprodukten mit der Option **Von Lager nach unten** zum Admin-Dashboard weitergeleitet werden und der `Invalid security or form key. Please refresh the page` oben auf dem Bildschirm angezeigt wird.
...
* **ACSD-54885** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem des Auscheckens mehrerer Adressen, wenn der Administrator die *Anmeldung als Kunde*-Funktion verwendet.
...
* **ACSD-53925** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, dass der Administrator den CMS-Block mit dem Produktkarussell nicht speichern kann, wenn `catalog_product_price` Dimensionsmodus auf &quot;*&quot;*.
* **ACSD-52714** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, dass der Datumsfilter im Admin-Raster nicht funktioniert, wenn das Datumsformat auf *Y-m-d* festgelegt ist.
...
* **ACSD-54983** (für Adobe Commerce >=2.4.2 &lt;2.4.7) - Es wurde das Problem behoben, dass das Abrufen des Firmenbenutzers UID mit der GraphQL-Anfrage nicht möglich ist, wenn der Benutzerstatus auf *[!UICONTROL Inactive]* festgelegt ist.
* **ACSD-53309** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, dass im Titel *[!UICONTROL Regular Price]* keine Steuer vollständig angewendet wird, wenn die anpassbare Option ausgewählt ist.
...
* **ACSD-54890** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wird das Problem behoben, bei dem `aggregate_sales_report_bestsellers_data` [!DNL MySQL] verursacht, da `/tmp` Datenträger nicht genügend Speicherplatz hat.
* **ACSD-55112** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, dass die Schaltfläche *[!UICONTROL Submit review]* mehrmals ohne [!DNL Google reCAPTCHA v3] Validierung angeklickt werden kann.
* **ACSD-54264** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - Behebt das Problem, dass die Fehlermeldung *„Das angeforderte Attribut kann nicht aktualisiert werden. Zeilenkennung: store_id“* wird angezeigt, wenn ein Kunde versucht, mit einem verhandelbaren Angebot aus einer anderen Shop-Ansicht auszuchecken.
...
* **ACSD-55238** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Fehlerbehebungen beim Speichern des leeren Produkts *[!UICONTROL Meta Description]*.
...
* **ACSD-55628** (für Adobe Commerce >=2.4.4-p2 &lt; 2.4.5 || >=2.4.5-p1 &lt; 2.4.6) - Fehlerbehebungen beim Hochladen einer Datei im Unternehmensregistrierungsformular und beim Ersetzen einer Datei durch ein Kundenattribut in der Storefront.
...
* **ACSD-53118** (für Adobe Commerce >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, dass bei Anwendung von *[!UICONTROL Cart Price Rule]* mit Couponcode ein leeres -Attribut auf das Produkt angewendet wurde, was hätte zur Invalidierung der Regel führen müssen.
...
* **ACSD-54040** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6) - Behebt das Problem, dass das Feld *[!UICONTROL Created]* bei aktivierten B2B-Modulen in der Reihenfolge Details enthält.
* **ACSD-54319** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6) - Behebt das Problem, dass der Produktpreis im Bericht *[!UICONTROL Product in Cart]* den Wert Null aufweist.
...
* **ACSD-55004** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Es wurde das Problem behoben, dass der Validator beim Hochladen einer Importdatei, die größer als der in `php.ini` konfigurierte Wert ist, abstürzte.
* **ACSD-54989** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - Behebt das Problem, dass ein Unternehmensadministrator keine Bestellung aufgeben kann, wenn *[!UICONTROL Enable Purchase Orders]* auf *[!UICONTROL Yes]* und *[!UICONTROL Purchase Order]* auf *[!UICONTROL No]* gesetzt ist.
...
* **ACSD-55031** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.6) - Behebt den *Typ „Mixed“ kann nicht null sein* Fehler während der Kompilierung.
* **ACSD-54961** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, dass ein eingeschränkter Admin-Benutzer den *Produktüberprüfungsstatus nicht* aktualisieren kann.
...
* **ACSD-53583** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Verbessert die partielle Neuindizierungsleistung für ** und *Produktkategorien* Indexer.
...
* **ACSD-54342** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebt eine falsche Fehlermeldung *Fehler in der Datenstruktur: Werte werden gemischt* beim Importieren einer CSV-Datei ohne gültige Daten.
...
* **ACSD-53998** (für Adobe Commerce und Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - Behebt das Problem, dass ein **[!UICONTROL Dynamic Block]** basierend auf einem **[!UICONTROL Customer Segment]** nach der Abmeldung von einem Kundenkonto nicht ordnungsgemäß funktioniert.
* **ACSD-53204** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Fehlerbehebungen *Das Produkt kann nicht gespeichert werden.* Fehler bei gleichzeitigen Anfragen zum Hinzufügen von Bildern zur Produktgalerie mithilfe des `rest/V1/products/<sku>/media`-Endpunkts.
...
* **ACSD-47669** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6) - Fehlerbehebungen *Verletzung der Integritätsbeschränkung: 1452 Untergeordnete Zeile kann nicht hinzugefügt oder aktualisiert werden: Eine Fremdschlüssel-Beschränkung schlägt fehl* Fehler beim Importieren von CSV-Produkten.
...
* **ACSD-53824** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Es wurde das Problem behoben, dass der `UpdateMultiselectAttributesBackendTypes` Migrationsdaten-Patch während der `setup:upgrade` das Größenlimit für Datenbanktransaktionen überschreitet.
...
* **ACSD-53795** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem mit dem ungültigen Datentyp im `indexer_update_all_views` Cron-Auftrag.
...
* **ACSD-53750** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebt den *Rohrbruch oder Verbindungsabbruch* Fehler bei der Neuindizierung von Multithread-`catalog_product_price`.
* **ACSD-49843** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - Behebt das Problem, dass der Link beim Produktdownload nicht verfügbar ist, nachdem der bestellte Artikel mit der Online-Zahlungsmethode automatisch fakturiert wurde, wobei die Einstellung **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** aktiviert ist.
...
* **ACSD-50887** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, bei dem die Produktattribut-Eigenschaft *[!UICONTROL Use in Search Results Layered Navigation]* auf *Ja* gesetzt werden kann, ohne dass die *[!UICONTROL Use in search]*-Option auf *Ja* gesetzt wurde.
* **ACSD-51846** (für Adobe Commerce und Magento Open Source >=2.4.3-p2 &lt;2.4.6) - Behebt das *Interne*, das auftritt, weil nicht alle Ebenen der REST-API-Payload validiert werden.
...
* **ACSD-52736** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.6) - Behebt das Problem, dass eine *Warenkorbpreisregel*, die Anforderungen für konfigurierbare Produktmengen enthält, nicht wie erwartet funktioniert.
...
* **ACSD-53176** (für Adobe Commerce >=2.3.7 &lt;2.4.5) - Behebt das Problem, dass *Zugehörige Produktregel* mit *eine der*-Bedingung nicht mit Produkten übereinstimmt.
* **ACSD-51666** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Behebt den Fehler *Die Sitzung ist abgelaufen. Bitte erneut anmelden.* Dies geschieht, nachdem ein Kunde versucht, sich anzumelden.
...
* **ACSD-52041** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebt das Problem, bei dem die Fehlermeldung *[FEHLER] 5 Sekunden lang gerendert [!DNL Page Builder], ohne Sperren freizugeben.* wird im Chrome-Browser angezeigt, wenn Inhalte gespeichert werden, die mit [!DNL Page Builder] bearbeitet wurden.
* **ACSD-52095** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.6) - Es wurde das Problem behoben, bei dem der Wert `manage_stock` in der CSV-Datei nach dem Produktexport fälschlicherweise auf 0 gesetzt wurde.
...
* **ACSD-48070** (für Adobe Commerce >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, bei dem die Bearbeitung eines geplanten Updates zu einer Ausnahme führte.
* **ACSD-51890** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, dass die Schaltfläche [!UICONTROL Submit review] mehrmals ohne Validierung [!DNL Google reCAPTCHA] Version 3 angeklickt werden kann.
* **ACSD-51984** (für Adobe Commerce >=2.4.5 &lt;2.4.7) - Behebt das Problem, dass deaktivierte *[!UICONTROL Use Default Value]* und *[!UICONTROL non-default product field]* Werte für die zweite Website-, Store- und Store-Ansicht nicht gespeichert werden.
...
* **ACSD-52786** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.6) - Es wurde das Problem behoben, dass eine Katalogregelbedingung *SKU* für alle Produkte gilt, die mit der angegebenen SKU beginnen.
...
* **ACSD-52831** (für Adobe Commerce >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, dass Kunden keine verhandelbaren Angebotsbestellungen aufgeben können, wenn [!DNL Google reCAPTCHA v3 Invisible] aktiviert ist.
...
* **ACSD-50813** (für Adobe Commerce >=2.4.5 &lt;2.4.7) - Es wurde das Problem behoben, bei dem der Administrator nicht in der Lage war, gebündelte Produkte mit einem Schrägstrich in der SKU mit der Funktion [!UICONTROL Add Products by SKU] zur Administratorreihenfolge hinzuzufügen.
...
* **ACSD-51739** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebt das Problem, dass ein Fehler zurückgegeben wird, wenn die `structure_id` in einer CompanyTeam GraphQL-Anfrage angefordert wird.
* **ACSD-51857** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Es wurde das Problem behoben, bei dem die langsame Leistung des `aggregate_sales_report_bestsellers_data` Cron-Berichts zu großen Sales_Order- und `sales_order_item`-Datenbanktabellen auf die Art und Weise zurückzuführen war, wie die Hauptdatenabfrage geschrieben wurde.
* **ACSD-48448** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem, dass während der Auftragsstornierungen ein Race-Condition-Problem auftritt, das zu einem doppelten Eintrag in der `inventory_reservation`-Tabelle führt.
...
* **ACSD-50478** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.5) - Korrigiert den Datenbankrollback-Befehl für einen Fall, in dem der DB-Dump Trigger und einen *Trennzeichen* SQL-Befehl enthält.
...
* **ACSD-51645** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebt den Fehler, der beim Speichern einer neuen Warenkorb-Preisregel ausgelöst wird, wenn die Erweiterung `Magento_OfflineShipping` deaktiviert ist.
...
* **ACSD-50368** (für Adobe Commerce und Magento Open Source >= 2.4.3 &lt;2.4.5) - Behebt das Problem, dass die `group_id` der Kundin oder des Kunden ignoriert wird, wenn eine Kundin oder ein Kunde über die Async REST-API oder die Async Bulk REST-API erstellt wird.
* **ACSD-51497** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.0 || >= 2.4.1 &lt;2.4.7) - Es wurde das Problem behoben, dass ein Kunde eine Katalogseite nicht nach benutzerdefinierten Attributen des Dropdown-Typs sortieren kann.
...
* **ACSD-51431** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Es wird das Problem behoben, dass ein Indexerstatus *funktioniert* auch wenn im Änderungsprotokoll keine neuen Einträge vorhanden sind.
...
* **ACSD-49628** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Es wurde das Problem behoben, bei dem die [!UICONTROL Page Builder's] mehrfachen Fehler verhinderten, dass der Administrator ein Produkt ohne Inhaltsberechtigungen speichern konnte.
...
* **ACSD-51379** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, dass Änderungen am Seitentext-Inhalt über [!UICONTROL Page Builder] nicht gespeichert wurden.
...
* **ACSD-51528** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebt das Problem, dass die Spalte *x_forwarded_for* in der Tabelle *sales_order* Nullwerte aufweist.
...
* **ACSD-51291** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.4-p4 || >=2.4.5 &lt;2.4.5-p3) - Behebt das Problem, dass Administratoren mit eingeschränktem Zugriff auf eine Website dem Produkt Bilder/Videos hinzufügen können, die mehreren Websites zugewiesen wurden.
* Es wurden neue Versionen für ACSD-50336 hinzugefügt.
* Patches von ACSD-49970 ersetzt.

## v1.1.31 {#v1-1-31}

* **ACSD-50345** (für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.4 || >=2.4.4-p1 &lt;2.4.6) - Behebt das Problem, dass reCAPTCHA v2 nach Übermittlung einer fehlgeschlagenen Zahlung nicht neu geladen wird.
...
* **ACSD-51036** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.5) - Es wird das Problem behoben, bei dem Race-Bedingungen während gleichzeitiger REST-API-Aufrufe zu einer Überschreibungen der Versandstatusinformationen in der Tabelle [!UICONTROL Items Ordered] führen.
...
* **ACSD-49877** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Behebt das Problem, dass die automatische Videowiedergabe auf Mobilgeräten nicht funktioniert [!DNL Safari], wenn das Video direkt mit einer Remote-Videodatei und nicht mit einem Streaming-Service verknüpft ist.
* **ACSD-50165** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Fehlerbehebung *Die Datei kann nicht gelöscht werden. Warnung!Verknüpfung aufheben: Keine solche Datei oder*, wenn der JS/CSS-Cache vom Administrator geleert wird.
...
* **ACSD-49129** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6) - Es wurde das Problem behoben, bei dem das *[!UICONTROL content]*-Attribut (base64-Bildcode) nicht in `rest/V1/products/sku/media` Produktmedien-API-Antworten zurückgegeben wird.
...
* **B2B-2598** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Hinzufügen einer Caching-Funktion zu den [!UICONTROL availableStores]-, [!UICONTROL countries]-, [!UICONTROL country]-, [!UICONTROL currency]- und [!UICONTROL storeConfig] GraphQL-Abfragen.
...
* **ACSD-49822** (für Adobe Commerce >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, dass Aktualisierungen auf der Seite [!UICONTROL Requisition List] nicht auf der [!UICONTROL Print Requisition List] angezeigt wurden.
* **ACSD-48771** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebt das Problem beim Upgrade des Inhaltstyps für Spaltenblöcke von älteren [!DNL Page Builder] Versionen.
...
* **ACSD-50234** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.5) - Behebt das Problem mit dem falschen Kundennamen in der Bestätigungs-E-Mail bei einer Bestellung mit [!DNL PayPal].
...
* **ACSD-49849** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.6) - Es wurde das Problem behoben, bei dem Kunden-E-Mails durch E-Mails mit [!DNL PayPal] ersetzt wurden, wenn eine Bestellung über GraphQL bei [!DNL PayPal Express] aufgegeben wurde.
...
* **ACSD-49970** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebt die fehlerhafte Behandlung von GraphQL-Fehlern, wenn das [!DNL New Relic] Reporting aktiviert ist.
...
* **ACSD-49370** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, dass das Produktattribut *Datum/Uhrzeit* im GraphQL-Schema einen *FilterMatchTypeInput*-Typ aufweist.
...
* **ACSD-48059** (für Adobe Commerce >=2.3.7 &lt;2.4.7) - Es wurde das Problem behoben, dass Händler &quot;[!UICONTROL Match product by rule]&quot; nicht in der Kategorie speichern konnten.
* **ACSD-48216** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - Behebt das Problem, dass [!UICONTROL AUTO_INCREMENT] der [!UICONTROL inventory_source_item]-Tabelle beim [!UICONTROL UPDATE]-Vorgang zunimmt.
* **ACSD-47908** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - Behebt den Fehler „Ein Wert kleiner oder gleich 0 wird erwartet“ bei der Auswahl von Herkunft und Menge im Versandschritt während des Checkouts.
...
* **ACSD-48694** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.1 &lt;2.4.7) - Behebt das Problem, bei dem der Fehler „Ungültige Statusänderung angefordert“ einen Kunden daran hindert, eine Bestellung aufzugeben.
...
* **ACSD-47937** (für Adobe Commerce und Magento Open Source 2.4.4 || >=2.4.5 &lt;2.4.6) - Es wurde das Problem behoben, bei dem Benachrichtigungen zu Preisnachlässen aufgrund des Caching auf Anwendungsebene nicht immer gesendet wurden.
...
* **ACSD-45168** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6) - Es wurde das Problem behoben, dass SEO-freundliche URLs nicht für Produkte generiert wurden, deren *url_key*-Attribute auf Store-Ansichtsebene überschrieben wurden.
...
* **ACSD-46770** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.6) - Es wurde das Problem behoben, dass E-Mails zu Admin-Bestellungen gesendet wurden, selbst wenn die E-Mail mit der *E-Mail-Bestellbestätigung* deaktiviert war.
...
* **ACSD-47920** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.6) - Es wurde das Problem behoben, dass Bestellungen als Gastbenutzer über die REST-API aufgegeben werden können, selbst wenn die Option *Gastauschecken zulassen* deaktiviert ist.
...
* **ACSD-46520** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.5*) - Behebt das Problem, dass Benutzende bei Rückerstattung über das Store-Guthaben einen falschen Bestellstatus erhalten.
...
* **ACSD-44851** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.6*) - Es wurde das Problem behoben, bei dem eine Kategorie mit Unterkategorien nicht geöffnet oder erweitert werden kann.
* **ACSD-46815** (*für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.6*) - Es wurde das Problem behoben, bei dem die Kategoriestrukturanforderung auf 20 Kategorien beschränkt war.
* **ACSD-45675** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.6*) - Es wurde das Problem behoben, bei dem der Produktexport Kategorienamen aus dem Bereich *Standardspeicheransicht* verwendet.
...
* **MDVA-42768-V2** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Behebt das Problem, dass konfigurierbares Produkt den regulären Preis als *0 anzeigt* wenn *Display Out-of-Stock* *Ja* ist.
* Aktualisierte Patches: MDVA-44562, ACSD-46213, MDVA-41305, MDVA-38346, MDVA-13203.
* Veralteter Patch: MDVA-42768.

## v1.1.19 {#v1-1-19}

* **ACSD-46213** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Es wurde das Problem behoben, bei dem die Kategoriestrukturanforderung auf 20 Kategorien beschränkt war.
* **ACSD-45781** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.2*) - Behebt das Problem, dass das Suchfeld für die Storefront auf Mobilgeräten nicht angezeigt wird.
...
* **ACSD-46404** (*für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.5*) - Es wurde das Problem behoben, dass sich ein Administrator nach einem Upgrade auf 2.4.4 nicht anmelden konnte.
...
* **ACSD-46146** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.6*) - Es wurde das Problem behoben, dass zwei E-Mails zur Bestellbestätigung nach einer Bestellung vom Administrator versendet wurden.
* **ACSD-45255** (*für Adobe Commerce >=2.4.3 &lt;2.4.6*) - Behebt eine Ausnahme auf der Seite „Low Stock Report“ für einen Benutzer mit eingeschränktem Administratorzugriff.
* **ACSD-45488** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6*) - Es wird das Problem behoben, bei dem ein konfigurierbares Produkt mit mehreren Quellen nicht automatisch an Lager zurückgegeben wird.
* **ACSD-45754** (*für Adobe Commerce und Magento Open Source >=2.3.1 &lt;2.4.6*) - Es wurde das Problem behoben, bei dem Belohnungspunkte nach Anwendung eines Coupons auf den Warenkorb nicht hinzugefügt wurden.
...
* **ACSD-43887** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem auf der Kassenzahlungsseite falsche Details angezeigt werden, wenn Bestellungen für Unternehmen aktiviert sind.
...
* **ACSD-44591** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.6*) - Behebt den Fehler, der auftritt, wenn eine Bestellung ohne CAPTCHA-Bestätigung aufgegeben wird.
* **ACSD-45520** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.6*) - Es wird das Problem behoben, bei dem Musteroptionen auf der Produktdetailseite nicht vorausgewählt sind, wenn ein Benutzer konfigurierbare Produkte aus dem Warenkorb bearbeitet.
* **ACSD-45169** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.6*) - Behebt das Problem, dass [!DNL Visual Merchandiser] nach einer Staging-Aktualisierung nicht den richtigen Bestand und Preis für ein konfigurierbares Produkt anzeigt.
...
* **MDVA-44703** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem die Bestellsummen im Bericht Bestellungen für den Benutzer mit eingeschränkten Administratorrechten falsch berechnet wurden.
...
* **MDVA-44562** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.2-p2*) - Es wurde das Problem behoben, bei dem die nicht standardmäßige Store-ID für Angebotselemente von der standardmäßigen Store-ID außer Kraft gesetzt wurde, obwohl die GraphQL-Anfrage von der nicht standardmäßigen Store-Ansicht stammte.
* **MDVA-43167** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Es wurde das Problem behoben, dass die Massenaktion „Raster für Admin-Bestellungen“ nicht für mehrere Seiten gilt, wenn der Admin-Benutzer alle Bestellungen auswählt.
...
* **MDVA-42509** (*für Adobe Commerce und Magento Open Source >=2.3.3 &lt;2.4.4*) - Behebt das Problem, dass für eine Schnellbestellung keine CSV-Datei hochgeladen werden konnte, was zu einem Fehler *Cookie kann nicht gesendet werden* führte.
...
* **MDVA-43983** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.5*) - Es wurde das Problem behoben, dass Produkte *Nicht einzeln sichtbar* weiterhin in den Ergebnissen der erweiterten Katalogsuche angezeigt werden.
...
* **MDVA-43605** (*für Adobe Commerce und Magento Open Source >=2.3.1 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem Bestelldaten bei Verwendung der REST-API negative Werte für Zeilensummen zurückgeben.
...
* **MDVA-44147** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.5*) - Behebt das Problem, dass eine GraphQL-Anfrage keine Anforderungslisten zurückgibt.
* **MDVA-44505** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.3*) - Behebt die Probleme, bei denen GraphQL bei der Anwendung von Belohnungspunkten die Gesamtsumme nicht aktualisiert und bei denen Store-Guthaben während der Auftragserteilung mehrmals angewendet wird.
* Aktualisierte Patches: MDVA-29148, MDVA-36464-V5, MDVA-42584, MDVA-39993-V2.

## v1.1.13 {#v1-1-13}

* **MDVA-42969** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.3*) - Behebt das Problem, dass die Zugehörige Produktregel nur funktioniert, wenn das Kundensegment auf &quot;*All“* ist.
* **MDVA-39605** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;2.4.5*) - Es wurde das Problem behoben, dass die Redis-Cache-TTL (Ablaufdatum) einen falschen Wert aufweist.
* **MDVA-43862** (*für Adobe Commerce und Magento Open Source >=2.3.3 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem Kunden Warenkorbelemente aufgrund eines GraphQL-Fehlers (UpdateCartItems *Mutation) nicht* können.
...
* **MDVA-43451** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.5*) - Behebt das Problem, bei dem der Fehler &quot;*angeforderte Store wurde nicht gefunden. Überprüfen Sie den Store und versuchen Sie es erneut.* Wird beim Konfigurieren eines freigegebenen Katalogs für eine bestimmte Website angezeigt.
* **MDVA-43491** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;2.4.5*) - Behebt das Problem, dass die Basisbildbeschriftung beim Importieren von Produkten für eine Website mit mehreren Geschäften nicht aktualisiert wird.
...
* **MDVA-42046** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;=2.3.5-p2 || >=2.4.0 &lt;2.4.5*) - Behebt das Problem, dass beim Aktualisieren eines Produkts einem Produktattribut mit einem Datumseingabefeld ein falscher Wert zugewiesen wird.
...
* **MDVA-39713** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.3.6*) - Behebt das Problem, dass Benutzende die Startzeit für ein aktives geplantes Update bearbeiten können.
* **MDVA-42410** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem in Couponberichten nur die Standard-Basiswährung angezeigt wurde.
* **MDVA-41136** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.5*) - Behebt das Problem, dass das Ablaufdatum des `mage-cache-sessid` nicht verlängert wird, was zu einer Bereinigung der Kundendaten führt.
* **MDVA-39993** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;=2.3.7-p2 || >=2.4.0 &lt;2.4.4*) - Es wurde das Problem behoben, dass Inventaränderungen, die über API vorgenommen wurden, nicht auf der Produktseite im Frontend widergespiegelt werden.
...
* **MDVA-42645** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem der Administrator Prämienpunkte nicht zurückerstatten kann, wenn die Funktion „Gutschrift speichern“ deaktiviert ist.
...
* **MDVA-41628** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.5*) - Es wurde das Problem behoben, dass bestehende Administratoren mit Zugriffsbeschränkungen auf die neuen Ressourcen zugreifen können, wenn neue Module hinzugefügt werden.
* **MDVA-43348** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.5*) - Behebt das Problem, dass die GraphQL-Anfrage für Geschenkkarten einen Fehler anzeigt, wenn `gift_card_options` &quot;*uid“*.
...
* **MDVA-42950** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.5*) - Behebt das Problem, dass Videos auf der Produktseite nicht wiedergegeben werden.
* **MDVA-42689** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Adobe Commerce beim Aktualisieren von Produktkategorien während des Imports einen Fehler *Integritätsbeschränkungsverletzung* ausgibt.
...
* **MDVA-43731** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem *Suchsynonyme* nicht mehr funktionieren, wenn in *Mindestbedingungen für übereinstimmende Werte*.
* **MDVA-43232** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem das Sortieren von Produkten in [!DNL Visual Merchandiser] nach dem Sonderpreis Nach unten/oben beim Speichern der Kategorie einen Fehler verursachte.
* **MDVA-43726** (*für Adobe Commerce und Magento Open Source >=2.3.3 &lt;2.4.3*) - Es wird das Problem behoben, bei dem die Katalogpreisregel, die auf der Attributübereinstimmung auf Store-Ebene basiert, nach der teilweisen Neuindizierung nicht angewendet werden kann.
* Aktualisierte Patches: MDVA-36464, MDVA-37478, MDVA-38608.
* Es wurden Patches für Metadaten für die [!DNL Site-Wide Analysis Tool] hinzugefügt.

## v1.1.11 {#v1-1-11}

* **MDVA-42790** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.5*) - Behebt das Problem, dass Produktpreisattribute für eine bestimmte Website nicht über die REST-API aktualisiert werden können.
...
* **MDVA-42269** (*für Adobe Commerce und Magento Open Source >=2.4.3-p1 &lt;2.4.5*) - Behebt das Problem, dass ein Admin-Benutzer sich aufgrund des Fehlers *TypeError: strtotime() nicht beim Admin anmelden kann und erwartet, dass Parameter 1 eine Zeichenfolge, null oder ein gegebener* ist.
...
* **MDVA-42237** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.5*) - Behebt das Problem, dass ein konfigurierbarer Sonderpreis für ein Produkt nach Änderungen am Unterproduktpreis nicht aktualisiert wird.
...
* **MDVA-38728** (*für Adobe Commerce und Magento Open Source >=2.3.2 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem eine Massenattribut-Aktualisierung eine URL-Umschreibung für den Default Store nur nach einer Änderung der *Produktsichtbarkeit)*.
* **MDVA-43091** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.4*) - Behebt das Problem, dass die Bestellung eines Bundle-Produkts vom Administrator im Backend den Fehler *Sie können für dieses Produkt keine Dezimalmenge verwenden* ausgibt.
* **MDVA-40816** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem verwandte Produktregeln Produkte aus Kategorien anzeigen, die in den Regelbedingungen nicht definiert wurden.
...
* **MDVA-39181** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem verwandte Produktregeln Produkte aus Kategorien anzeigen, die in den Regelbedingungen nicht definiert wurden.
* **MDVA-42584** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Behebt das Problem, dass der konfigurierbare Lagerstatus im Backend nach dem Ändern der Menge und des Lagerstatus über Import oder API nicht aktualisiert wird.
* **MDVA-40175** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.3*) - Behebt das Problem, dass *Click to change shipping method* keine Optionsfelder anzeigt, um Versandmethoden in Admin während der Neubestellung auszuwählen.
* **MDVA-42768** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem konfigurierbares Produkt den regulären Preis als 0 anzeigt, wenn *Display Out-of-Stock* Ja ist.
* **MDVA-43201** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Behebt das Problem, dass bei der Kundenanmeldung bei der Verwendung des DOB-Attributs mit bestimmten Gebietsschemata ein Fehler auftritt.
* Aktualisierte Patches: MDVA-35092, MDVA-33970.

## v1.1.9 {#v1-1-9}

* **MDVA-38346** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.5*) - Es wurde das Problem behoben, dass Datumsfilter nicht richtig funktionieren, wenn die Adobe Commerce-Zeitzone sich von der Zeitzone der lokalen Umgebung unterscheidet.
* **MDVA-42657** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem Admin-Benutzende in den Segmentbedingungen des Kunden keine Kategorien auswählen konnten.
* **MDVA-42806** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Es wird das Problem behoben, bei dem jedes Mal, wenn ein bestehendes Unternehmen über die REST-API aktualisiert wird, eine E-Mail *Neue* gesendet wird.
* **MDVA-37984** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.5*) - Es wurde das Problem behoben, bei dem die Funktion [!DNL Visual Merchandiser] *Match product by rule* Produkte nicht korrekt mit Staging-Updates filtert.
...
* **MDVA-39163** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;2.4.5*) - Behebt das Problem, dass Versandmethoden nicht verfügbar sind, wenn ein neuer Benutzer registriert wird und Produkte im Warenkorb aus der Gastsitzung stammen.
* **MDVA-38626** (*für Adobe Commerce und Magento Open Source >=2.3.3 &lt;2.4.5*) - Behebt das Problem, dass der Administrator bzw. die Administratorin nicht in der Lage ist, über die [!DNL PayPal Payflow Pro] Bezahlung eine Bestellung im Backend aufzugeben.
* **MDVA-38666** (*für Adobe Commerce und Magento Open Source >=2.3.2 &lt;2.3.6*) - Behebt das Problem, dass Admin-Benutzerinnen und -Benutzer die konfigurierbaren Produktoptionen im Warenkorb des Kunden nicht ändern können.
* **MDVA-38526** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Admin-Benutzende nicht auf die [!DNL Site-Wide Analysis tool] zugreifen können.
* Aktualisierte Patches: MDVA-40101.

## v1.1.8 {#v1-1-8}

* **MDVA-41215** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Es wird das Problem behoben, bei dem Benutzer nach dem Setzen des Cookies *mage-messages* den Fehler „500“ erhalten, sofern es bereits vorhanden ist, aber keine neuen Nachrichten vorhanden sind.
* **MDVA-41139** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.4*) - Behebt das Problem, dass konfigurierbare Produkte nach dem Produktimport nicht mehr vorrätig sind, wenn die Menge=0 eines einfachen Produkts für eine seiner Quellen ist.
* **MDVA-42326** (*für Adobe Commerce und Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Behebt das Problem, dass Kunden nach einem Sitzungs-Timeout beim Checkout einen Fehler erhalten, selbst wenn der persistente Warenkorb aktiviert ist.
* **MDVA-42341** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Behebt das Problem, dass die `categoryList` GraphQL-Abfrage die Ergebnisse nicht filtert, wenn eine Anfrage die Store-Kopfzeile hat.
* **MDVA-38393** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Es wurde das Problem behoben, dass Katalogregeln bei einem konfigurierbaren Produkt nicht mehr funktionieren, wenn dessen einfaches Produkt umbenannt wurde.
* **MDVA-39153** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Behebt das Problem, dass ein Rabattbetrag bei der Neubestellung im Admin falsch berechnet wird.
...
* **MDVA-40311** (*für Adobe Commerce und Magento Open Source >=2.4.2-p2 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Admin-Benutzer die Fehlermeldung &quot;*oder Formularschlüssel“ erhalten. Bitte aktualisieren Sie die Seite* nachdem Sie sich beim Administrator angemeldet haben, wenn der benutzerdefinierte Administratorpfad konfiguriert und der geheime Schlüssel aktiviert ist.
* **MDVA-41631** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.4*) - Es wird das Problem behoben, bei dem Benutzende einen Fehler erhalten, wenn sie versuchen, Bestellinformationen ohne einen optionalen Wert (*)* GraphQL abzurufen.
...
* **MDVA-40550** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;2.4.4*) - Behebt das Problem mit fehlenden Produkten im Frontend während der Neuindizierung.
* **MDVA-40120** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.4*) - Es wurde das Problem behoben, dass die Sortierung von GraphQL nach DESC/ASC bei Produkten mit gleicher Relevanz oder demselben Preis nicht funktionierte.
...
* **MDVA-40609** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Behebt das Problem, dass deaktivierte Produktdaten in der `cataloginventory_stock_status`-Indextabelle fehlen und falsche deaktivierte Produktmengen angezeigt werden.
* **MDVA-39031** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.4*) - Behebt das Problem, dass das Hinzufügen eines Produkts zum Warenkorb über GraphQL möglich ist, auch wenn es nicht der Ziel-Website zugewiesen ist.
...
* **MDVA-27456** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;2.3.7*) - Es wurde das Problem behoben, bei dem beim Laden von [!DNL Swagger] ein Fehler auftrat.
* **MDVA-32776** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.2*) - Der Lagerstatus wird nicht aktualisiert, wenn eine Bestellung aufgegeben, aber nicht versendet wird.
...
* **MDVA-41236** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Behebt das Problem, dass es nicht möglich ist, ein neues oder ein vorhandenes geplantes Update für ein Produkt zu erstellen, wenn das Enddatum zuvor entfernt wurde.
...
* **MDVA-40545** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Behebt das Problem, dass nur der erste Knoten für eine Seite abgerufen wurde, selbst wenn für dieselbe Seite mehr als ein Knoten vorhanden war.
* **MDVA-41164** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3-p1*) - Es wurde das Problem behoben, dass ein Admin-Benutzer ein Unternehmen mit einem benutzerdefinierten Datei- oder Bildtyp-Kundenattribut nicht speichern oder bearbeiten kann.
* **MDVA-39229** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Behebt das Problem, das dazu führt, dass nach der Aktualisierung der Startzeit für das Staging-Update der Katalogregel der folgende Fehler angezeigt wird: *Cron Job staging_synchronize_entities_period hat einen Fehler: Das aktive Update kann nicht gelöscht werden.*
* **MDVA-40619** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Änderungen an der CMS-Seitenhierarchie einen 500-Fehler verursachen, wenn versucht wurde, eine CMS-Seite inline zu bearbeiten.
* **MDVA-41061** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Es wurde das Problem behoben, bei dem der Lagerstatus auf „skalierbar“ zurückgesetzt wurde, wenn ein Produkt vom Administrator gespeichert wurde.
...
* **MDVA-37748** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Behebt das Problem, dass eine GraphQL-Abfrage Produkte zurückgibt, die keinem freigegebenen Katalog zugewiesen sind.
...
* **MDVA-40101** (*für Adobe Commerce und Magento Open Source >=2.3.2 &lt;2.4.0*) - Behebt das Problem, dass Artikel nach einer erfolgreichen Bestellplatzierung per [!DNL PayPal Express] Checkout nicht aus dem Mini-Warenkorb entfernt werden.
* **MDVA-40401** (*für Adobe Commerce und Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Behebt das Problem, dass sich der Couponnutzungswert ändert, selbst wenn eine Bestellung fehlschlägt.
* **MDVA-40537** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;=2.4.0-p1*) - Behebt das Problem, dass Benutzer beim Erstellen einer Store-Ansicht einen Fehler erhalten, wenn mehrere CMS-Seiten mit demselben URL-Schlüssel vorhanden sind.
* **MDVA-37725** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;=2.4.3-p1*) - Behebt das Problem, dass E-Mails zu asynchronen Bestellungen, die von nicht standardmäßigen Websites gesendet werden, Logo-URLs von der Standard-Website enthalten.
* **MDVA-39482** (*für Adobe Commerce und Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Behebt das Problem, dass ein Produkt nicht vorrätig ist, wenn es mit „0“ Menge importiert wird, wenn Nachbestellungen aktiviert sind.
...
* **MC-42528** (*für Adobe Commerce und Magento Open Source >=2.4.3 &lt;=2.4.3-p1*) - Behebt das Problem, bei dem die `categoryList` GraphQL-Abfrage sowohl zugewiesene als auch nicht zugewiesene Kategorien zurückgibt.
* **MDVA-29400** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*) - Behebt das Problem mit duplizierten Bestellungen, die mit [!DNL PayPal Express Checkout] aufgegeben werden.
* **MDVA-26005** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;=2.3.5-p2*) - Behebt das Problem, dass es nicht möglich ist, eine Kategorie in einer Kategoriestruktur für Warenkorb-Preisregelbedingungen auszuwählen.
...
* **MDVA-40262** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Behebt das Problem, dass GraphQL-Suchanfragen in gängigen Suchbegriffen im Admin-Bereich nicht angezeigt werden.
* **MDVA-40601** (*für Adobe Commerce und Magento Open Source >=2.3.1 &lt;=2.4.2-p2*) - Behebt das Problem, dass Benutzende einen Fehler erhalten, wenn sie versuchen, Informationen über die Kategorie abzurufen, die durch das geplante Update über GraphQL geändert wurde.
* **MDVA-37234** (*für Adobe Commerce und Magento Open Source >=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*) - Behebt das Problem, dass beim mehrmaligen Hinzufügen eines Artikels zum Warenkorb (parallele Anfrage) für dieselbe SKU ein doppelter Zeileneintrag für dieselbe Warenkorb-ID erstellt wird.
* **MDVA-33606** (** für Adobe Commerce und Magento Open Source >=2.4.1 &lt;=2.4.2-p2 *) - Behebt das Problem, dass beim Speichern einer CMS-Seite, die einer Hierarchie zugewiesen wurde,* Fehler „Eindeutige Einschränkungsverletzung“ angezeigt wird.
* **MDVA-31590** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;=2.4.1-p1*) - Behebt das Problem, dass Benutzer Attribute nicht stapelweise mithilfe asynchroner MySQL-Warteschlangen aktualisieren können.
...
* **MDVA-38929** (*für Adobe Commerce und Magento Open Source >=2.3.0 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem die Rechnung mit FPT eine falsche Gesamtsumme anzeigt, wenn die Bestellung über den Ladenkredit bezahlt wird.
...
* **MDVA-39195** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Behebt das Problem, dass die Schaltfläche *Zum Warenkorb hinzufügen* auf der Kategorieseite inaktiv war, wenn die Umleitung zum Warenkorb aktiviert war.
* **MDVA-37115** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Es wurde das Problem behoben, bei dem ein unnötiger *Nur 0 verbleibt*-Hinweis auf der konfigurierbaren Produktseite angezeigt wurde.
* **MDVA-39521** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Benutzende Versandadressen auf dem Warenkorb nicht mit einer leeren Telefonnummer über GraphQL festlegen konnten.
* **MDVA-39384** (*für Adobe Commerce und Magento Open Source >=2.3.1 &lt;=2.3.6 || >=2.4.1 &lt;=2.4.3*) - Behebt das Problem, dass das benutzerdefinierte Kundenattribut für einen Unternehmensbenutzer nicht gespeichert wird.
* **MDVA-39043** (*für Adobe Commerce und Magento Open Source >=2.3.4 &lt;=2.4.3*) - Es wird das Problem behoben, bei dem Admin-Benutzende mit eingeschränktem Zugriff einen Fehler erhalten, wenn sie versuchen, das Widget *Produkte* zur CMS-Seite hinzuzufügen.
...
* **MDVA-39986** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.3*) - Es wurde das Problem behoben, durch das Benutzende in der Admin von MacOS keine Bestellung über den Safari-Browser aufgeben konnten.
* **MDVA-38447** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.4*) - Behebt zwei Probleme: Wenn die *Nicht einzeln sichtbar* konfigurierbaren untergeordneten Produkte in der GraphQL-Antwort zurückgegeben werden und die MySQL-Abfrage für GraphQL-Produkte mit dem Kategoriefilter optimiert wird.
* **MDVA-40134** (*für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.3*) - Behebt das Problem, dass GraphQL keine verwandten Produkte zurückgibt, wenn der freigegebene Katalog aktiviert ist.
* **MDVA-39935** (*für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.4*) - Behebt das Problem, dass GraphQL konfigurierbare untergeordnete Produkte zurückgibt, die auf Website-Ebene deaktiviert sind.

## v1.1.1 {#v1-1-1}

* **MDVA-36021** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem der Fehler *Aufruf an ein Mitglied der Funktion getId()* auf der Seite mit den Bestelldetails in Admin angezeigt wurde.
...
* **MDVA-39305** (*für Adobe Commerce und Magento Open Source >=2.4.0 &lt;=2.4.2-p1*) - Behebt das Problem, dass registrierte Kunden sich nicht mit aktiviertem Google ReCaptcha anmelden können.
...
* **MDVA-38827** (*für Adobe Commerce >=2.3.3-p1 &lt;2.4.4*) - Es wurde das Problem behoben, bei dem Kunden eine E-Mail mit einer Fehlermeldung erhalten, in der sie eine Bestellung erhalten hatten.
...
* **MDVA-34680** (*für Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;2.4.3*) - Behebt das Problem, dass die Erstellungszeit des Kundenkontos im Kundenraster nicht korrekt gefiltert wird.
...
* **MDVA-38608** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Behebt das Problem, dass temporäre Tabellen nicht gelöscht werden, wenn die Neuindizierung nicht erfolgreich abgeschlossen wurde.
...
* **MDVA-37082** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Behebt das Problem, wenn beim Speichern des benutzerdefinierten Bestands an gruppierten Produkten das Produkt im Frontend nicht vorrätig angezeigt wird.
* **MDVA-36572** (*für Adobe Commerce >=2.3.5 &lt;2.4.3*) - Behebt das Problem, wenn Aktualisierungen von Gutschriften nicht mehr zu doppelten Produktreservierungsaktualisierungen in der Datenbank führen.
* **MDVA-38132** (*für Adobe Commerce >=2.3.3 &lt;2.4.3*) - Behebt das Problem, wenn das Admin-Bedienfeld aufgrund eines Fehlers *zu viele Weiterleitungen* nicht erreichbar ist.
...
* **MDVA-34330** (*für Adobe Commerce >=2.3.1 &lt;=2.4.2-p1*) - Behebt das Problem, dass Bestellungen im Bestellraster nicht nach der Admin-Zeitzone gefiltert werden.
...
* **MDVA-37224** (*für Adobe Commerce >=2.3.3 &lt;=2.4.2-p1*) - Es wurde das Problem behoben, dass Kunden für ein verhandelbares Angebot nicht mit [!DNL PayFlow Pro] eines anderen Produkts im Warenkorb bezahlen können.
* **MDVA-36286** (*für Adobe Commerce >=2.3.6 &lt;=2.4.2-p1*) - Behebt das Problem, dass die Widget-Vorschau von Page Builder-Produkten unterbrochen wird, wenn dieselbe SKU in Unterkategorien eine andere Position hat.
* **MDVA-30186** (*für Adobe Commerce >=2.3.4 &lt;=2.3.5-p2, >=2.4.0 &lt;=2.4.0-p1, >=2.4.2 &lt;=2.4.2-p1*) - Behebt das Problem, bei dem Attributoptionen in der GraphQL-Antwort nach Optionswert anstatt nach Attributelementanzahl sortiert werden.

## v1.0.22 {#v1-0-22}

* **MDVA-36718** (*für Adobe Commerce >=2.3.0 &lt;=2.4.2*) - Behebt das Problem, dass die alten benutzerdefinierten Optionen nach einer Änderung über die API erhalten bleiben.
...
* **MDVA-37182** (*für Adobe Commerce >=2.4.1 &lt;=2.4.2*) - Es wurde das Problem behoben, dass falsche Suchergebnisse sowohl in [!DNL Elasticsearch] 6 als auch in Version 7 erhalten wurden.
* **MDVA-36253** (*für Adobe Commerce >=2.4.0 &lt;=2.4.1-p1*) - Behebt das Problem, dass nach dem Löschen des Artikels im Mini-Warenkorb ein falscher Zwischenwert angezeigt wird.
...
* **MDVA-36424** (*für Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 || >=2.0.0 &lt;2.2.0*) - Behebt das Problem, dass an Page Builder-Elemente angehängte Medienbilder verschwinden, wenn der Inhalt wiederholt bearbeitet wird, wenn sich die Backend-Basis-URL von der Storefront-Basis-URL unterscheidet.
...
* **MDVA-36170** (*für Adobe Commerce >=2.3.4 &lt;2.4.2*) - Damit wird das Problem behoben, bei dem die GraphQL-Abfrage mithilfe des Kategorie-Cache-Tags nicht zwischengespeichert wird.
...
* **MDVA-11189** (*für Adobe Commerce >=2.3.0 &lt;2.3.5*) - Behebt das Problem, wenn nach dem Import einer CSV-Datei zur Aktualisierung des Produktbestands Zeilen aus der `cataloginventory_stock` gelöscht werden.
* **MDVA-26639** (*für Adobe Commerce >=2.3.3-p1 &lt;2.3.6*) - Es wurde das Problem behoben, dass bei der Erstellung einer neuen E-Mail-Vorlage für Auftragsbestätigungen die Bestellelemente in der Auftrags-E-Mail fehlen.
...
* **MDVA-22026** (*für Adobe Commerce >=2.3.2 &lt;2.3.4*) - Es wurde ein Problem behoben, bei dem der Import von Produkten aus CSV-Dateien, einschließlich Bildern von externen URLs, fehlschlug.
...
* **MDVA-35356** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Behebt das Problem mit der falschen Rückgabe von Filialguthaben nach teilweise fakturierter Auftragsstornierung.
* **MDVA-33289** (*für Adobe Commerce >=2.4.0 &lt;2.4.3*) - Es wurde das Problem behoben, bei dem der unformatierte JavaScript-Code während des Checkouts in der Rechnungsadressen-Benutzeroberfläche angezeigt wird, wenn [!DNL Google Tag Manager] aktiviert ist.
* **MDVA-35982** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Behebt das Problem, dass Admin-Benutzer, die auf eine bestimmte Website beschränkt sind, keine Sendung für die auf derselben Website aufgegebene Bestellung erstellen können.
...
* **MDVA-34591** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Behebt das Problem mit einer falschen Warenkorbregelberechnung für *Maximaler Mengenrabatt wird angewendet auf* und *Rabattmengenschritt (Kaufen X)*.
...
* **MDVA-35569** (*für Adobe Commerce >=2.3.4 &lt;2.4.2*) - Behebt das Problem, dass das Feld *Feste Produktsteuern* in der GraphQL-Antwort nicht ausgefüllt wird, wenn der Status angegeben ist.
...
* **MDVA-31307** (*für Adobe Commerce >=2.4.0 &lt;2.4.2*) - Behebt das Problem, dass bei bestimmten Kategorien aufgrund von Problemen mit der dynamischen CSP-Whitelist für zwischengespeicherte Blöcke *Speicherfehler)*.
...
* **MDVA-34102** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Legt fest, dass die Anzahl der standardmäßigen Lagerbestände für deaktivierte Produkte auf den Produktraster- und Produktseiten im Admin-Bereich bei Null liegt.
* **MDVA-35286** (*für Adobe Commerce >=2.4.0 &lt;2.4.2*) - Behebt das Problem, dass ein Fehler auftritt, wenn ein Kunde gebündelte Produkte im Warenkorb hat und vom Checkout mit mehreren Adressen zum OnePage-Checkout wechselt.
...
* **MDVA-34012** (*für Adobe Commerce >=2.3.1 &lt;2.4.3*) - Das Problem wurde behoben, dass das Kontrollkästchen *Standardwert verwenden* nach Anwendung der geplanten Änderungen deaktiviert wurde. Das Problem tritt auf, wenn die geplanten Änderungen nicht mehr wirksam sind.
* **MDVA-35064** (*für Adobe Commerce >=2.3.3 &lt;2.4.3*) - Es wurde das Problem behoben, dass URL-Neuschreibungen für konfigurierbare Produkte, die über die API erstellt wurden, nicht generiert wurden.
* **MDVA-34943** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem Quick Order die zuvor eingegebenen SKUs zwischenspeichert.
...
* **MDVA-34867** (*für Adobe Commerce >=2.3.0 &lt;2.4.3*) - Es wurde das Problem behoben, bei dem Werte für ein Bedingungsfeld, das für ein geplantes Update festgelegt wurde, nicht gespeichert wurden.
* **MDVA-35092** (*für Adobe Commerce >=2.3.5 &lt;2.4.3*) - Es wurde das Problem behoben, durch das Benutzer aufgrund der veralteten [!DNL Vimeo]-API keine [!DNL Vimeo] hinzufügen können.

## v1.0.16 {#v1-0-16}

* **MDVA-33453** (*für Adobe Commerce >=2.3.6 &lt;2.4.3*) - Behebt das Problem, dass die Inhaltstyp-Vorschau von Page Builder-Produkten funktioniert, wenn übereinstimmende Produkte unterschiedliche Preise für jede Website haben.
...
* **MDVA-34192** (*für Adobe Commerce >=2.3.4 &lt;2.4.3*) - Das Problem, dass es nicht möglich ist, das Geburtsdatum des Kunden im Format TT/MM/JJJJ zu ändern/anzugeben, wurde behoben.
...
* **MDVA-33393** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Behebt den Fehler *Bereitgestellte countryId existiert nicht*.
...
* **MDVA-34469** (*für Adobe Commerce >=2.4.1 &lt;2.4.2*) - Es wird das Problem behoben, bei dem GraphQL-Mutationen im Warenkorb eines Kunden bei der Verwendung mehrerer Store-Ansichten fehlschlagen.
...
* **MDVA-33970** (*für Adobe Commerce >=2.3.4 &lt;2.4.2*) - Behebt das Problem, dass in der Gutschrift das falsche Währungszeichen angezeigt wird, wenn der Umfang des Preisattributs auf „Website“ festgelegt ist.
...
* **MDVA-33382** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Behebt die Probleme mit der Indexerinvalidierung nach dem Entfernen von Produkten aus einer Kategorie.
* **MDVA-28511** (*für Adobe Commerce >=2.3.5 &lt;2.3.6*) - Behebt das Problem, dass es nicht möglich ist, [!DNL PayPal] Auschecken abzuschließen, wenn das Feld „Name“ bestimmte Zeichen (z. B. Großbuchstaben mit Akzenten) enthält.
...
* **MDVA-33281** (*für Adobe Commerce >=2.3.4 &lt;2.3.6*) - Es wurde ein Problem behoben, bei dem in `inventory:reservation:list-inconsistencies` ein schwerwiegender Fehler aufgrund eines falschen SKU-Parametertyps auftrat.
* **MDVA-24201** (*für Adobe Commerce >=2.3.0 &lt;2.3.5*) - Behebt das Problem, dass Preise die Preisregel des geplanten Warenkorbs erst dann widerspiegeln, wenn sie manuell neu indiziert wurden.
* **MDVA-32694** (*für Adobe Commerce >=2.3.0 &lt;2.3.6 || >= 2.4.0 &lt;2.4.2*) - Behebt das Problem, dass ein Admin-Benutzer ein Produkt nicht zu einem verhandelbaren Angebot hinzufügen kann, wenn es mit einem nicht standardmäßigen Store verbunden ist.
...
* **MDVA-33975** (*für Adobe Commerce >=2.3.4 &lt;2.4.2*) - Behebt mehrere Probleme im Zusammenhang mit der Preisberechnung in GraphQL-Anfragen.

## v1.0.13 {#v1-0-13}

* **MDVA-30858** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Behebt das Problem, dass [!DNL PayPal] Abrechnungsberichte nicht wie erwartet unter **Berichte** > **Verkäufe** > **[!DNL PayPal]** Abrechnung verfügbar sind.
* **MCP-87** (*für Adobe Commerce >=2.3.1 &lt;2.4.2*) - Verbesserte Indexierungszeit für Produkt- und Aktienindizierer der Kategorie für große Profile.
* **MDVA-33106** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem die neu geplanten Produktänderungen nach Ausführung des Cron-`run` gelöscht wurden.
* **MDVA-19391** (*für Adobe Commerce >=2.3.0 &lt;2.3.5*) - Es wurde das Problem behoben, bei dem `analytics_collect_data` einen Fehler ausgibt, der auf NULL-Beschreibungseinträge in der `catalog_category_entity_text` zurückzuführen ist.
* **MDVA-20376** (*für Adobe Commerce >=2.3.2 &lt;2.3.4*) - Behebt das Problem mit dem Fehler *Keine solche Entität mit customerId = 1* in der `exception.log` für angemeldete Kunden nach der Bestellplatzierung.
...
* **MDVA-13203** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem der Fehler *Integritätsbeschränkung - Verletzung der search_tmp_-* nach einer vollständigen Neuindizierung angezeigt wurde.
...
* **MDVA-22150** (*für Adobe Commerce >=2.3.1 &lt;2.3.4*) - Behebt das Problem, dass Kunden mit einem konfigurierbaren Produkt im Warenkorb und einem angewendeten Coupon sich nicht anmelden können, wenn dieses konfigurierbare Produkt in Admin deaktiviert ist.
* **MDVA-32545** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Es wurde das Problem behoben, dass Rechnungen beim Erstellen von Bestellungen vom Administrator nicht automatisch versendet wurden.
...
* **MDVA-31399** (*für Adobe Commerce >=2.3.2 &lt;2.4.2*) - Addiert die *Zwischensumme (inkl. Steuer)* Option zu den Preisregelbedingungen.
* **MDVA-31236** (*für Adobe Commerce >=2.4.0 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem Administratoren mit benutzerdefiniertem Ressourcenzugriff keine 2FA einrichten oder sich anmelden konnten.
* **MDVA-30845** (*für Adobe Commerce >=2.3.5 &lt;2.3.7*) - Behebt das Problem, bei dem der *Es tut uns leid, für diese Bestellung sind derzeit keine Angebote verfügbar* Fehler angezeigt wird, wenn keine Verbindung zu UPS XML/USPS/DHL hergestellt werden kann und keine andere Versandmethode verfügbar ist.
...
* **MDVA-32632** (*für Adobe Commerce >=2.3.2 &lt;2.3.5*) - Behebt das Problem, dass Bestellungen im Zahlungssystem, aber nicht in Adobe Commerce erscheinen.
* **MDVA-32449** (*für Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || >=2.4.1 &lt;2.4.2 mit B2B-Erweiterung*) - Behebt das Problem, dass der Auftragsverlauf sehr langsam oder gar nicht geladen wird.
* **MDVA-32739** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Fehlerkorrektur - Durch die Aktivierung asynchroner E-Mail-Benachrichtigungen werden alte Verkaufs-E-Mails versendet.
...
* **MDVA-31168** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Behebt das Problem, dass die CSV-Datei des Produktexports nicht angezeigt wird und ein Fehler bei der Speicherzuweisung vorliegt.
...
* **MDVA-30428** (*für Adobe Commerce >=2.3.4 &lt;2.3.5*) - Es wurde das Problem behoben, dass Kunden kein Produkt auf die Wunschliste setzen können, wenn dieses Produkt einer benutzerdefinierten Inventarquelle zugewiesen ist.
...
* **MDVA-29148** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Behebt das Problem beim Erstellen eines Produkts über einen API-Aufruf. Das benutzerdefinierte Produktattribut vom Typ `\Magento\Eav\Model\Entity\Attribute\Backend\ArrayBackend` (z. B. Multiselect) verwendet nicht seinen Standardwert, wenn in der Payload kein Wert angegeben wurde.
* **MDVA-30837** (*für Adobe Commerce >=2.3.1 &lt;2.3.5*) - In der Konfiguration der Versandmethode wurde eine Konfigurationseinstellung *Steuer auf Betrag einschließen: Ja/Nein* hinzugefügt. Wenn *Steuer in Betrag einbeziehen* auf *Ja* gesetzt ist, wird der Mindestauftragsbetrag als Zwischensumme + Steuer berechnet. Wenn *Steuer in Betrag einbeziehen* auf &quot;*&quot; gesetzt*, wird der Mindestauftragsbetrag als Zwischensumme berechnet
* **MDVA-25028** (*für Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*) - Behebt das Problem, wenn Bestellungen, die mit [!DNL PayPal Payflow Pro] aufgegeben werden, nicht auf den Status „Verdächtiger Betrug“ gesetzt werden, wenn Betrugsfilter ausgelöst werden.
* **MDVA-31224** (*für Adobe Commerce >=2.3.3 &lt;2.3.5*) - Verbessert die Leistung des `catalog_product_price` Neuindizierungsvorgangs für Bundle-Produkte.
* **MDVA-31321** (*für Adobe Commerce >=2.3.2 &lt;2.3.5*) - Behebt eine leere Seite (Fehler), wenn *Alle anzeigen* ausgewählt ist. [!DNL Elasticsearch] gibt eine große Liste von Produkt-IDs zurück. In diesem Szenario wird die ORDER BY-Klausel in ein falsches SQL-Format konvertiert.
...
* **MDVA-28993** (*für Adobe Commerce >=2.3.4 &lt;2.4.0*) - Implementiert die Funktion *Minimum muss übereinstimmen* und die Teilsuche nach [!DNL Elasticsearch]. Löst Probleme mit Bindestrichen in Suchanfragen.
...
* **MDVA-30357** (*für Adobe Commerce >=2.3.2 &lt;=2.4.0*) - Behebt das Problem, dass beim Generieren einer Sitemap von Cron falsche Bild-URLs erstellt werden.
* **MDVA-30565** (*für Adobe Commerce >=2.3.2 &lt;=2.3.3-p1*) - Behebt das Problem, dass *Keine solche Entität mit cartid = 0*-Fehler für einen Gast-Kunden beim Checkout in der Storefront angezeigt wird, wenn der beständige Warenkorb aktiviert ist.
* **MDVA-29787** (*für Adobe Commerce >=2.3.0 &lt;=2.4.0*) - Behebt das Problem, dass die Zielregel für verwandte Produkte nicht funktioniert, wenn *eine der Bedingungen ist* zum Definieren der anzuzeigenden Produkte verwendet wird.
* **MDVA-30977** (*für Adobe Commerce >=2.3.4 &lt;=2.3.5-p2*) - Behebt das Problem, dass nach der Neuindizierung zufällige Produkte in den Kategorien fehlen.
...
* **MDVA-28300** (*für Adobe Commerce >=2.3.0 &lt;2.3.6*) - Behebt das Problem, dass die GQL-Anfrage die Preisänderungen aus den Katalogpreisregeln nicht widerspiegelt.
...
* **MDVA-30841** (*für Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - Behebt das Problem mit der mehrschichtigen Navigation, bei der der *No*-Wert für boolesche Produkteigenschaften nicht in die mehrschichtige Navigation aufgenommen wurde, wenn [!DNL Elasticsearch] als Suchmaschine verwendet wurde.
...
* **MDVA-29959** (*für Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 mit B2B-Erweiterung*) - Es wurde das Problem behoben, bei dem ein eingeschränkter Admin-Benutzer mit *Companies*-Berechtigungen das Unternehmenskonto nicht löschen darf.
...
* **MDVA-28409** (*für Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - Behebt das Problem, dass der `sales_clean_quotes` Cron-Vorgang mit dem Fehler *Nicht genügend Arbeitsspeicher* fehlschlägt, wenn die Anzahl der abgelaufenen Anführungszeichen in der Datenbank riesig ist.
...
* **MDVA-30107** (*für Adobe Commerce >=2.3.0 &lt;2.3.6*) - Es wurde das Problem behoben, dass der Store Switcher nicht wie erwartet funktioniert, wenn verschiedene Basis-URLs für Store-Ansichten verwendet werden.
* **MDVA-28763** (*für Adobe Commerce >=2.3.2 &lt;2.3.4*) - Es wurde das Problem behoben, dass das Produktbild nach der mehrmaligen Aktualisierung der Produktinformationen mit der REST-API dupliziert wurde.
* **MDVA-30284** (*für Adobe Commerce >=2.3.0 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem der Katalogsuchindizierer aufgrund des folgenden *[!DNL Elasticsearch]fehlschlägt: Die Gesamtfeldgrenze im Index wurde überschritten.*
...
* **MDVA-30428** (*für Adobe Commerce >=2.3.3 &lt;2.4.2*) - Es wurde das Problem behoben, dass Kunden kein Produkt auf die Wunschliste setzen können, wenn dieses Produkt einer benutzerdefinierten Inventarquelle zugewiesen ist.
* **MDVA-28661** (*für Adobe Commerce >=2.3.0 &lt;2.4.2 mit B2B-Erweiterung*) - Es wurde das Problem behoben, bei dem nach einer Änderung des Firmenadmins ein Fehler im Abschnitt Firmenkonto des Firmenbenutzers ausgelöst wurde.
...
* **MDVA-28656** (*für Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) - Behebt das Problem, dass, wenn eine Bestellung ohne erforderliche Zahlungsinformationen aufgegeben wurde (z. B. mit 100% Rabatt) und eine Rechnung für die Bestellung erstellt wurde, der Bestellstatus zu &quot;*Geschlossen* statt „Abgeschlossen“ wechselt.
...
* **MDVA-30123** (*für Adobe Commerce >=2.3.4 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem Attributoptionenbeschriftungen für GraphQL-Abfragen nicht korrekt übersetzt wurden.
* **MDVA-29996** (*für Adobe Commerce >=2.3.3 &lt;2.4.2*) - Es wurde das Problem behoben, dass nach der Aktivierung der Kategorienberechtigung die Kategorieseite nicht vom vollständigen Seiten-Cache zwischengespeichert wird.
* **MDVA-30164** (*für Adobe Commerce >=2.3.1 &lt;2.4.2*) - Es wurde das Problem behoben, bei dem Kundendatensätze nicht aus dem Raster „Kunden“ exportiert werden können, wenn benutzerdefinierte Kundenattribute vorhanden sind.
* **MDVA-30444** (*für Adobe Commerce >=2.3.0 &lt;2.4.1*) - Es wurde das Problem behoben, dass bei Bestellungen, die mit GraphQL aufgegeben wurden, keine Bestätigungs-E-Mail gesendet wurde.
...
* **MDVA-29965** (*für Adobe Commerce >=2.3.3 &lt;2.4.0*) - Es wurde das Problem behoben, bei dem Kunden beim Hinzufügen eines Produkts zum Warenkorb den Fehler „Ungültiger Formularschlüssel“ erhalten.
...
* **MC-35984** (*für Adobe Commerce >=2.4.0 &lt;2.4.1*) - Behebt das Problem, dass Händler nach dem Erstellen eines Versandtitels für eine Rücksendeautorisierung (Return Merchandise Authorization, RMA) nicht mit Seitenelementen auf der Rückgabeseite interagieren konnten.
...
* **MDVA-27825** (*für Adobe Commerce >=2.3.0 &lt;2.4.1*) - Es wurde das Problem behoben, bei dem der Export großer Datenmengen aufgrund von Speicherleck fehlschlug.
* **MDVA-29085** (*für Adobe Commerce >=2.3.0 &lt;=2.3.5-p1*) - Behebt das B2B-Problem, bei dem keine erforderlichen neuen Unternehmens-E-Mails gesendet werden, wenn ein Unternehmen durch API erstellt wird.
* **MDVA-29344** (*für Adobe Commerce >=2.3.5 &lt;=2.4.0-p1*) - Behebt das Problem, dass Page Builder nach dem Kopieren von Text aus einem Kopfzeilenelement in ein Textelement hängen bleibt.
* **MDVA-29835** (*für Adobe Commerce >2.3.1 &lt;2.4.2*) - Es wurde das Problem behoben, dass bei Geschenkkartenbestellungen zwei Codes anstelle eines Codes enthalten waren.
* **MDVA-30052** (*für Adobe Commerce >=2.3.2-p2 &lt;2.3.5*) - Behebt das Problem, dass der private Inhalt (lokaler Speicher) nicht korrekt ausgefüllt wurde, was zu Leistungsproblemen führte.
* **MDVA-30131** (*für Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - Behebt das Problem mit der mehrschichtigen Navigation, bei der der *No*-Wert für boolesche Produkteigenschaften nicht in die mehrschichtige Navigation aufgenommen wurde, wenn [!DNL Elasticsearch] als Suchmaschine verwendet wurde.
* **MDVA-35514** (*für Adobe Commerce >=2.4.0 &lt;2.4.1*) - Behebt das Problem beim Erstellen einer Versandkennzeichnung und beim Hinzufügen bestellter Produkte zu einem Paket im modalen Fenster Pakete erstellen .
