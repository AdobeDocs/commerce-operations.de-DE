...
* **ACSD-55566** (für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.7) - Behebung des Problems, bei dem die `mergeCart` Mutation schlägt mit einem &quot;*Interner Server-Fehler*&quot; in der [!DNL GraphQL] Antwort beim Zusammenführen von Quell- und Zielcarts, die dieselben Bundle-Elemente aufweisen.
* **ACSD-56546** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem konfigurierbare und Bundle-Produkte als **Nicht vorrätig** auf der Storefront, wenn die **Anzeigen außerhalb der Produktkonfiguration** is *Behinderte*.
* **ACSD-56635** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem der importierte Kunde mit derselben E-Mail-Adresse dupliziert wird, wenn ein Import mit **Kontofreigabe** auf *Global*.
* **ACSD-56741** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebt die Fehlermeldung &quot;*Versuch, auf den Array-Versatz für den Wert des Typs null zuzugreifen*&quot;, das während `setup:upgrade` wenn die Datenbank eine benutzerdefinierte [!DNL MySQL] Trigger, der nicht mit dem Indexierungsmechanismus in Zusammenhang steht, und [!DNL MView].
* **ACSD-57315** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems bei der Erstellung einer neuen Transaktion in [!DNL PayPal Payflow Pro] jedes Mal [!UICONTROL Fetch] auf die Schaltfläche **[!UICONTROL View transaction]** im Admin-Bildschirm angezeigt.
* **ACSD-57337** (für Adobe Commerce >=2.4.4 &lt;2.4.6) - Behebung des Problems, bei dem ein Admin-Benutzer mit Zugriffsbeschränkungen auf bestimmte Websites Unternehmen von allen Websites in der **[!UICONTROL Companies]** Gitter.
* **ACSD-57394** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems der falschen Sortierung nach mehreren Sortierungsfeldern in [!DNL GraphQL].
* **ACSD-57565** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem die **[!UICONTROL Order]** -Dashboard zeigt fehlerhafte Bestellinformationen an, bis der Zeitraum aktualisiert wurde. Das Dashboard zeigt nun die richtigen Bestellstatistiken beim ersten Laden an.
* **ACSD-57854** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebt das Problem beim Produkt [!DNL GraphQL] -Anfragen gaben deaktivierte Kategorien in den Kategorieaggregationen zurück.
...
* **ACSD-46767** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.6) - Behebung des Problems, bei dem die Kategorieseite invalidiert, wenn sich die Lagermenge ändert, selbst wenn das Produkt noch auf Lager ist.
* **ACSD-54656** (für Adobe Commerce >=2.4.5 &lt;2.4.6) - Behebung des Problems, bei dem das unsichtbare Recaptcha beim Checkout fehlschlägt, wodurch verhindert wurde, dass eine Bestellung platziert wird.
...
* **ACSD-56842** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem die verzögerten Proxys und die verzögerten Proxy-Fabriken nach der Ausführung fehlen `setup:di:compile`.
* **ACSD-57003** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem der Auftragsstatus in geändert wird *[!UICONTROL Complete]* anstatt geändert zu werden in *[!UICONTROL Processing]* wenn eine Bestellung teilweise zurückerstattet und teilweise versandt wird.
...
* **ACSD-56790** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem ein Benutzer beim Sortieren von Kategorieprodukten mit dem **Aus Lager nach unten verschieben** und die `Invalid security or form key. Please refresh the page` wird oben auf dem Bildschirm angezeigt.
* **ACSD-56280** (für Adobe Commerce >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem die Bestellung von Artikeln aus einer Geschenkregistrierung zu einer Ausnahme führt.
...
* **ACSD-55427** (für Adobe Commerce >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem der Admin-Benutzer die Zuweisung eines Produkts aus einem freigegebenen Katalog nicht von der Produktseite in Admin aufheben kann.
...
* **ACSD-55231** (für Adobe Commerce >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem Sie mit der Funktion für die schnelle Bestellung keine Produkte zu einem Warenkorb hinzufügen können.
* **ACSD-54283** (für Adobe Commerce >=2.4.3 &lt;2.4.4) - Behebung des Problems, bei dem Produkte/Kategorien, die nicht dem freigegebenen Katalog für die Standardgruppe (Allgemeine Gruppe) zugewiesen sind, weiterhin in der XML-Sitemap enthalten sind.
...
* **ACSD-54885** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebt das Problem mit dem Checkout für mehrere Adressen, wenn der Administrator die *Als Kunde anmelden* Funktionalität.
...
* **ACSD-55334** (für Adobe Commerce und Magento Open Source >=2.4.3 &lt;2.4.7) - Behebt Übersetzungen für Beschriftungen über Übersetzungswörterbücher in der GraphQL-Antwort.
...
* **ACSD-53925** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem der Administrator den CMS-Block nicht mit dem Produktkarussell speichern kann, wenn `catalog_product_price` dimensions-mode auf *website*.
* **ACSD-52714** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem der Datumsfilter im Admin-Raster nicht funktionierte, wenn das Datumsformat als *Y-m-d*.
* **ACSD-55055** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Erhöht die Leistung beim Laden von Produktattributen in Warenkorbpreisregeln im Warenkorb.
...
* **ACSD-54983** (für Adobe Commerce >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem das Abrufen der Benutzer-UID des Unternehmens mit der GraphQL-Anforderung nicht möglich ist, wenn der Benutzerstatus auf *[!UICONTROL Inactive]*.
* **ACSD-53309** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems, bei dem die Steuer im *[!UICONTROL Regular Price]* beschriften, wenn die anpassbare Option ausgewählt ist.
* **ACSD-55305** (für Adobe Commerce >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem die *[!UICONTROL Edit Company User]* Popup im **[!UICONTROL myAccount]** > **[!UICONTROL Company Structure]** Seite friert mit einem Lader auf dem Bildschirm ein.
* Aktualisierte Patches: ACSD-49013

## v1.1.42 {#v1-1-42}

* **ACSD-53658** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem *[!UICONTROL Recently Viewed]* Produktdaten werden in der Store-Ansicht nicht ordnungsgemäß aktualisiert.
* **ACSD-54626** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem Sie keine neue Bestellregel erstellen können (`createPurchaseOrderApprovalRule`) durch die `NUMBER_OF_SKUS` Attribut über [!DNL GraphQL].
* **ACSD-53845** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebt die [!DNL MySQL] Verbindungs-Timeout-Problem bei `consumer max_messages` = 0.
* **ACSD-54890** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem `aggregate_sales_report_bestsellers_data` verursacht [!DNL MySQL] Fehler aufgrund `/tmp` Speicherplatz vergeudet.
* **ACSD-55112** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem die *[!UICONTROL Submit review]* -Schaltfläche kann mehrmals ohne [!DNL Google reCAPTCHA v3] Validierung.
...
* **ACSD-54418** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem ein fester Rabatt fälschlicherweise auf jedes untergeordnete Produkt des dynamisch primierten Bundles angewendet wird.
...
* **ACSD-48910** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.6) - Es wurde ein Problem behoben, bei dem ein Bundle-Produkt, das mehreren Quellen zugewiesen wurde, nach der Fakturierung und dem Versand einer Bestellung nicht mehr vorrätig war, selbst wenn es noch eine Menge ungleich null hat.
* **ACSD-55381** (für Adobe Commerce >=2.4.2 &lt;2.4.7) - Behebung eines internen Serverfehlers bei der Abfrage `configurable_product_option_uid` und `configurable_product_option_value_uid` -Felder aus einem [!DNL B2B] *[!UICONTROL Requisition list]* via [!DNL GraphQL].
...
* **ACSD-54680** (für Adobe Commerce >=2.4.0 &lt;2.4.6) - Behebung des Problems, bei dem es nicht möglich ist, ein B2B-Zitat für ein Produkt mit mehreren zugewiesenen Quellen zu verarbeiten.
* **ACSD-54040** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5) || >=2.4.5-p4 &lt;2.4.6) - Behebung des Problems, bei dem die *[!UICONTROL Created]* -Feld in den Bestelldetails leer ist, wenn B2B-Module aktiviert sind.
* **ACSD-54319** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.6) - Behebung des Problems, bei dem der Produktpreis im *[!UICONTROL Product in Cart]* Bericht.
* **ACSD-53378** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Verbessert die Ladezeit für Kassen-Seiten für Kunden mit großen Adressbüchern.
* **ACSD-52657** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem der Minicart nicht im sekundären Store aktualisiert wird, der eine Subdomain verwendet.
* **ACSD-53414** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem ein eingeschränkter Administrator CMS-Seiten außerhalb des Berechtigungsbereichs anzeigen kann.
* **ACSD-54472** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem Kunden eines abgelehnten Unternehmens sich weiterhin authentifizieren können und Kunden eines blockierten Unternehmens und eines abgelehnten Unternehmens weiterhin Bestellungen aufgeben können. Der Patch fügt zusätzliche Validierungen für GraphQL-Endpunkte hinzu.
...
* **ACSD-55004** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem der Validator beim Hochladen einer Importdatei abstürzt, die größer ist als der in `php.ini`.
* **ACSD-54989** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5) || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - Behebung des Problems, bei dem ein Unternehmensadministrator keine Bestellung aufgeben kann, wenn *[!UICONTROL Enable Purchase Orders]* auf *[!UICONTROL Yes]* und *[!UICONTROL Purchase Order]* auf *[!UICONTROL No]*.
* **ACSD-54007** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebt den Fehler *&quot;Undefinierter Array-Schlüssel &quot;_scope&quot;* über den Import von Kundendaten.
* **ACSD-55031** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.6) - Behebt die *Typ &quot;gemischt&quot;kann nicht null sein* Fehler während der Kompilierung.
* **ACSD-54961** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem ein eingeschränkter Administrator die Massenaktualisierung der *Produktprüfung* -Status.
* **ACSD-55256** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem nur das erste Bild erfolgreich im Bildregler angezeigt wurde.
* Aktualisierte Patches: ACSD-52041, ACSD-54106

## v1.1.39 {#v1-1-39}

* **ACSD-53704** (für Adobe Commerce >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem der Verlauf des Guthabenbilds nach Ablauf der Bonuspunkte falsch berechnet wird.
...
* **ACSD-54026** (für Adobe Commerce >=2.4.6 &lt;2.4.7) - Behebt eine falsche Fehlermeldung für eine `updateCompanyRole` GraphQL-Anfrage an einen nicht autorisierten Benutzer.
* **ACSD-54106** (für Adobe Commerce und Magento Open Source >=2.4.1 &lt;2.4.5) - Behebung des Problems, bei dem die Sortierung von Kategorieprodukten nach Namen für türkische Akzentzeichen falsch ist.
* **ACSD-52219** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem gespeicherte Filter von Admin-Rastern nicht wie erwartet funktionieren, wenn häufig zwischen Lesezeichenansichten gewechselt wird.
* **ACSD-54342** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebt eine falsche Fehlermeldung *Fehler in der Datenstruktur: Werte werden gemischt* wenn eine CSV-Datei ohne gültige Daten importiert wird.
* **ACSD-54660** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.6) - Es wurde ein neues Eingabeattribut hinzugefügt. *sort* Sortieren von Kundenbestellungen in GraphQL nach `sort_field` und `sort_direction`.
* **ACSD-54776** (für Adobe Commerce >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem die Option deaktiviert war *[!UICONTROL Use Default Value]* und nicht standardmäßige Produktfeldwerte werden nicht für die zweite Website-, Store- und Store-Ansicht gespeichert.
* **ACSD-53998** (für Adobe Commerce und Magento Open Source >=2.4.4-p2 &lt;2.4.5) || >=2.4.5-p1 &lt;2.4.7) - Behebung des Problems, bei dem ein **[!UICONTROL Dynamic Block]** auf der Grundlage einer **[!UICONTROL Customer Segment]** funktioniert nicht ordnungsgemäß, nachdem sich ein Kundenkonto abgemeldet hat.
* **ACSD-53204** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Fehlerbehebungen *Das Produkt kann nicht gespeichert werden.* Fehler bei gleichzeitigen Anfragen zum Hinzufügen von Bildern zur Produktgalerie mithilfe der `rest/V1/products/<sku>/media` -Endpunkt.
...
* **ACSD-52613** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem Cache und Indizes aktualisiert werden, selbst wenn keine Aktualisierungen an vorgenommen werden `Inventory_source` -Elemente nach der REST-API.
...
* **ACSD-53628** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Behebung des Problems, bei dem der CSV-Bericht zu Verkaufsstellen falsche Sonderzeichen anzeigt.
...
* **ACSD-52606** (für Adobe Commerce und Magento Open Source >=2.4.0 &lt;2.4.7) - Behebung des Problems, bei dem die Fehlermeldung *Ihre Bestellung ist noch nicht bereit zur Übernahme* angezeigt wird, wenn der Benutzer auf **[!UICONTROL Notify Order is Ready for Pickup]**.
...
* **ACSD-53728** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.7) - Behebung des Problems, bei dem der Produkt-EAV-Indexer länger dauert.
...
* **ACSD-52085** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem ein konfigurierbares Produkt mit einem Sonderpreis nicht im Karussell des Produkts sichtbar ist.
* **ACSD-53795** (für Adobe Commerce und Magento Open Source >=2.4.2 &lt;2.4.7) - Behebung des Problems mit einem ungültigen Datentyp in `indexer_update_all_views` Cron-Auftrag.
...
* **ACSD-53750** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebt die *Beschädigte Leitung oder geschlossene Verbindung* Fehler bei mehrprozessgestützten `catalog_product_price` reindex.
* **ACSD-49843** (für Adobe Commerce und Magento Open Source >=2.3.7 &lt;2.4.0) || >=2.4.1 &lt;2.4.7) - Behebung des Problems, bei dem der Link beim Produkt-Download nicht verfügbar ist, nachdem der bestellte Artikel automatisch von der Online-Zahlungsmethode mit dem **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** -Einstellung aktiviert.
