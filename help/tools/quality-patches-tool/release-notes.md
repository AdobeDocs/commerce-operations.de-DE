...
* **ACSD-63244** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem ein [!DNL JavaScript] -Fehler verhindert, dass [!DNL Google Maps] korrekt dargestellt wird. Behebung des Problems, bei dem viele *uncaught TypeError vorhanden sind: dies._each ist keine Fehler der Funktion* in der Konsole im Bedienfeld [!UICONTROL Admin].
* **ACSD-63242** (für Adobe Commerce und Magento Open Source >=2.4.6-p8 &lt;2.4.7) || >=2.4.7-p3 &lt;2.4.8) - Behebung des Problems mit der Langsamkeit des Imports beim Hinzufügen von Katalogprodukten mit mehr als 10.000 Einträgen.
* **ACSD-63062** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebung des Problems, bei dem falsche Rabattberechnungen für den Warenkorb bei Anwendung mehrerer überlappender Regeln auftreten.
* **ACSD-62979** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem die Verwendung des falschen [!UICONTROL Store ID] in der [!DNL GraphQL]-Kopfzeile einen schwerwiegenden Speicherfehler verursacht.
* **ACSD-62971** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem beim Import von Lagerquellen mit nicht numerischen Werten in der Spalte **quantity** der Wert **quantity** auf *0* eingestellt wurde.
* **ACSD-62872** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems mit der eindeutigen Attributvalidierung, bei dem Zeitplanaktualisierungen falsch validiert werden.
* **ACSD-62755** (für Adobe Commerce und Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - Behebung des Problems, bei dem für [!DNL TinyMCE] 7 die Schriftgröße und -schrift speziell in den Initialisierungseinstellungen des Editors hinzugefügt werden muss.
* **ACSD-62670** (für Adobe Commerce und Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - Behebung des Problems, bei dem der Export von [!UICONTROL Products Ordered] Berichten nach [!DNL CSV] und [!DNL XML] einen Fehler zurückgibt.
* **ACSD-62577** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - behebt das Problem mit der langsamen Leistung von Storefront-Suchabfragen durch Optimierung von Abfrage- und Tabellenindizes.
* **ACSD-62475** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebung des Problems, bei dem die [!UICONTROL Gift Card] -Produkte falsch im Warenkorb zusammengeführt wurden.
* **ACSD-62428** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem `is_out_of_stock` im Katalogsuchindex auf einen falschen Wert gesetzt ist, wenn der [!DNL SKU] nicht als durchsuchbares Attribut festgelegt ist.
* **ACSD-62355** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.8) - Erhöht die Ladezeit der konfigurierbaren Produktebearbeitungsseite, wenn das konfigurierbare Produkt auf vielen Attributen mit vielen Werten basiert.
* **ACSD-61805** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem die Produkte nach der Aktualisierung des Status der Rückbestellung über den [!DNL REST API] nicht auf Lager sind.
* **ACSD-60811** (für Adobe Commerce und Magento Open Source >=2.4.7 &lt;2.4.8) - Behebung des Problems, bei dem das Aktualisieren des Bestellstatus mit einem benutzerdefinierten Wert oder Kommentar nur möglich ist, wenn der aktuelle Status entweder *Verarbeitung* oder *Betrug* lautet.
* **ACSD-62952** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem das Datum [!UICONTROL Gift Registry] im Storefront ungenau angezeigt wird.
* **ACSD-55339** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem ein Produkt [!DNL SKU], das mit &quot;0&quot;beginnt (null), den Wert &quot;0&quot;entfernt, sodass das Anführungszeichen nicht aktualisiert wird.
**
* Aktualisierte Patches: **ACSD-59514**
* Versionen aktualisiert: **ACSD-60816**
* Ersetzte Patches: **ACSD-59967**
...
* **ACSD-60267** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem die korrigierte Produktsteuer (FPT) ordnungsgemäß angewendet wird, wenn einfache Produkte mit FPT direkt zum Warenkorb hinzugefügt werden, aber bei der Auswahl dieser Produkte über konfigurierbare Produktoptionen fehlschlägt.
* **ACSD-61103** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem die Fehleranzahl in der Tabelle `customer_entity` nicht auf null zurückgesetzt wird, nachdem sich ein Kunde erfolgreich über API-Endpunkte angemeldet hat.
* **ACSD-61134** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - behebt das Problem, dass die [!DNL Braintree Vault]-Zahlungsmethode im Checkout-Workflow automatisch deaktiviert wird, wenn ein Käufer seine Rechnungsadresse aktualisiert, indem das Kontrollkästchen *[!UICONTROL My billing and shipping address are the same]* deaktiviert wird.
* **ACSD-61199** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem die CMS-Registerkarte für die Seitenhierarchie beim Bearbeiten einer CMS-Seite mit einer vorhandenen Hierarchie keine ordnungsgemäße Baumstruktur anzeigt.
* **ACSD-61200** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem bei den Umsatzberechnungen für *[!UICONTROL Total Amount]* und *[!UICONTROL Total Amount Actual]* die Werte *[!UICONTROL Discount Tax Compensation Amount]* und *[!UICONTROL Shipping Discount Tax Compensation Amount]* fehlen, was zu Diskrepanzen bei den Daten der Verkaufsaufträge führt.
* **ACSD-61522** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem es möglich ist, E-Mail-Adressen in die Felder *[!UICONTROL First Name]* und *[!UICONTROL Last Name]* des Gastkunden einzugeben und ungültige E-Mails zur Bestellbestätigung zu senden.
* **ACSD-61756** (für Adobe Commerce >=2.4.4 &lt;2.4.7) - Verbessert die Leistung von `AdvancedSalesRule` Filtern.
* **ACSD-61799** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.5) - Behebung des Problems, bei dem der Gesamtrabatt falsch berechnet wird, wenn mehrere Warenkorbregeln mit fixen Rabatten auf das Angebot angewendet werden.
...
* **ACSD-48318** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem eine Verschachtelung der Umgebung nicht zulässig ist. Jetzt beginnt die Emulation während des `send()` -Aufrufs, sobald die Emulation während des `getInfoBlockHtml()` -Aufrufs beendet wird.
* **ACSD-59930** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - verbessert die Leistung der Flüsse **[!UICONTROL Create]**, **[!UICONTROL Save]** und **[!UICONTROL Delete]** des Unternehmens.
* **ACSD-60584** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem ein Zugriffstoken, das für den Benutzer auf einer Website erstellt wurde, berechtigt ist, auf Kundeninformationen auf anderen Websites zuzugreifen oder diese zu ändern.
* **ACSD-60804** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem beim Bearbeiten eines mit einem gelöschten Unternehmen verknüpften Kunden der Fehler *Aufruf der Mitgliederfunktion `getSuperUserId()` bei null* auftritt.
* **ACSD-61133** (für Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.8) - Behebung des Problems, bei dem die `sales_clean_quotes` [!DNL cron] Anführungszeichen aus nicht genehmigten Bestellungen löscht.
* **ACSD-61528** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebung des Problems, bei dem beim Abrufen von Rollen aus dem [!UICONTROL Admin] mit [!DNL GraphQL] keine Ergebnisse zurückgegeben werden.
* **ACSD-61553** (für Adobe Commerce und Magento Open Source >=2.4.5 &lt;2.4.7) - Behebung des Problems, bei dem **[!UICONTROL Cart Price Rule]** Rabatte falsch berechnet werden, wenn mehrere Rabatte mit unterschiedlichen Prioritäten und **[!UICONTROL Maximum Qty Discount is Applied To]** auf das Produkt angewendet werden.
* **ACSD-61667** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Verbessert die Lagerbestandsleistung für die Erstellung des Versands bei vielen Quellen mit In-Store-Abruf.
* **ACSD-61969** (für Adobe Commerce >=2.4.7 &lt;2.4.8) - Behebung des Problems, bei dem der Benutzer einen Gutscheincode eingeben muss, der zwischen Groß- und Kleinschreibung unterschieden werden muss, damit er genau mit dem Gutscheincode übereinstimmt.
* Versionen aktualisiert: **ACSD-54989**, **ACSD-60632**

## v1.1.52 {#v1-1-52}

* **BUNDLE-3375** (für Adobe Commerce und Magento Open Source) - Fügt alle erforderlichen Felder hinzu, um die Anforderungen an das 3DS-VISA-Mandat bei Verwendung von [!DNL Braintree] als Zahlungs-Gateway zu erfüllen.
* **ACSD-59366** (für Adobe Commerce >=2.4.6 &lt;2.4.8) - Behebung des Problems, bei dem ein Fehler auftritt, wenn versucht wird, ein Team zu löschen, das deaktivierte Benutzer enthält, die nicht in der Teamliste angezeigt werden.
* **ACSD-59865** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Behebung des Problems, bei dem ein [!UICONTROL Cart Price Rule] die zuvor angewendeten Regeln nicht aufhebt, wenn die Produktmenge im Warenkorb nicht ausreicht, um die Regeln anzuwenden.
* **ACSD-59925** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.8) - Behebung des Problems mit der Sortierung von Elementen in der [!UICONTROL Media Gallery] nach Position in GraphQL.
* **ACSD-59952** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem beim Erstellen einer [!UICONTROL Shared Catalog] mit einer Gruppen-ID, die einer vorhandenen [!UICONTROL Shared Catalog] zugewiesen ist, ein Fehler auftritt.
* **ACSD-60590** (für Adobe Commerce und Magento Open Source >=2.4.4 &lt;2.4.7) - Verbessert die Leistung beim Generieren von [!UICONTROL Bestsellers Aggregated Daily Reports] für eine große Anzahl von aufgegebenen Bestellungen.
* **ACSD-60673** (für Adobe Commerce >=2.4.4 &lt;2.4.8) - Behebung des Problems, bei dem die [!UICONTROL Cart Price Rule] für mehrere Zahlungsmethoden beim Checkout nicht angemessen auf die spezifische Zahlungsmethode angewendet wird.
* **ACSD-60684** (für Adobe Commerce und Magento Open Source >=2.4.6 &lt;2.4.7) - Behebung des Problems, bei dem die GraphQL-Sortierung nach mehreren Feldern nicht wie erwartet funktioniert.
