...
# ACSD-67089: `inventory/export-stock-salable-qty` API gibt beim Festlegen von `page_size` falsche `total_count` zurück
...
Die `inventory/export-stock-salable-qty`-API gibt beim Festlegen von `page_size` einen falschen `total_count` zurück.
...
1. Ändern Sie `page_size` in *5* und senden Sie eine GET-Anfrage an:
