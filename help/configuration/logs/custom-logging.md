...
description: Erfahren Sie, wie Sie Fehler mithilfe benutzerdefinierter dateibasierter Protokollierung in Adobe Commerce untersuchen können, einschließlich der Einhaltung von PSR-3 und Überlegungen zur zentralisierten Protokollierung.
...
- Die Verarbeitung von Protokolldaten wird durch spezielle Tools wie [Logstash](https://www.elastic.co/products/logstash), [Logplex](https://devcenter.heroku.com/articles/logplex) oder [fluentd](https://www.fluentd.org/) effektiver, ohne dass sich dies auf den Produktionsserver auswirkt.
...
Der [PSR-3-Standard](https://docs.laminas.dev/laminas-log/) definiert eine gemeinsame PHP-Schnittstelle für die Protokollierung von Bibliotheken. Das Hauptziel von PSR-3 besteht darin, Bibliotheken zu ermöglichen, ein `Psr\Log\LoggerInterface` Objekt zu empfangen und Protokolle auf einfache und universelle Weise darauf zu schreiben.
...
Commerce 2 erfüllt den PSR-3-Standard. Standardmäßig verwendet Commerce [Monolog](https://github.com/Seldaek/monolog). Monolog wird als Voreinstellung für `Psr\Log\LoggerInterface` im Commerce-Programm-[`di.xml`](https://github.com/magento/magento2/blob/2.4/app/etc/di.xml#L9) implementiert.
