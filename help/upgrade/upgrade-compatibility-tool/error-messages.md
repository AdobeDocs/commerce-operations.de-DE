...
- **Fehlerempfohlene Aktion**: Enthält ggf. Anleitungen zur Fehlerbehebung und Fehlerbehebung.
...
| 2002 | Die Kerndatei wurde geändert. | Führen Sie die `composer install` aus dem Stammverzeichnis des Projekts. |
...
| 1410 | Adobe Commerce-Methode ohne Aufruf | Aktualisieren Sie den Code, um eine Klasse zu verwenden, die als `@api`. |
...
| 5010 | Vorlage `@vars` Kommentarblock enthält ungültige JSON | Fehlerbehebung für ungültige JSON. |
...
| 5026 | Nicht anwenden `"text/javascript"` Typattribut | Verwenden Sie nur öffentliche Mitglieder. |
...
| 5048 | Klasse `Magento\Framework\View\Element\UiComponent\ArrayObjectFactory` ist eingeschränkt | Vorgeschlagener Ersatz: Factory, die eine benutzerdefinierte Klasse erstellt, erweitert von `ArrayObject` mit überschriebenen Serialisierungs-/Deserialisierungs-Methoden. |
...
Wenn Verweise auf veralteten Code erkannt werden, werden benutzerdefinierte Code-Warnungen ausgelöst. Solche Verweise sollten durch die unterstützten Erweiterungspunkte ersetzt werden. Achten Sie auf die `@see` Anmerkung des veralteten Elements für Empfehlungen. Diese Fehler werden auch gemeldet, wenn kleinere Kodierungsstandards verletzt wurden.
...
| 1132 | Importieren von Adobe Commerce `@deprecated` class | Die erweiterte Klasse wird in den kommenden Versionen entfernt. Erwägen Sie die Verwendung der Adobe Commerce-Klasse, die als `@api` anstatt. |
...
| 1439 | Adobe Commerce aufrufen `@deprecated` method | Die veraltete Methode wird in den kommenden Versionen entfernt. Erwägen Sie stattdessen, sich auf Methoden zu verlassen, die in API-Schnittstellen deklariert sind. |
...
| 1534 | Verwenden von Adobe Commerce `@deprecated` property | Die veraltete Methode wird in den kommenden Versionen entfernt. Erwägen Sie stattdessen, sich auf Methoden zu verlassen, die in API-Schnittstellen deklariert sind. |
...
| 1536 | Zuweisung von Adobe Commerce `@deprecated` property | Die veraltete Methode wird in den kommenden Versionen entfernt. Erwägen Sie stattdessen, sich auf Methoden zu verlassen, die in API-Schnittstellen deklariert sind. |
...
| 5074 | Verwendung der veralteten Methode `getResource()` , um erkannte Daten zu speichern/zu laden/zu löschen. | Verwenden Sie stattdessen ein Repository. |
